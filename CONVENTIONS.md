...
distribution が kind URI、JSON-LD descriptor、TypeScript helper、provider
...
- **Alias resolution は operator-owned。** `worker` などの short alias は
  ...
  する。
- **Kind descriptor metadata は operator tooling の入力。** JSON-LD descriptor
  は spec / publishes / listens / outputs / aliases を定義し、docs や provider
  package が参照する。
- **Materializer 実装が runtime 挙動を持つ。** 新 kind は任意 domain の URI +
  JSON-LD descriptor metadata + implementation binding で publish する。private
  な in-process-only distribution でも descriptor record は operator registry
  に置き、 public に共有する場合は JSON-LD として公開する。Takosumi reference
  kernel では binding を `KernelPlugin` で表す。
...
- Provider integration check env names use `TAKOSUMI_PLUGIN_*`.
...
`outputs` (apply の戻り値) は kind descriptor metadata の `publishes[]`
に登録された material として namespace registry に publish
される。フィールド名と型は同じ kind URI の provider 横断で stable にする。
...
2. JSON-LD descriptor metadata を publish する。takosumi.com reference
   descriptor examples に追加する場合は
...
- 新 component kind は **URI + JSON-LD descriptor metadata + implementation
  binding** で publish する。
