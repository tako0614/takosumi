...
Shape catalog は Takosumi が curate する (kernel が known shape として扱える
contract を維持するため)。 第三者は provider か template を増やす。 Shape
を増やす場合は ecosystem RFC が必要。
...
template は既存 Shape / Provider の合成だけで作れる (新 Shape は増やさない)。
これは kernel `POST /v1/deployments` の入力ではなく、 installer / compiler layer
が deploy 前に `resources[]` へ展開するための authoring helper。
...
を呼ぶか、 bundled template として `mod.ts` 一括 register に追加する。 kernel
boot path では template を register しない。

### 6. 既存 docs を更新

[Templates](/reference/templates) ページの bundled list に新 template
を追記する。
...
4. **≥ 2 provider を実装** — portability 不変式: 1 shape = 最低 2 provider。 1
   つの cloud に縛られた Shape は portable とは呼ばないため reject される。
...
複数 provider が同じ output shape を返すために、 field 名と型は shape
...
`post-activate-hook@v1` のような shape を `resources[]` として publish
...
kernel 側で追加できるのは desired-state resource shape と provider plugin まで。
template expansion は installer/compiler layer、 workflow runner や scheduler は
above-kernel product の責務。 詳細は
