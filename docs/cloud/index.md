...
Cloud-only fetch handlers は `takosumi-cloud/platform/worker.ts` で mount され、
OSS platform の `cloud_extensions` seam に追加されます。AI Gateway、
Cloudflare Workers-compatible profile、S3-compatible endpoint、Cloud usage endpoint は
別 Worker として deploy されません。
