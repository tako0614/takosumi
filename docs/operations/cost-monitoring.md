...
- **Billing account / payer**: operator account plane が管理する legal payer /
  payment processor / entitlement record。Space billing に紐づくが、Run gate の
  正本ではありません。

billing line item は Space 単位で集計します。Installation id は runner minutes、
managed resource usage、artifact / backup storage、egress などの内訳分析に使います。
...
| `enforce`  | hosted SaaS 用。plan 後に credit reservation を作り、approval/apply で reservation を検証する。 |
...
| Managed resource usage          | Takosumi platform worker           | provider bill export joined to Space / Installation attribution |
| Artifact / backup storage usage | Takosumi platform worker           | R2 object inventory joined to Space / Installation attribution  |
| Workload usage report ingest    | operator account-plane extension   | workload usage reports idempotently joined to Space use         |
...
3. Workload Service usage report の idempotency key と Space / Installation mapping。
...
| Installation spend spike | Installation-attributed spend > 3x its 30-day p95 daily spend | inspect abuse, quota, or noisy workload         |
...
hosted billing `enforce` を有効化する前に、Takosumi 側に cost attribution
dashboard artifact と validator を追加し、その validator が本 runbook の metric
contract と dashboard reference を検証する状態にします。validator が未整備の
間は `showback` までに留め、credit 不足で apply を止める `enforce` mode を
公開しません。
