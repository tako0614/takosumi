...
The first command builds `1.1.4` in a temporary directory and compares the
...
writes `tmp/provider-compatibility/1.1.4-state-proof.json` and its SHA-256
...
```bash
bun run provider:compatibility:state-proof -- --evidence /operator/evidence/provider-1.1.4-proof.json
bun run provider:compatibility:release-check -- --evidence /operator/evidence/provider-1.1.4-proof.json
```
...
five resources and nine fields stay only in the `1.1.4` minor candidate. The
...
The current corrected version is `1.1.4` and remains unpublished. The signed
...
before artifact creation because the candidate job lacked OpenTofu. Signed
`provider/v1.1.3` then passed the exact OpenTofu checks, all quality gates, the
deterministic build, and artifact sealing in run `29685320924`. Its promotion
run `29685864746` was cancelled before stable mutation because the immutable
tag still embedded the retired protected-environment authority. `1.1.4` keeps
the exact provider implementation and carries the single-operator technical-
authorization workflow on a new immutable source commit and tag. The commands
...
```bash
commit=$(git rev-parse 'refs/tags/provider/v1.1.4^{commit}')
bun run provider:release:build -- \
  --tag provider/v1.1.4 \
  --source-commit "$commit" \
  --output /srv/takosumi-provider-candidates/1.1.4
```
...
```bash
bun run provider:release:verify -- --root /srv/takosumi-provider-candidates/1.1.4
```
...
```bash
bun run provider:release:prepublish -- --root /srv/takosumi-provider-candidates/1.1.4
```
