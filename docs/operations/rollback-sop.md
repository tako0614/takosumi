...
| Capsule Installation rollback          | create a rollback plan for a retained `Deployment` with `POST /api/deployments/:deploymentId/rollback-plan`, review the generated plan, then use the normal approval/apply flow. Rollback must create a new Run/Deployment ledger entry. | installationId, target deployment id, rollback plan run id, apply run id, resulting state/output generation |
