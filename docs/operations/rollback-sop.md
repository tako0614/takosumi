...
| Capsule state rollback                 | create a rollback plan for a retained StateVersion / source identity / Output evidence target, review the generated plan, then use the normal approval/apply flow. Rollback must create a new Run plus StateVersion / Output / AuditEvent evidence. Legacy `Deployment` rollback endpoints, where still present, are compatibility shims for this flow and are not the public model. | capsuleId, target StateVersion id, rollback plan run id, apply run id, resulting StateVersion / Output generation |
...
production GA readiness では、rollback rehearsal とは別に
`release-rollback` operation-drill batch を使って証跡を集めます。

```bash
bun run plan:takosumi-operation-drills -- \
  --batch release-rollback \
  --print-batch-template \
  --out-file takosumi-private/evidence/release-rollback-template.json
```

batch plan には session-gated rollback-plan 作成、rollback run の
events/log summary 取得、manifest validation の public-safe command が含まれます。
実行は operator 承認済みの drill window と previous known-good digest が揃ってからにし、
raw deployment log は public summary に入れません。受理されるのは
`operation-drills-production.json` に入った redacted `rollbackRunId` /
`targetDigest` / public summary です。
