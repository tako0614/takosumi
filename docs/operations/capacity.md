...
| `TAKOSUMI_SOURCE_BUILD_CACHE_DIR`         | `/tmp/takosumi-source-build-cache` | 明示的 source build の package download              | Bun/npm/XDG cache root。build output、credential、state は保存しない。共有 cache を許容しない substrate では未設定にする                                                         |
