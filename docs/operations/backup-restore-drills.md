...
Takosumi の backup は **control backup** と **Installation service-data backup**
の 2 層です。このページは Takosumi operator がその backup / restore をどの頻度で
検証し、どの evidence を残すかを定義します。
...
この drill と service-data restore evidence が揃って初めて、operator は
production restore readiness を満たしたとみなします。
