...
- projected Output metadata and StateVersion pointers
...
復元・照合します。Secret は encrypted envelope と metadata だけを扱い、raw
...
| ----------------------------- | -------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------- |
| Control ledger restore        | monthly              | staging / isolated control-plane restore target | Workspace/Project/Capsule/Run/StateVersion/Output counts, state/output pointer inventory | Takosumi platform owner            |
| Service-data restore sample   | monthly when enabled | staging                                         | one supported Capsule service-data restore transcript, smoke result, RTO / RPO sample    | platform on-call owner             |
...
3. 以下を検証する:
   - Workspace / Project / Capsule / Source metadata /
     ...
     inventory と一致すること
   - current StateVersion pointer と R2 state inventory が一致すること
   - Output projection と raw output artifact manifest が一致すること
   - CredentialRecipe resolution、ProviderConnection status、
     ProviderBinding status、egress/custom runner policy が復元後も一致すること
   - 既知の staging Workspace で Capsule list / inspect が動くこと
   - quota/showback records が live payment processor に接触せず load されること
...
5. 以下を検証する:
   ...
   - 既知の staging Capsule の public route または Output projection が応答する
