...
> room を立て、mitigation / affected-user communications / RCA / postmortem を進める
> 標準手順。
...
- SEV-1 / SEV-2 が宣言された
- tenant data exposure / data loss / secret exposure の疑いがある
- deploy rollback が失敗し、user/tenant impact が継続している
- operator が設定した service SLO breach が検知され、誤検知と断定できない
- authenticated user/tenant report が monitoring と矛盾し、影響範囲が不明
...
Incident commander は operator が SEV policy に設定した response target 内に war room を作ります。
...
Known user/tenant impact:
Current mitigation owner:
Next update due:
```

War room の operator-configured initial response window で決めること:

1. SEV level と scope
2. user/tenant-visible impact の有無
3. writes / deploy / background jobs を止めるか
4. rollback / traffic shift / feature flag / credential disablement の候補
5. affected-user update の初回時刻
...
| monitoring   | user/tenant impact は止まったが再発監視中       | configured observation window green                |
...
2. 既知の healthy platform version へ rollback
3. 不調な runtime / failure domain から traffic を逃がす
4. feature flag や integration を無効化する
5. 影響を受けた runner queue / selected Runner adapter or pool を drain / restart する
...
SEV ごとの初回通知 target、更新 cadence、通知 channel は operator policy が定義します。
external status provider、official support escalation、contractual SLA cadence は host-owned
readiness contribution と runbook が追加します。汎用手順では、通知対象がある場合に initial
update、material change update、final update を記録します。

affected-user update には impact と次回 update 予定時刻を含めます。raw stack trace、
secret 名、provider account id、private channel link、未確定の root cause を
含めてはいけません。

## Readiness Evidence

汎用 baseline では synthetic alert、ack、status update、postmortem / mitigation
reference を private evidence として記録します。hosted/commercial deployment が
追加で要求する drill、status provider、support/SLA evidence は、その host が
readiness contribution と host-owned runbook で宣言します。OSS runbook は特定の
host、incident vendor、private evidence path を要求しません。
...
10:09 JST - mitigation owner - rollback started - platform version pv_123 -> pv_120 - monitoring apply latency
...
| Incident                           | Draft/review policy                        | Action review                          |
| ---------------------------------- | ------------------------------------------ | -------------------------------------- |
| SEV-1                              | operator-configured mandatory timeline     | until critical actions are closed      |
| SEV-2 with material user impact    | operator-configured impact timeline        | configured follow-up cadence           |
| SEV-2 rollback failure / data risk | operator-configured elevated-risk timeline | until elevated-risk actions are closed |
| SEV-3                              | required when policy or recurrence demands | normal backlog or configured cadence   |
...
- 必要なら affected-user final update を送付済み
- RCA owner を任命済み
