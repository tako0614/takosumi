...
deploy する。wrapper は closed Cloud usage handler を in-process mount し、
operator-private realized platform config の `TAKOSUMI_CLOUD_EXTENSIONS` descriptor は
その handler key を参照する。これは customer API ではなく collector 用の Cloud-only
extension で、1 request は 1 Workspace に分割する。この handler は
...
で、customer-facing lifecycle API ではない。公式 `app.takosumi.com` の staging /
production では closed platform wrapper が同じ Worker 内で handler を mount する。
