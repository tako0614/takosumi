...
- D1 control ledger: accounts plane と control plane の ledger。Cloudflare realized
  ...
  CredentialRecipe / InstallConfig / SourceSnapshot / DependencySnapshot / Backup / UsageEvent / Billing は
...
| `TAKOSUMI_PROVIDER_CATALOG_EVIDENCE_REF`                | ProviderConnection policy / CredentialRecipe / internal resolver evidence                                                                                            |
...
- manifest の Provider Catalog claim が Cloudflare / AWS / GitHub / Kubernetes ProviderConnections、
  GCP reserved status、internal resolver の egress / runner policy gate を記録していない
...
Provider Catalog evidence は Cloudflare / AWS / GitHub / Kubernetes provider catalog policy、
