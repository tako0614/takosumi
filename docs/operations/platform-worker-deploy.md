...
- OSS entry: `takosumi/deploy/platform/worker.ts`（accounts plane + in-process control plane + dashboard + runner dispatch）
- official Cloud entry: `takosumi-cloud/platform/worker.ts`（OSS platform worker を wrap し、AI Gateway /
  Cloudflare Workers provider compatibility profile / Cloud usage / Cloud Edge
  Runtime の closed fetch handler を in-process mount する）
- config: **`takosumi-private/platform/wrangler.toml`（実値の正本）**。
  `takosumi/deploy/platform/wrangler.toml` は OSS placeholder の reference template（相対パスは config dir 基準）。
  公式 staging / production の realized config は closed Cloud entry を main にする。
...
```bash
# 1. dashboard SPA と app.takosumi.com/docs を build
cd takosumi
...
TAKOSUMI_SOURCE_BUILD_CACHE_DIR = "/tmp/takosumi-source-build-cache"
...
managed public hostname の既定 namespace と短い URL 枠も realized config を
正本にする。`TAKOSUMI_MANAGED_PUBLIC_BASE_DOMAIN` は operator が所有する base
domain、`TAKOSUMI_MANAGED_VANITY_HOST_SLOTS_PER_OWNER` は owner account ごとの
`<label>.<managed-base-domain>` 枠数です。Workspace-scoped の
`<workspace-handle>-<label>.<managed-base-domain>` はこの枠を消費しません。

```toml
[vars]
TAKOSUMI_MANAGED_PUBLIC_BASE_DOMAIN = "app.takos.jp"
TAKOSUMI_MANAGED_VANITY_HOST_SLOTS_PER_OWNER = "1"
```

compatibility API と OpenTofu Run は同じ OSS public-host reservation ledger を
使います。Cloud extension 側の KV / Durable Object を hostname ownership の別正本に
してはいけません。custom domain verification / certificate lifecycle を composition
していない deployment は、任意ドメイン要求を runtime activation せず fail closed
にします。
...
Cloud extension OAuth introspection client は runtime service token
検証にも使う。公式 Cloud では、非 secret var として
`TAKOSUMI_ACCOUNTS_CLIENT_RUNTIME_SERVICE_TOKEN_INTROSPECTION="enabled"` を
platform worker config に設定する。この flag は secret を持つ confidential client でだけ
有効にでき、`tokenEndpointAuthMethod="none"` の public client では設定エラー
にする。これにより `/gateway/ai/v1/*` は `taksrv_...` runtime service token
を endpoint scope で検証し、closed AI Gateway handler へは raw bearer を渡さず
pre-auth context だけを渡す。
...
`POST /internal/platform/cloud/usage` へ runtime usage を記録する。official Cloud
wrapper は Edge Runtime handler を同じ Worker に in-process mount するため、realized
config の同じ Worker env に secret `TAKOSUMI_CLOUD_USAGE_RECORD_TOKEN` と非 secret var
`TAKOSUMI_PLATFORM_USAGE_ORIGIN=https://app.takosumi.com` を設定する。後方互換として
platform route は token 未設定時に `TAKOSUMI_DEPLOY_CONTROL_TOKEN` を受けられるが、
GA 運用では usage 記録専用 token を使う。

S3-compatible Object Storage endpoint は `TAKOSUMI_S3_COMPAT_BUCKETS` を
realized platform config の非 secret var として持ち、AWS SigV4 access key
descriptor は secret `TAKOSUMI_S3_COMPAT_ACCESS_KEYS` として同じ Worker に設定する。
secret value は `takosumi-private/.secrets/{staging,production}/` に置き、
`wrangler secret put` / `secret bulk` で push する。通常の Takosumi Cloud API key
(Takosumi Accounts personal access token) とは別物で、S3 SDK にはこの SigV4
credential を渡す。
...
Cloud storage / managed resource metering を有効にする場合は、official Cloud entry
`takosumi-cloud/platform/worker.ts` を staging / production の Worker entry として
deploy する。wrapper は closed Cloud usage handler と S3-compatible Object
Storage handler を in-process mount し、operator-private realized platform config
の `TAKOSUMI_CLOUD_EXTENSIONS` descriptor はその handler key を参照する。

`/cloud/usage` は customer API ではなく collector 用の Cloud-only extension で、
1 request は 1 Workspace に分割する。この handler は
`POST /cloud/usage/storage-inventory` と `POST /cloud/usage/resource-meters` を持つ。
後者は closed backend が実測した `cloudflare.containers` /
`cloudflare.durable_objects` の public meter を usage ledger に流すための endpoint
で、customer-facing lifecycle API ではない。

`/compat/s3/v1` は customer-facing S3-compatible data-plane で、S3 SigV4
`Authorization` を Cloud handler が検証するため `authMode: "handler"` を使う。
公式 `app.takosumi.com` の staging / production では closed platform wrapper が
同じ Worker 内で両方の handler を mount する。
...
[
  ...
  },
  {
    ...
    "handlerKey": "TAKOSUMI_CLOUD_S3_COMPAT",
...
Cloud-only extension smoke は account session token を使うが、証跡には
...
Cloudflare Workers provider compatibility envelope、および Cloudflare Workers
script の `PUT -> GET -> DELETE` lifecycle を確認する。
...
請求証跡まで GA 判定に含める場合は、runtime service token 経由の
...
`--require-provider-compat-cloudflare-workers-usage-ledger` で必須化する。

```bash
cd takosumi-cloud

# mounted Cloud-only extension smoke: deploy 後の wrapper mount/reachability/auth/AI/compat-list 確認
bun run smoke:cloud-extensions -- \
  ...
  --platform-version <wrangler-cloud-wrapper-version-id> \
...
bun run smoke:cloud-extensions -- \
  ...
  --require-provider-compat-cloudflare-workers-usage-ledger \
  --provider-compat-cloudflare-workers-usage-workspace-id <workspace-id> \
  --provider-compat-cloudflare-workers-usage-installation-id <compat-installation-id> \
...
bun run smoke:cloud-billing-credit-gate -- \
  ...
  --confirm-mutate-balance \
  --out-file ../takosumi-private/evidence/cloud-billing-credit-gate-production.json \
  --json
...
bun run smoke:cloud-extensions -- \
  ...
  --require-ai-runtime-service-token \
