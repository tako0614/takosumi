...
Takosumi control/state backup を先に復元し、provider snapshot adapter または Capsule-defined export がある Installation
...
- **Cloud account**: provider account used by a Space-owned ProviderConnection.
  AWS/GitHub/Kubernetes/custom provider access is restored through
  ProviderConnections / SecretBlob / provider policy state, not by taking over a
