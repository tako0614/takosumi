...
Takosumi control backup を先に復元し、必要な Installation service-data backup
をその後に reattach / restore します。
...
- **Space**: Takosumi の owner namespace。billing plan、credit ledger、
  Installation graph の正本。
- **Billing account / payer**: Takosumi platform worker の billing ledger
  (`BillingAccount` / `SpaceSubscription`) と payment processor reference。
- **Cloud account**: operator-owned Cloudflare account tenancy for hosted
  managed default。AWS/GCP/GitHub/Kubernetes/custom provider access is restored
  through Space-owned Connection / SecretBlob / provider env set policy state, not by taking
  over a provider account as operator-owned infrastructure.

DR では Takosumi platform worker の identity / billing / OIDC records、Space
直下の Installation DAG / StateSnapshot / OutputSnapshot、operator infrastructure
...
3. Installation service-data を reattach / restore する。
   - critical Space の Installation graph を確認する。
   - 必要な Installation だけ service-data backup を restore する。
   - generated root / state generation / OutputSnapshot の整合性を確認する。
   - Provider Template resolution、Provider Env Set status、
     Connection status、egress/custom runner policy の整合性を確認する。
...
5. customer-facing の critical path を検証する。
   ...
   - Source git read
   - Capsule compatibility check / deploy plan resolve
   - 既知の Installation public route 1 件
   - Space billing / usage read path
