...
- **Billing account / payer**: operator account-plane distribution が管理する
  legal payer / payment processor / entitlement record。
- **Cloud account**: operator-owned Cloudflare account tenancy。

DR では Takosumi control ledger、operator account-plane records、Cloud account
resource の recovery が必要です。raw secret と payment processor credential は
repo 外の operator vault から復旧します。 :::
...
2. control-plane data を restore する。
   - control backup manifest、state/artifact inventory、D1/Postgres backup に従う。
