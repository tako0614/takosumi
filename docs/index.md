...
Takosumi は Git を source of truth にする OpenTofu control plane です。
普通の OpenTofu/Terraform module をそのまま実行でき、必要な場合は
`takosumi_*` Resource Shape を Target / Adapter に解決できます。

このページは software としての Takosumi / Takosumi for Operator の docs です。
あなたが運営する公式 hosted service としての Takosumi Cloud docs は
...
| Resource Shape API、Compatibility API framework、Adapter system  | [Takosumi API](./reference/api.md) と [Model reference](./reference/model.md)                                                                                                  |
...
CredentialRecipe に従って Run 時だけ env/file を注入する
OpenTofu/Terraform を runner sandbox で実行する
plan / apply / destroy を Run ledger に記録する
StateVersion / Output / log / AuditEvent を保存する
Resource Shape を TargetPool / Policy / Adapter に解決する
```
...
alias に流せます。同じ Resource Shape を使い、TargetPool / Policy / Adapter によって
operator が有効化した target へ解決できます。

## 作り直さないもの

既存の industry-standard API / protocol / OpenTofu provider で足りるものは、
Takosumi が作り直しません。
...
標準 surface がなく、繰り返し使う service form がある:
  typed Takosumi Resource Shape として定義する。

一回限りの不足:
  generic-env ProviderConnection と通常の OpenTofu module で扱う。
```

`takosumi/takosumi` provider は Takosumi を使うための必須入口ではありません。
既存 provider が十分なら Stack flow でそのまま使います。Takosumi provider は
Takosumi が所有する typed Resource Shape や operator/admin object が必要な場合だけ使います。
...
詳細を見たい場合は [Model reference](./reference/model.md) を参照してください。
