...
Cloudflare-compatible import、既存 OpenTofu provider、Dashboard action、
`takosumi/takosumi` Resource Shape は、backend API を叩く前に Cloud managed
operation boundary へ正規化されます。WfP などの backend は Target / Adapter /
manager が選ぶ実装詳細です。
