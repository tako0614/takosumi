...
| publisher identity     | publisher id / homepage / optional verified status | TLS + policy                 | operator account plane / install-source policy |
...
| provider resolution    | resolved implementation / connector decision       | kernel ledger                | operator distribution; kernel records evidence |
| installation ownership | Installation source / binding / grant records      | append-only ledger           | operator account plane / kernel                |
| install bootstrap      | one-time opaque launch token                       | TLS + ledger                 | operator account plane + app                   |
| runtime session        | OIDC ID token                                      | signed                       | operator account-plane issuer                  |
...
7. Runtime bootstrap uses the operator account-plane launch-token / OIDC flow.

The chain records immutable source identity and provider decisions before apply.

## Identity / HTTPS {#identity-https}

runtime identity は operator account-plane issuer の OIDC ID token
を基本とする。reference provider adapter は operator distribution が通常の
TypeScript module として import して渡す。package manager
lockfile、HTTPS、private registry、vendoring などは operator policy で扱う。
...
```text
1. install ready → operator account plane issues opaque token
2. user browser is redirected to the app with launch_token
3. app redeems the token against the operator account plane
4. operator account plane atomically marks token as used and returns installation/account context
...
- `manifestDigest`: `.takosumi.yml` raw file bytes の sha256。prose では AppSpec
  digest と説明する。
- source pin / digest: git source は commit SHA、prepared source は tar payload
  sha256。`expected.sourceDigest` は prepared source にだけ使う。 `local` source
  は portable source byte digest を持たず、`manifestDigest` だけを apply guard
  に使う dev/operator-local mode
...
`kindAliases` と reference adapter array (`plugins` option) を使い、deploy 時に
解決結果を Deployment evidence に記録します。
