...
- provider / policy resolution evidence: operator / reference implementation が Deployment を説明するために保持する opaque evidence。Reference kernel may store structured digest families for replay / restore, while public wire compatibility uses the source pin/digest and `manifestDigest`.
