...
[Quota / Rate Limit](/reference/quota-rate-limit) で定義された
...
- `quotaTierId` は operator-controlled ID と同じ kebab-case suffix
  文法を持つ文字列 ([Resource IDs](/reference/resource-ids) 参照)。 suffix は
  operator が選ぶ (例: `tier:free`、`tier:pro`、`tier:internal`)。 kernel は
  suffix を解釈しない。
- v1 で tier 集合は **flat**: 継承、parent tier、tier composition は無い。 各
  Space は 1 つの tier に resolve する。
- Tier record の永続化先は [Storage Schema](/reference/storage-schema)
  で宣言される partition。 kernel restart、journal compaction、restore from
  backup の across で生存する。
...
tier 登録は内部 HTTP surface 経由 ([Kernel HTTP API](/reference/kernel-http-api)
参照)。
...
tier を登録することを要求する。 慣習として `tier:default` を登録し、 operator
が追加 tier を導入するまですべての Space をこれに bind する。 suffix
...
Tier lifecycle が発行する audit event ([Audit Events](/reference/audit-events)
参照):
...
Tier record は [Storage Schema](/reference/storage-schema) と整合する専用 record
class として永続化される。
