...
`listen.<binding>.path` は exact match 用の dotted path です。URL path ではなく、
Space 内の publication 名です。
...
- first segment `takosumi` / `system` と `_` prefix は reserved
...
ちょうど 1 件でなければ apply error です。`required: true` で 0 件なら apply
error、optional な `many: true` で 0 件なら空の collection です。
...
は順序を deterministic にし、Deployment の記録に選択結果を残します。operator
は一致結果を黙って切り捨てません。selector が広すぎる場合は labels
で絞るか、apply 前に policy / size error として失敗させます。

## Path Ownership

path inventory、lifecycle、ownership は、その path を提供する distribution や
organization が定義します。Takosumi core は grammar、reserved prefix guard、
exact-match resolution を扱います。`takosumi.*` と `system.*` は platform /
operator namespace として予約され、具体的な公開 path は各 distribution spec
が定義します。
...
5. 選択した service state と materialization evidence を Deployment
   の記録に残す。
6. exact path または discovery の対象が absent で `required: true` の場合、apply は resource creation
   の前に失敗する。
7. exact path の対象が absent で `required` が omitted / false の場合、その
   binding は作られない。
8. `many: true` の discovery が 0 件で `required` が omitted / false の場合、
   空の collection material として解決する。
