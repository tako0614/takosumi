...
Reference kernel adapter (`KernelPlugin`) は Connector の下流 consumer (= kernel 側の implementation adapter) です。Source author は Connector を直接選ばず、 operator distribution の resolution が implementation binding / Connector binding を選びます。
