...
identity、 record schema、 source/data input、 Space visibility ルール、 signing
expectation、 envelope バージョニング、 そして Connector lifecycle を管理する
operator 専用 operation を定義する。
...
- `acceptedKinds` (required): resolved artifact kinds the Connector accepts.
  Empty array means the Connector reads source/spec directly instead.
...
これを同じ `connector:<id>` identity に bound された新規 Connector record として
扱う。 以前の record は audit と replay 用に保持される。

## Source / Data Inputs {#source-data-inputs}

`acceptedKinds` ベクトルは、古い artifact-backed connector が consume できる
resolved artifact kind を列挙する互換 field です。source-backed connector は
`acceptedKinds: []` とし、`LifecycleApplyRequest.preparedSource` から必要な file
を読みます。

例: reference `worker@v1` connector は `spec.entrypoint` を prepared source から
読みます。`js-bundle` のような artifact kind は Takosumi AppSpec の contract
ではありません。

per-kind size cap、 registered metadata、 discovery API は
[Data Assets](./kind-catalog.md#data-assets) を参照。

Plan-time enforcement:

- An artifact-backed request whose `kind` is not in `acceptedKinds` is rejected
  with an `artifact_kind_mismatch` error.
...
source locator で決まる。Implementation のマッチングは resolver を通る。
...
## 署名期待 {#signing-expectations}

`signingExpectations` は、 Connector が受け付ける artifact に対して何を要求する
かを宣言する。

- `none`: Connector は unsigned artifact を受ける。 per-key artifact policy
  が依然として signing を要求しうる。
- `optional`: signed / unsigned 両方を受ける。 plan は signature 不在を log
  するが reject しない。
- `required`: plan 時に unsigned artifact を reject する。

> signing expectation は v1 では connector のメタデータである。 Artifact 署名の
> 検証自体は operator がインストールした signing backend が行い、 Connector
...
upgrade path は `replace` operation で、 以前と新規の envelope version の両方
を記録する。
...
## Connector consumer plugin の利用 {#connector-consumer-plugin-consumption}

> **用語の正本**: 本ページで「Connector consumer plugin」 と呼ぶのは、 Connector
> を下流から consume する kernel-side plugin (= 旧 `ProviderPlugin` contract /
> 現 `KernelPlugin` materializer factory のいずれか) のこと。 kernel-side
> materializer は AppSpec component を Connector lifecycle に dispatch
> する役目を持つ。「provider plugin」 という総称は kernel-side materializer (=
> `KernelPlugin`) と Connector consumer (= 本セクション) の 2 concept
> を同時に指してしまうため、 本ページでは Connector の下流側を 「Connector
> consumer plugin」 と呼び、 kernel-side の plugin contract は `KernelPlugin` /
> 旧 `ProviderPlugin` (`@deprecated`、 `KernelPlugin` adapter で bridge される)
> と表記する。

Connector consumer plugin は Connector の下流 consumer (= kernel 側の
materializer plugin) である。

- A Connector consumer plugin declares the `connector:<id>` identities it
  depends on. The kernel resolves each declared identity at apply time and
  rejects the apply if any declared Connector is not visible to the active
  Space.
- Connector consumer plugins receive the resolved Connector record (`id`,
...
- A Connector consumer plugin must not invent new `connector:<id>` identities.
  Plugins that need a new Connector raise the request through the operator
  `install` operation.
...
- Source-backed connectors read from `preparedSource`; artifact-backed
  connectors may fetch bytes through the kernel's artifact partition using
