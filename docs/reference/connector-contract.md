...
source-backed connector の input contract は connector-specific `spec` と resolved source locator で決まる。implementation binding / connector binding のマッチングは resolver を通る。
...
Reference kernel adapter (`KernelPlugin`) は Connector の下流 consumer (= kernel 側の implementation adapter) です。manifest author は Connector を直接選ばず、 operator profile の resolution が implementation binding / Connector binding を選びます。
