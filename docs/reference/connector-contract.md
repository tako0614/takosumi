...
ユーザーは component kind / spec を書き、operator distribution の resolver が connector-local selector と Space visibility に bound された Connector 実装を選びます。
...
source-backed connector の input contract は connector-specific `spec` と resolved source locator で決まる。`spec` は public AppSpec を開いたまま渡す場所ではなく、operator-selected adapter が kind descriptor validation と listen projection を済ませた後の closed lifecycle input です。implementation binding / connector binding のマッチングは resolver を通る。
...
Reference kernel adapter (`KernelPlugin`) は Connector の下流 consumer (= kernel 側の implementation adapter) です。manifest author は Connector を直接選ばず、 operator distribution の resolution が implementation binding / Connector binding を選びます。
...
kernel-side implementation adapter (`KernelPlugin`) の record schema と registration API は [Kind Binding Implementations](./kind-bindings.md) を参照。
