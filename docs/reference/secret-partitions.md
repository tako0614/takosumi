...
> このページでわかること: secret partition の v1 仕様 — Space 分離 / AES-GCM
> 鍵派生 / AAD verify / multi-cloud env / rotation protocol。

## Partition model

secret partition は **Space と cloud partition tag の組** に対して 1:1 で
分離される独立した暗号箱である。 kernel core は partition
の中身を直接保持しない。 各 partition から復号する際に partition key を一時的に
derive するだけで動く。
...
があればそれを使い、 無ければ global にフォールバックする。
