...
- Takosumi は raw secret value を永続化、cache、log しない。 decrypt は **request scope** に閉じ、transient buffer を越えて保持しない。 decrypt 結果を受け取る connector / runtime-agent は、自分の process lifetime、zeroization、 log rejection の責務を負う。
