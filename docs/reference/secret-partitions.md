...
> このページでわかること: secret partition の v1 仕様 — Space 分離 / AES-GCM 鍵派生 / AAD verify / multi-cloud env / rotation protocol。

## Partition model

secret partition は **Space と cloud partition tag の組** に対して 1:1 で
分離される独立した暗号箱である。 kernel core は partition の中身を直接保持しない。
各 partition から復号する際に partition key を一時的に derive するだけで動く。
...
- partition の cross 参照は禁止。 reference (`${secret:...}`) は許される。
...
同じでも Space や cloud partition が違えば独立 key になる。 `info` に generation
...
partition tag が `aws` の partition は AWS 専用 env があればそれを使い、 無ければ
global にフォールバックする。
...
rotation は `(spaceId, partitionTag)` 単位で実施する。 世代 counter を 1 増やして
全 entry を re-encrypt する。
...
- `apply` / `activate` / `destroy` / `rollback` は rotation 進行中も継続して
  動く。 decrypt path は world view を読み、 encrypt path は new generation を書く
