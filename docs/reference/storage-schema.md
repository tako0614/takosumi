...
## OperationJournalEntry 互換行 {#operationjournalentry-compatibility-row}

installer lifecycle が記録する WAL stage record。 現行実装では
`takosumi_operation_journal_entries` で実装される。

本 record は内部 `JournalEntry` よりも意図的に狭い: OperationPlan の preview
から導出され、 provider 呼び出し前後の stage 進行を記録する。 installer
entrypoint に対し durable な replay 証跡と effect-digest
...
| --------------------- | --------- | -------- | ------------------------------------------------------------------------------------------------------------ |
| `id`                  | string    | yes      | 行識別子。                                                                                                   |
| `spaceId`             | string    | yes      | installer-resolved Space scope。                                                                             |
| `deploymentName`      | string    | no       | manifest metadata 由来の deployment 名。                                                                     |
| `operationPlanDigest` | sha256    | yes      | 決定的に算出される OperationPlan preview digest。                                                            |
...
| `operationKind`       | string    | yes      | operation kind (例: `create` / `delete`)。                                                                   |
| `resourceName`        | string    | no       | manifest 上の resource 名。                                                                                  |
| `providerId`          | string    | no       | manifest が選択した provider id。                                                                            |
| `effectDigest`        | sha256    | yes      | canonical WAL effect payload の digest。                                                                     |
...
Persistence: full journal compaction policy が有効になるまでは Deployment record
と同一の policy で保持。 `(spaceId, operationPlanDigest)` /
...
| `tenantId`         | string          | yes      | tenant / Space scope。                                                 |
...
| `name`        | string    | yes      | Installation / Deployment lock key。               |
...
Persistence: installer apply / rollback lock を保持している間のみ存在。 primary
key は `(tenantId, name)`。 expiry 確認用に `(lockedUntil)` を index。
...
[RevokeDebt Model](./revoke-debt.md) 参照。
...
| `deploymentName`      | string    | no       | debt が Deployment 由来のときの deployment 名。                                                                                         |
...
[Compliance Retention](./compliance-retention.md) 規定の cleared 後 retention
window まで保持。 実装テーブルは `takosumi_revoke_debts`、 key は `id`、
`sourceKey` は unique。 `(ownerSpaceId, status)` /
...
See also: [Connector Contract](./connector-contract.md).
...
See also: [Audit Events](./audit-events.md).
...
**operator-pinned sha256 digest** (詳細:
[Catalog Release Trust](./catalog-release-trust.md))。
...
まで保持。 `(spaceId, adoptedAt)` / `(catalogReleaseId)` で index。
Immutability: operator のみ append 可能。

See also: [Catalog Release Trust](./catalog-release-trust.md).
...
See also: [Cross-Process Locks](./cross-process-locks.md).
...
See also: [Secret Partitions](./secret-partitions.md).

## Account-plane アイデンティティレコード {#account-plane-identity-records}
...
See also: [Tenant Export / Deletion](./tenant-export-deletion.md).
...
See also: [Cost Attribution](./cost-attribution.md).
...
See also: [Zone Selection](./zone-selection.md).

## Workflow 拡張レコード {#workflow-extension-records}
...
/ hook の state は installer API を呼ぶ外部 CI / operator product に属する。
詳細は
...
- [Journal Compaction](./journal-compaction.md) の retention rule を
  満たすために必要な field を drop すること。
- secret 値を inline で永続化すること。 secret は参照のみで、 embed しない
  (詳細は [Audit Events](./audit-events.md))。
