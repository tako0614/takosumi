...
| `ArtifactRecord` | build output や uploaded artifact の digest、size、retention metadata。 |
...
`AuditLogEvent` は kernel operation の audit envelope です。event type と payload
の詳細は [Audit Events](./audit-events.md) にあります。
