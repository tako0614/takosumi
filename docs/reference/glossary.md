...
component の種類を示す文字列（Takosumi は値を解釈しない）。operator が `kindAliases` で省略名 / URI を kind の定義と binding に解決する。takosumi.com の例は [Kind カタログ仕様](./type-catalog.md)。
...
operator が kind URI / descriptor を concrete backend runtime や resource operation に結びつける実装側の binding。どの仕組みで binding を読み込むかは operator profile が選ぶ。
...
revoke できなかった cleanup task の管理レコード (コード: `RevokeDebt`)。resource side effect の取り消しが失敗した場合に記録され、operator action を要求する。
