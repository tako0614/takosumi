...
- `pre-commit`: provider / connector verification と external バリデーション (credential reachability / collision check / freshness re-confirm) を fail-closed で確認。 actual-effects 書込なし。失敗は `abort`。
...
- `post-commit`: commit 後の evidence / projection / metadata sync を記録する。 provider side effect は新規に実行しない。 external cleanup が完了できないときは CleanupBacklog を `external-revoke` / `link-revoke` reason で enqueue。
