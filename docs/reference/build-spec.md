...
Takosumi は build を実行しません。build service、CI、operator automation
は Installer API を呼ぶ前に source を準備できます。Takosumi が受け取るのは
...
Takosumi はその profile を public contract にしません。
...
OpenTofu は operator distribution や `takos-private/` が使う
infrastructure tool です。build service が OpenTofu plan を作る場合でも、state、
lock、provider credential、apply permission は operator 側に残します。
Takosumi は OpenTofu を実行せず、operator catalog が公開した
