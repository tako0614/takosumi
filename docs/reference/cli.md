...
CLI は manifest 送信、 content-addressed DataAsset 管理、 runtime-agent 操作、
kernel server 起動を行います。 decision authority は kernel 側 (public deploy
Space または control-plane Space context) で、 CLI は thin client です。
...
local mode は Space 状態を持たず、 同一プロセス内 bundle の shape / provider
registry で resolution します。 remote mode は kernel HTTP server と通信し、
Space scope で [WAL Stages](/reference/wal-stages) に従って resolution /
planning / journaling します。
...
- deploy / DataAsset 系: `TAKOSUMI_DEPLOY_TOKEN` (1 つの public deploy Space /
  tenant scope に対応)。 Space は `TAKOSUMI_DEPLOY_SPACE_ID`、 未設定時は
  `takosumi-deploy`。 manifest に Space field は載せません。
- runtime-agent 系: `TAKOSUMI_AGENT_TOKEN`。
...
flag / source 何も無ければ local mode。 remote 必須 endpoint (`status`、
`artifact ...`) はその場合 exit code 2。

## 設定ファイル

`~/.takosumi/config.yml` を起動時に 1 度読みます。 path は
`TAKOSUMI_CONFIG_FILE` で上書き可。 schema は closed。
...
- `$HOME` (Windows は `$USERPROFILE`) 未設定で `TAKOSUMI_CONFIG_FILE` も無い:
  silently skip
- file 不在 / 空: silently skip
- YAML mapping でない: stderr に 1 度 warning を出して ignore
- その他読込エラー: stderr に 1 度 warning を出して ignore

env > config file、 flag > env > config file の優先順位。
...
`--detach` は fork しません (Deno に portable な detach API がないため)。
supervisor template を出力するだけで、 systemd / docker compose / nohup の配線
は operator 側で行います。
...
Exit codes: `0` SIGINT/SIGTERM 正常終了、 `1` bind 失敗 / kernel boot エラー。

rolling upgrade / drain / kernel ↔ runtime-agent skew は
[Schema Evolution](/reference/migration-upgrade) 参照。
...
fixture `examples/direct-deploy/` が sample。 `.takosumi/` や `takosumi-git`
は使わず、 `manifest.yml` と `.github/workflows/deploy.yml` のみ。

action は `takosumi deploy <manifest> --remote ... --token ...` を実行し
`POST /v1/deployments` に POST します。 AppInstallation ownership / AppBinding /
billing / grant record は作りません。 これらが必要なら Takosumi Accounts の
install flow を使ってください。
...
remote `plan` は `POST /v1/deployments` を `mode: "plan"` で送信し、 response
...
remote は `mode: "destroy"` で送信。 kernel が persist 済 handle を引いて
runtime-agent に発送します。 retry で provider destroy を二重実行しないよう
fresh `X-Idempotency-Key` を付与。 local は in-process で破棄し、 事前 state が
無ければ best-effort。
...
引数なしは `GET /v1/deployments` で一覧、 `<name>` 指定は
`GET /v1/deployments/:name` で 1 件。 いずれも
...
`GET /v1/deployments/:name/audit` を呼びます。 引数が deployment id の場合は
先に `GET /v1/deployments` で id → name を解決します。
...
compile 済 Shape manifest を生成し、 path を `takosumi deploy` に渡せるように
します。 project layout scaffold は `takosumi-git` の責務です。

```text
takosumi init [<output>]
```

`<output>` 指定時は当該 path に出力、 無指定時は stdout。 top-level `template`
は現行 kernel の public field ではありません。

### `takosumi doctor`

CLI の manifest / target mode / auth 状態を表示します。
...
remote 解決を共有します。 出力: manifest path、 resolved resource 数、
deployment 名、 local vs remote、 token 有無、 次の推奨 command。 manifest が
無い / 不正なら `1`。

### `takosumi artifact <push | list | rm | gc | kinds>`

kernel artifact store の content-addressed DataAsset を管理します。 全
subcommand に remote URL と deploy scope token が必須。 不足時は exit code 2。
...
- `push`: `POST /v1/artifacts` で bytes を upload、
  `{ hash, kind, size, uploadedAt }` envelope を表示。 hash を manifest に埋め
  込んで使う
- `list`: `GET /v1/artifacts` の paginated 結果を辿る
- `rm`: 単一 hash を削除
- `gc`: persist 済 DesiredSnapshot 参照 graph に対する mark-and-sweep
- `kinds`: `GET /v1/artifacts/kinds` で kernel が認識する kind 一覧を返す

semantics は [DataAsset Kinds](/reference/artifact-kinds) を参照。

### `takosumi runtime-agent <serve | list | verify>`

cloud credential を保持して kernel の代わりに lifecycle 作業を行う Takosumi
...
- `verify`: `POST /v1/lifecycle/verify` で read-only smoke test。 failed
  connector があれば exit code 2

## Trigger / Step subcommand

trigger / step CLI subcommand はありません。 kernel は trigger / hook /
execute-step primitive を持たず、 workflow / cron / hook は plugin が提供する
manifest resource として宣言し、 通常の `takosumi deploy` で送信し、 実行は
`takosumi-git` 等の上流 product が担当します。 境界は
[Workflow Placement Rationale](/reference/architecture/workflow-extension-design)。

## Project Layout

`deploy` / `plan` / `destroy` / `doctor` は manifest path 明示が必須 (位置引数
`<manifest>` か `--manifest <path>`)。 CLI 組み込みの project layout discovery
はありません。 working tree root の `.takosumi/manifest.yml` や `manifest.yml`
を探したりはしません。

repository convention (`.takosumi/` directory、 `.takosumi/workflows/*.yml`、
git push / webhook / build pipeline / cron / hook 配線) は `takosumi-git` の
所有。 `takosumi-git` が project repository を Manifest に変換し
`POST /v1/deployments` へ送信します。
...
`70` 以上は signal 駆動 exit 用の予約 (現状未使用)。 CLI は process signal を
個別 exit code に mirror しません。
