...
process replacement 中の kernel pod 群は
[Cross-Process Locks](./cross-process-locks.md) を共有することで直列化 される。
schema 不整合が発生する process replacement は後述の rollback gate に よって
rejection される。
...
forward-only として扱い、 down migration は提供しない (rollback gate がそもそも
...
rollback は release-specific な operator 作業。 public reference は可否判定の
...
migration / upgrade 関連の audit event は [Audit Events](./audit-events.md) の
hash chain に連結される。
