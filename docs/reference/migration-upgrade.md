...
Takosumi v1 における kernel version の rolling replacement、schema evolution の
up / down semantics、rollback gate、kernel ↔ runtime-agent skew tolerance、
runtime-agent drain protocol を定義する。これは pre-GA の current spec であり、
公開ユーザー向けの切替手順や互換期間の約束ではない。
...
binary は触らない。schema evolution の実行コマンドは release ごとの operator
runbook に閉じ、public reference では current invariant だけを扱う。
