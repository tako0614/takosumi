...
[Cross-Process Locks](/reference/cross-process-locks) を共有することで直列化
される。 schema 不整合が発生する process replacement は後述の rollback gate に
よって rejection される。
...
に渡す ([Lifecycle](/reference/lifecycle))。
