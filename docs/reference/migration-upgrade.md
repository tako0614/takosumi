...
> Rationale: alignment 範囲を 2 minor 以内に縛ることで implementation binding / runtime-agent の wire shape 検証を bounded に保つ。 3 minor 以上を許すと N×M version pair の testing matrix が現実的でなくなり、 dispatcher が抱える release-set 分岐が線形に増える。
