...
- escalation timeout の長さは kernel 定数ではなく **policy-controlled** で、Space の policy pack 上で設定する。
...
- `operator-action-required` 1 件以上の状態を **production deploy gate 失敗** として扱い、kernel の `/readyz` に直接は反映しないが、operator gate で deploy を止める運用にする。
