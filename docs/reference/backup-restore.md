...
> の両方に二重実装する保守コストを避け、cross-major recovery は release-specific
> private runbook で扱う設計にしている。
...
- `commit` cursor が無い場合、operation は provider side effect 未到達として
  terminal `abort` へ進める。provider compensate は呼ばない。
...
各 event は payload に backup export 時点の `backupChainHead` を運ぶ。restore を
backup に対して検証することは、 `restore-completed` payload の `backupChainHead`
が `backup-completed` payload の `backupChainHead` と一致する
ことの確認に等しい。restore event 自体を同じ audit chain に append する場合、
restore 後の current chain head は当然別の値になる。
