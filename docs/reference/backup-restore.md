...
は public restore contract ではない。restore ツールは cross-major 直接 restore
を拒否し、release-specific private runbook と検証済み evidence を要求する closed
な `failed_precondition` エラーを発行する。
