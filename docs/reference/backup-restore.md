...
これらが集合として current reference profile の **backup set** を構成する。別 operator profile は、同等の Installation / Deployment restore semantics を満たす別の storage layout と backup set を定義できます。
...
- `commit` cursor が無い場合、operation は provider side effect 未到達として terminal `abort` へ進める。provider compensate は呼ばない。
...
in-flight operation はステップ 5 で記録された recovery mode を通じて resume する。各 Implementation が `recoveryMode = continue` と `recoveryMode = compensate` をどう扱うかは [Provider Implementations](./providers.md) が定める。
