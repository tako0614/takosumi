...
Interface の read-only な portable projection は
`GET /apis/forms.takoform.com/v1alpha1/interfaces` にもあります (optional、
discovery の `features.interface_declarations` で広告)。こちらは「何が存在するか」
だけを返し、write も認可も持ちません。詳細は
[portable form host API](portable-form-host-api.md) を参照してください。
...
`Interface`を作ります。`Interface.spec`は、利用側が解釈する`type` / `version`、任意の
...
利用側の権限は`InterfaceBinding`で明示します。BindingはPrincipal、
...
delivery方式は安全側に停止し`NotReady`になります。
...
Workspace全体のplan/applyや利用側 Capsuleの再applyは起動しません。OpenTofu間の
...
Interface type/versionだけを使用します。Resolved Interfaceと永続 Activity evidenceが
...
InstallConfig の read 系 projection は `Secret` と同じ write-only 規約に従います。
`variablePresentation` で `secret: true` と宣言した変数と、名前が credential 形をした
変数 (`*_token` / `*_password` / `*_api_key` など) は、値ではなく `[REDACTED]` を返します。
per-install config は Capsule record から id で辿れて Workspace membership だけで
読めるため、install 時に入力された catalog token をそのまま返してはいけません。書き込みは
従来どおり merge semantics なので、値を変えない patch は `[REDACTED]` を送り返さず、
その変数を単に省略します。
...
未設定、例外) はすべて安全側に停止します。provider-applied StateVersion / Output と実際の
...
設定しない場合、その Run は直ちに安全側に停止します。コマンドは、実行制約として `timeout_seconds` /
...
contract、安全側に停止する挙動を持つ別仕様として定義します。
