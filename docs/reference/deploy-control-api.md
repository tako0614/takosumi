...
> **account-plane product surface との区別**: account plane は別の resource として **`/v1/installations`**
> (takos product の AppInstallation app-distribution API) と **`/v1/connections`** (account-plane connections) を
> 持ちます。これらは `/api/v1` の deploy-control Installation / Connection とは別 resource であり、意図的に別 prefix で
> 共存します。本ドキュメントが扱うのは `/api/v1` deploy-control surface です。
