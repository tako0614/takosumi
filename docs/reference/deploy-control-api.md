...
公開される control-plane surface は `/api/*`、`/install`、inbound webhook seam の `/hooks/*` です。dashboard が使う
account session route と accounts/CLI 向けの in-process seam は operator distribution の内部経路であり、本 API surface
ではありません。

| Surface    | 用途                     | 認証                                                                |
| ---------- | ------------------------ | ------------------------------------------------------------------- |
| `/api/*`   | 公開 control plane       | host が scoped principal を解決。reference fallback は bearer token |
...
`/api` の各 route は、host worker が解決した scoped principal で保護されます。dashboard session、accounts plane、
...
resolver も未設定の host は `/api` route を `404 not_found` で隠します (未設定 surface を public host で漏らさない)。
...
API reader が依存する contract は `/api/*`、`/install`、`/hooks/*` だけです。

## `/api` surface

version prefix は付けず `/api` にまとめます。`/api` route は host が解決した scoped principal で保護され、
reference fallback のみ bearer token を使います。operator-only admin route は operator-scoped principal だけが使えます。
...
| GET    | `/api/sources?spaceId={spaceId}`    | Space 内の Source 一覧 (hook secret は含まない)                                                    |
...
| GET    | `/api/sources/{sourceId}/snapshots` | Source の immutable archive snapshot 一覧 (commit / digest / R2_SOURCE key)                        |
...
| POST   | `/api/connections/cloudflare/oauth/start`    | Cloudflare OAuth helper 開始。成功すると authorization URL / state を返す                         |
| GET    | `/api/connections/cloudflare/oauth/callback` | Cloudflare OAuth helper 完了。write-only `provider_env_set` Connection を作成                     |
...
| POST   | `/api/connections/gcp/oauth/start`           | Google Cloud OAuth helper 開始。成功すると authorization URL / state を返す                       |
| GET    | `/api/connections/gcp/oauth/callback`        | Google Cloud OAuth helper 完了。write-only `provider_env_set` Connection を作成                   |
| POST   | `/api/connections/gcp/impersonation`         | Google service-account impersonation 用の write-only Connection を作成                            |
...
| PUT    | `/api/operator-connection-defaults`          | operator-scoped bearer 専用。provider の instance-wide default Connection を設定                  |
| GET    | `/api/operator-connection-defaults`          | operator-scoped bearer 専用。instance-wide default Connection 一覧                                |

Operator default connections は instance-wide な管理機能です。route は `/api` inventory に載りますが、operator-scoped
...
| Method | Path                          | 用途                   |
| ------ | ----------------------------- | ---------------------- |
| GET    | `/api/providers`              | Provider Template 一覧 |
| GET    | `/api/providers/{providerId}` | Provider Template 取得 |
...
| Method | Path                                     | 用途                                                                                                       |
| ------ | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| GET    | `/api/install-configs`                   | official / Space-scoped InstallConfig 一覧                                                                 |
| GET    | `/api/install-configs/{installConfigId}` | InstallConfig 取得                                                                                         |
...
| Method | Path                            | 用途                                                                                                              |
| ------ | ------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| POST   | `/api/spaces/{spaceId}/uploads` | ローカル Capsule の `tar`(zstd) archive を binary body で ingest。R2_SOURCE に保存し upload origin SourceSnapshot を記録 |
| POST   | `/api/deploy`                   | upload snapshot を pin して `@space/name` Installation を解決/作成し plan Run を起こす                            |

`POST /api/spaces/{spaceId}/uploads` は JSON ではなく **archive バイト列**を request body に取ります (JSON-schema
...
`POST /api/deploy` は upload snapshot を pin して Installation を解決/作成し、plan Run を起こします。`vars` は
...
| Method | Path                                               | 用途                                                                                                                                                                                                                       |
| ------ | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | `/api/installations/{installationId}/dependencies` | consumer をこの Installation とする Dependency edge 作成。mode は `variable_injection` / `remote_state` / `published_output`。`remote_state` は same-Space trusted dependency、cross-Space は OutputShare 経由。cycle 拒否 |
...
| POST   | `/api/installations/{installationId}/drift-check`  | read-only drift-check Run (apply できず `waiting_approval` にもならない)                         |
...
| POST   | `/api/spaces/{spaceId}/drift-check`    | `space_drift_check` RunGroup を作成 (active Installation ごとに read-only drift_check Run) |
...
| POST   | `/api/output-shares/{shareId}/approve` | receiving Space 側の acceptance flow                                                    |
...
| Method | Path                             | 用途                                        |
| ------ | -------------------------------- | ------------------------------------------- |
| GET    | `/api/spaces/{spaceId}/activity` | Space の Activity event 一覧 (`limit` 対応) |

### Billing

Billing は Space 単位の公開 surface です。実装適合状況は [`core-conformance.md`](../core-conformance.md) に集約します。
`GET /billing` の plan projection は typed `BillingPlanLimits` を含み、plan completion は active subscription の
...
| Method | Path                                          | 用途                                                          |
| ------ | --------------------------------------------- | ------------------------------------------------------------- |
| POST   | `/api/installations/{installationId}/backups` | Installation を解決し、その owning Space の backup を作成する |
| POST   | `/api/spaces/{spaceId}/backups`               | Space の backup を作成する                                    |
| GET    | `/api/spaces/{spaceId}/backups`               | Space の backup ledger pointer を newest-first で列挙する     |
