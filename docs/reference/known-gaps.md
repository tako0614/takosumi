...
3. **`applyV2`** (`domains/deploy/apply_v2.ts`) — `ApplyService.applyManifest`
   に `resources[]` internal authoring payload を渡したときだけ到達。provider compensate/destroy
...
fine-grained stages は書かない。provider.apply を数件実行した後・bulk commit append の前に
...
silently drop してしまうため、現状は create-only を維持。
