...
```text
apiVersion | metadata | components
```

`apiVersion` は `"v1"` 固定 (Wave L 以降 group prefix 無し)。 旧 `kind: App`
root field は Wave K で物理削除済 (= top-level field として宣言できない)。
...
| 値     | 意味                            |
| ------ | ------------------------------- |
| `"v1"` | v1 で kernel が受理する唯一の値 |

breaking change は `"v2"` 等で発行し kernel が version ごとに routing。 Wave L
以降 `apiVersion` から k8s 風 group prefix (`takosumi.dev/`) は削除済 (=
Takosumi parser は `.takosumi.yml` のみ扱うので group prefix は redundant)。
...
`apiVersion` は必須で `"v1"` に固定 (Wave L 以降 group prefix 無し)。 旧
`kind: App` root field は Wave K で物理削除済 (= top-level field として宣言
できない)。 未知の top-level field は schema validation で失敗する (= warning
