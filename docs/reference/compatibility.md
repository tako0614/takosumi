...
Takosumi kernel の deploy / runtime surface は backend-neutral な compiled Shape
manifest を基準にし、resource layer も backend-neutral な abstract type (sql,
object-store, key-value, etc.) で書く。Cloudflare / AWS / GCP / k8s / local
は同じ public contract を共有し、差分は operator 内部の adapter
で吸収する。ただしここでの compatibility は schema / translation parity
...
Takosumi kernel が parity の対象にしているもの:
...
| feature                                      | compiled Shape manifest | `.takosumi/app.yml` | runtime notes                                     |
| -------------------------------------------- | ----------------------- | ------------------- | ------------------------------------------------- |
