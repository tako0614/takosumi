...
> **実装状況 (Takosumi service)**: 現状の reference apply pipeline は dry-run plan
...
Risk サマリー、approval プロンプト、prediction digest、approval token はこの preview を囲む operator/account layer の拡張です。これらは operator フィールドまたは account layer record を通じて運ばれ、Takosumi の Source surface には追加しません。
