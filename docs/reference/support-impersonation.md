...
## Support-staff アクター {#support-staff-actor}
...
## Impersonation 付与 {#impersonation-grant}
...
## Impersonation セッション {#impersonation-session}
...
## レート制限とアクティブセッション上限 {#rate-limit-and-active-session-caps}
...
できる window 内での上限到達の繰り返しは [Incident Model](./incident-model.md)
auto 検知 path の trigger family となる: 拒否された grant request や session
open のバーストが持続すると、 `support-impersonation-burst` family の下で
incident が auto mint される。

## 監査イベント {#audit-events}

v1 support-impersonation の audit event 分類は closed で、
[Audit Events](./audit-events.md) の closed enum に加わる。
...
session は Space の compliance regime ([Audit Events](./audit-events.md) 参照)
のもと audit store に残る。 顧客 admin はこれらの event を、kernel が Space
scope の任意の event 向けに公開する同じ audit query で読み取る。

## オペレーター専用エンドポイント {#operator-only-endpoints}
...
support impersonation は [Storage Schema](./storage-schema.md) を 2 つの record
class で拡張する。
