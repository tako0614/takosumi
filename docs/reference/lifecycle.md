...
このページは reference kernel の internal lifecycle note です。public lifecycle
model は AppSpec / Installation / Deployment で、wire operation は Installer API
の install / deploy / rollback です。WAL、
OperationPlan、runtime-agent、connector、cleanup worker は operator
implementation の実行モデルです。

Reference kernel の lifecycle は
...
Fail-closed の precondition: 最新の unfinished mutation WAL entry (`apply` /
...
reference kernel v1 では 6 phase を 1:1 に区別します。各 phase は対応する
...
| `rollback` | 巻き戻し対象 Deployment の recorded source / snapshot / evidence | Installation / GroupHead / ActivationSnapshot pointer update。Deployment は新規作成しない | `pre-commit` → `commit` → `post-commit` → `finalize`             |
...
- `destroy` は現行 Deployment の recorded evidence を authority として managed /
  ...
  operation evidence / WAL / activation state に記録し、既存
  `ResolutionSnapshot` は編集しません。external / operator-owned input refs
  は触りません。
- `rollback` は巻き戻し対象 Deployment の source pin、snapshot、internal
  evidence を authority として current pointer を戻します。Deployment record は
  新規作成せず、activation / GroupHead 更新と audit evidence を記録します。 DB /
...
- `recovery` は kernel restart 時に WAL を読み直し、最後に記録された stage の
  **次** から resume します。
- `observe` 起動 / 維持は kernel control-plane readiness 連動。 storage / lock /
...
reference runtime-agent の `verify` request は optional reference operator
preflight であり、Installer API や public lifecycle の一部ではありません。WAL
