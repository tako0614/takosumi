...
Takosumi の public Installer API は Installation を中心にした 5 endpoint です。対象は manifest / Installation / Deployment の 3 つで、endpoint はその lifecycle に対応します。
...
この 5 route が Takosumi public Installer API です。operator profile が account layer API や facade を同じ host / URL prefix に置く場合、それらは operator-owned API contract として versioning します。
...
参照 API の形式は Cloud account layer API、operator-managed read API、operator admin tooling のどれでもかまいません。route inventory、pagination、authentication、redaction rule は operator profile が定義します。
...
下の `outputs` 例は、official gateway の kind 定義と service-binding の kind 定義を採用した operator が作る catalog-shaped な出力データです。Core は non-secret output の出力データを記録し、kind の定義の semantics は catalog / operator 側が定義します。
...
core dry-run response は `changes[]` と `expected` guard です。Cost estimation、 billing quote、approval prompt、account layer policy message は、この Installer API call の周辺にある operator profile response です。
...
`outputs.components[componentName][publicationName]` は、その Deployment で materialize された manifest `publish` entry の public / non-secret output です。 public installer response は catalog で定義された publish の出力データを JSON object として返します。上の例は Takosumi Kind Catalog の `http-endpoint` と `service-binding` の出力データを使った response 例です。出力データ field の意味は選択された catalog / operator profile が定義し、operator-facing ledger は operator-held apply evidence を別に保持できます。
...
Installation が `suspended` の場合でも、dry-run endpoint は preview を返せます。 side effect を伴う deploy / rollback endpoint は、operator-owned lifecycle flow が Installation を resume するまで 409 `failed_precondition` を返します。 suspension 中の runtime serving behavior は operator profile または account layer projection が定義します。
...
Provider rollout、activation、domain projection などの runtime-routing と operator rollout view は operator profile docs で扱います。Installer API の wire guarantee は、current pointer が `succeeded` Deployment だけを指すことです。
...
public Deployment wire は source identity、`manifestDigest`、status、public / non-secret `outputs` を保証します。portable な記録の summary は documented extension field で公開できます。implementation と operator profile は resolution / 実体化の記録を ledger に残します。
