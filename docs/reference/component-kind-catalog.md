...
> このページでわかること: `.takosumi.yml` の `components[*].kind` で使える 5 種類
> の schema と output。
...
artifact として動きます。 旧 `worker@v1` + `web-service@v1` を統合した kind です。
...
Managed PostgreSQL instance。 旧 `database-postgres@v1` の rename。
...
DNS record + public domain TLS termination。 旧 `custom-domain@v1` の keep。
