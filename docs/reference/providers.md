# Provider プラグイン {#provider-plugins}
...
## Capability 語彙: open string + reserved prefix {#capability-vocabulary-open-string--reserved-prefix}
...
### Deno Deploy (opt-in) {#deno-deploy-opt-in}
...
## Deno Deploy opt-in フロー {#deno-deploy-opt-in-flow}
...
## 公開 API surface {#public-api-surface}
...
## Implementation 契約 {#implementation-contract}
...
### Operation request envelope {#operation-request-envelope}
...
### Operation status enum (5 値) {#operation-status-enum-5-values}
...
### 回復モードの挙動 {#recovery-mode-behaviour}
...
### Effect bound ルール {#effect-bound-rule}
...
### Dry materialization フェーズ {#dry-materialization-phase}
...
### Connector との関係 {#connector-relationship}
...
### 失敗モードと journal entry のマッピング {#failure-mode-to-journal-entry-mapping}
