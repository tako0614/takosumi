...
| 変数                                   | 必須                           | 既定値      | 決めること                                                                                                                                   |
| -------------------------------------- | ------------------------------ | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `TAKOSUMI_CONTROL_D1_SCHEMA_MODE`      | 任意                           | `bootstrap` | `bootstrap` はリクエスト時にスキーマを用意します。`predeployed` はそれを止め、現行マイグレーション台帳を厳密に読み取り検証します。`predeployed-bridge` は公式 v66/v67 移行中だけ使う exact two-ledger bridge です。物理台帳が exact v66 の間は通常 route を維持しますが、v67 専用 Interface intent を必要とする explicit/sealed Plan sidecar、commit/restore、drain を fail closed/no-work にし、exact v67 検証後だけ有効化します |
