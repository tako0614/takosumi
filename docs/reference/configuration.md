...
| 変数                                   | 必須                           | 既定値      | 決めること                                                                                                                                   |
| -------------------------------------- | ------------------------------ | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `TAKOSUMI_CONTROL_D1_SCHEMA_MODE`      | 任意                           | `bootstrap` | `bootstrap` はリクエスト時にスキーマを用意します。`predeployed` はそれを止め、現行マイグレーション台帳を厳密に読み取り検証します。`predeployed-bridge` は公式 v66/v67 移行中だけ使う exact two-ledger bridge です。物理台帳が exact v66 の間は通常 route と InstallConfig re-adoption を維持しますが、v67 専用 Interface intent を必要とする explicit/sealed Plan sidecar、commit/restore、drain を fail closed/no-work にします。re-adoption は fresh exact-v66 proof の場合だけ不存在の intent table を省略し、v67 では Capsule CAS と intent retire を同一 batch に保ちます。exact v67 検証後だけ Interface intent 機能を有効化します |
