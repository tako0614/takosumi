...
inventory の実装例です。reference Takosumi kernel implementation では、operator
が必要な package を import して reference adapter array
...
When an operator profile adopts the official gateway descriptor and maps the
`gateway` alias, that descriptor represents listener / TLS / route rule intent.
reference provider package は gateway provider factory を export でき、その内部
lifecycle で `Route53` / `Cloudflare DNS` / `CoreDNS` などの DNS client /
sub-adapter を使えます。DNS provider rows are sub-adapters used by gateway
providers; AppSpec authors use the alias or descriptor URI accepted by their
operator profile.
...
not arbitrary path routing. If an AppSpec asks for route features the selected
...
AppSpec portability は matching kind URI、material contract、operator-selected
support constraints / extension fields、credential、operator evidence が揃う範囲
で成立します。
...
| `cloudflareWorkerProvider`        | `@takos/cloudflare-worker` | `worker`                     | `scale-to-zero`, `websocket`, `long-request`, `geo-routing` |
...
Azure は external connector metadata example です。別 operator distribution が
