...
## 単一リージョン不変条件 {#single-region-invariant}
...
([Resource IDs](./resource-ids.md) 参照)。 例: `az-1a`、`az-1b`、`rack-c`。
kernel は値を解釈しない: 似た名前の 2 つの zone は無関係で、 kernel
は文字列形から隣接性や距離を推論しない。
...
record はそれぞれの zone field を [Storage Schema](./storage-schema.md)
に従って永続化する。

## AppSpec 境界 {#appspec-boundary}
...
## クロスゾーンリンクポリシー {#cross-zone-link-policy}
...
- zone-down 状態を検知した connector (provider API が zone
  ...
  ([Drift Detection](./drift-detection.md) 参照)。 drift event は zone
...
zone 関連の audit event ([Audit Events](./audit-events.md) 参照):
...
zone field は [Storage Schema](./storage-schema.md) に整合する既存 record class
上に永続化される。
...
- [Manifest — Expand Semantics](./manifest.md#expand-semantics)
