...
Queues / Workflows として見せます。Workers for Platforms は Workers Script を実現する内部
...
`resourceMetadata.backend: "cloudflare.workers_for_platforms"` のような内部実装名も
public usage / billing payload には入れません。
