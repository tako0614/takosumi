...
Cloud の API key は Takosumi Accounts personal access token です。S3-compatible
endpoint のように標準 protocol 自体が署名方式を持つ場合は、その protocol の署名を使います。
...
Resource Shape API は `takosumi_*` provider resources、CLI、dashboard、compatibility
facade が使う canonical API です。
