...
Takosumi は first-party Terraform/OpenTofu provider を同梱しません。portable Form と
...
credential 自身が Workspace に紐づく場合 (`workspace_id` を指定した personal access
token、Capsule の Workspace に発行時 bind される OAuth access token) は、`/api/v1` の
すべての route でその Workspace 以外を扱えません。subject が他の Workspace の member
であっても `403` になり、`GET /api/v1/workspaces` にも自分の Workspace しか現れません。
制限のない account session / account-wide PAT の挙動は変わりません。
...
Dashboard などの対話的 client は Workspace 一覧を bounded page で読みます。
`limit` は最大100、`cursor` は応答の opaque token、`order` は
`created_asc` または `updated_desc` です。現在選択中の Workspace を
`selectedWorkspaceId` に指定すると、その Workspace が通常の page 外でも認可確認後に
追加1件として返り、`pinnedWorkspaceId` に記録されます。この場合も通常 page は
`limit` 件、応答全体は最大 `limit + 1` 件です。
...
page 応答は `workspaces`、`returned`、`limit`、`truncated` と任意の
`nextCursor` / `pinnedWorkspaceId` を持ちます。exact `total` が必要な管理操作だけ
`includeTotal=true` を指定します。通常の対話的readはこれを省略し、D1/Postgresで
`count(*)` を追加せず `limit + 1` probeだけで完結します。queryなしの
...
hostname の予約は `public_endpoint` projection がある Capsule config だけで走るため、
per-install patch (`PATCH /api/v1/capsule-configs/:id`) はこの projection を削除できません
(削除すると endpoint 変数だけが残り、予約と所有チェックを飛ばせてしまいます)。
...
具体的には、managed base domain を持つ operator では `public_endpoint` projection の
...
provider固有の別ledgerやauthoring経路は持ちません。
...
endpoint では、billable preview が exact OSS `OfferingSelection`、closed
`CommercialOfferingBinding`、versioned `PriceCatalog` に基づく `DeploymentQuote` を返し、apply は
`quoteId + quoteDigest` を必須とします。quote は Resource spec digest、Offering selection / resolution
fingerprint、commercial binding / PriceCatalog version、SKU line items、currency、estimated total micros、
issued/expiry を固定します。Cloud は backend 作業前に reserve、Resource
...
Resource だけを全 Space 横断の古い順に、bounded な永続 lease で重複なく観測します。
...
support を OSS policy record に混ぜられません。Form-backed Offering resolver はこの exact identity
と activation を要件として再確認し、商用 availability はその `OfferingSelection` を参照する closed
...
module/adapter、FormActivation の scope/audience から安全側に停止する形で導出されます。
この discovery は書き込みの admission gate でもあります。`FormRef` と `packageDigest`
は公開された content identity なので、`/v1/resources` の preview / apply / import は
package が installed であることに加えて、その principal を許可する `status: "active"`
な FormActivation を要求します。該当する activation がない request は
`form_not_activated` (HTTP 409、portable host では `form_unavailable`) で失敗します。
既に pin 済み Resource の recovery / replay path は、後から activation を外しても
teardown が詰まらないよう retained package と同じ扱いを続けます。
Target 名、implementation/manager identity、credential、region、raw capacity は返しません。
価格、SKU、請求、Cloud commercial binding は OSS discovery には入りません。
...
### Generic Offering catalog / availability / selection API

OSS operator は Service Form に限定されない exact noncommercial Offering catalog を公開し、principal は availability を
照会して exact selection を解決できます。
...
catalog の各 Offering は exact `catalog id/version + offering id/version`、open な subject
`type + ref + version + digest`、exact requirement reference、audience、profile、region、maturity、active state を持ちます。
selection は exact subject/requirements、resolver id、resolution fingerprint、resolved timestamp を固定します。`latest` fallback
はなく、unknown subject type、resolver 不在、inactive catalog/Offering、audience 不一致、stale requirement は fail closed です。
Form subject の resolver は exact FormRef / package / FormActivation を再確認しますが、non-Form subject も別 resolver で同じ
engine を使えます。

OSS Offering / selection に price、SKU、payment、manager、credential、raw capacity、SLA、support は入りません。Takosumi
Cloud は Core が返した exact `OfferingSelection` にだけ closed `CommercialOfferingBinding` を付け、implementation /
manager / capacity / SKU / PriceCatalog / payment evidence を固定します。Cloud は parallel catalog decision や second
selection engine を持ちません。
...
class を暗黙に変更する selector ではありません。
...
Takosumi host API/adapter conformance が必要です。
...
で必要な scope を宣言できます。`openid` は必須です。client id は Capsule が所有し、
`clientIdVariable` に他 Capsule の client id を入れた install は
`failed_precondition` (`oidc_client_id_already_bound`) で止まります。Accounts が発行する
