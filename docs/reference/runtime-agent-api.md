...
runtime-agent は reference Takosumi topology で使う execution host です。 operator が cloud / OS credential を握る host で起動し、Takosumi の下流 execution surface として runtime-handler-local selector 単位の lifecycle リクエストを受けます。
...
| ------------ | ---------------------- | ----------------------------------- | ------------------------------- |
| Agent bearer | `TAKOSUMI_AGENT_TOKEN` | `/v1/lifecycle/*`、`/v1/runtime-handlers` | `Authorization: Bearer <token>` |

`TAKOSUMI_AGENT_TOKEN` は Takosumi と runtime-agent が共有する shared secret。未設定起動の agent は lifecycle / runtime-handlers request 全部に 401。 `/v1/health` のみ無認証で orchestrator probe (Kubernetes / Nomad / docker healthcheck) を想定。

prepared source を読む runtime handler には、reference dispatcher / operator-configured Takosumi が `LifecycleApplyRequest.preparedSource` に resolved source view の runtime-agent transport locator を載せて渡す場合があります。`workingDirectory` は co-located / operator-local dispatch 用の transport locator で、Installer API の `source.kind: "local"` ではありません。portable remote agent には `url` + `digest` を渡します。prepared handoff では Installer API は引き続き `source.kind: "prepared"` を受け取ります。runtime file path は source-root-relative path です。

operator が optional asset extension として `/v1/artifacts` を mount しており、asset bytes 取得が必要な runtime handler には、 reference dispatcher / operator-configured Takosumi が `LifecycleApplyRequest.artifactStore` に `baseUrl` と `TAKOSUMI_ARTIFACT_FETCH_TOKEN` を載せて渡す場合があります。この operator-mounted asset extension は agent token と別の credential family を使い、scope は optional asset extension の `GET /v1/artifacts/:hash` のみ ([Authentication](./service-http-api.md#authentication))。 `artifactStore.baseUrl` は operator-owned artifact endpoint を指してよく、 Takosumi Installer API process が blob data plane になることを要求しません。
...
| ------ | -------------------------- | ----------- | -------------------------------------------------------------------- |
| GET    | `/v1/health`               | -           | `{ status: "ok", handlers: <count> }`                              |
| GET    | `/v1/runtime-handlers`           | Agent token | 起動時に登録された runtime-handler-local selector と acceptedArtifactKinds |
...
| POST   | `/v1/lifecycle/verify`     | Agent token | runtime handler ごとに `verify` operation を smoke test                    |
...
interface LifecycleApplyRequest {
  readonly shape: string; // runtime-handler-local selector, e.g. "object-store@v1"
  readonly provider: string; // 例: "aws-s3"
  readonly resourceName: string; // component / internal resource name
  readonly spec: JsonValue; // runtime-handler-local lifecycle input projected by the selected adapter
...
runtime handler は `destroy` を delete-if-exists な冪等動作として実装します。実体が既に消えている場合は HTTP 200 + `ok: true` + `note` で返すのが推奨。

WAL-backed lifecycle destroy でも apply と同じ internal key が転送されます。削除 API に external request token が無い provider は、runtime handler 内部の local ledger / tag / annotation で同じ key を使って重複 side effect を抑止します。
...
`compensate` は commit 済み effect を逆再生する runtime handler-native operation です。専用 operation が無い runtime handler は handle-keyed `destroy` に fallback。完全に逆再生できない場合は `revokeDebtRequired: true` を返し、 Takosumi が CleanupBacklog として保持します。
...
`describe` は runtime handler の read-only API (`HeadBucket` / `DescribeService` / `docker inspect` / `systemctl is-active` 等) で実体を毎回問い合わせる方式。 Takosumi apply 時の outputs に依存しないので、 runtime-agent restart 後も同じ結果を返せる必要があります。

### `POST /v1/lifecycle/verify` {#post-v1-lifecycle-verify}

runtime handler の `verify` operation を smoke test します。Request:
...
`shape` / `provider` を省略した場合は登録済 runtime handler 全てを対象にします。Response:
...
interface LifecycleVerifyResult {
  readonly shape: string; // verify 対象の runtime-handler-local selector
  readonly provider: string; // verify 対象の provider id
  readonly ok: boolean; // smoke test 結果 (true = 健全)
  readonly code?: string; // ok=false 時に設定される runtime-handler-local code
  readonly note?: string; // 詳細メッセージ (operator が読む)
}
```

`results[]` は順序保証なし。 caller は runtime-handler-local selector で集計します。

## Runtime handler retry / credential refresh {#runtime-handler-retry--credential-refresh}

runtime-agent Takosumi は `withRuntimeHandlerResilience()` を提供します。operator-owned boot code は runtime handler の lifecycle operation をこの
wrapper で包めます。 retry 対象は次のみ:

- `HTTP 408` / `425` / `429` / `500` / `502` / `503` / `504`
- `TypeError` / `ECONNRESET` / `ETIMEDOUT` などの network failure
- `retryable: true` を持つ runtime handler error

`HTTP 400` 等の backend validation error、`retryable: false`、permission denied 等の恒久 failure は retry せず。retry は bounded exponential backoff で同じ envelope を再投入します。runtime handler は `idempotencyKey` / backend-native client token / handle-keyed delete で重複 side effect を抑止します。

credential refresh は opt-in。 `RuntimeHandlerResilienceOptions.refreshCredentials` を渡した場合のみ、 wrapper は `HTTP 401` / expired token を検出して
refresh を 1 回呼び、同じ operation を再試行します。 refresh 未設定なら credential error は通常の runtime handler failure として返ります。
...
- `apply` 成功は `running` に遷移。失敗は `runtime_handler_failed` を返し Takosumi 側で `error` projection。
...
- `verify` は status を materialize しません。 runtime handler credential / network reachability の health probe に専念し、結果は `LifecycleVerifyResponse.results[].ok` に集約します。
...
interface LifecycleErrorBody {
  ...
  readonly code: LifecycleErrorCode; // closed enum + runtime-handler-extended:* 予約
  readonly retryable?: boolean;
  readonly details?: JsonObject;
}
```

`code` は reference runtime-agent レスポンス v1 の closed enum。current reference implementation では `runtime-handler-extended:` prefix を runtime handler 拡張用に予約し、Takosumi は共通 error logic に載せず runtime handler の string をそのまま actor に伝えます。

| `code`                   | HTTP   | 発生条件                                                             |
| ------------------------ | ------ | -------------------------------------------------------------------- |
| `unauthorized`           | 401    | bearer 不足 / mismatch                                               |
| `bad_request`            | 400    | request body validation 失敗                                         |
| `runtime_handler_not_found`    | 404    | selector に対応する runtime handler が registry にいない                   |
| `artifact_kind_mismatch` | 400    | asset-backed runtime handler の operator asset metadata と spec が合わない |
| `runtime_handler_failed`       | 500    | runtime handler が throw した想定外エラー                                  |
| `runtime-handler-extended:*`   | (任意) | runtime handler 拡張用の予約 prefix                                        |
