...
deletion API を internal caller として呼び出し、 soft-delete → hard-delete の
...
[Compliance Retention](/reference/compliance-retention) の regime minimum と
整合する範囲でのみ許可される。 `pci-dss` / `hipaa` / `sox` regime の Space は
短縮できない。 operator は trial に該当 regime を割り当てない設計を取る。
