...
Shape manifest から concrete deployment plan を生成する **provider resolution**
のアルゴリズムを定義します。 `resources[].provider` を optional hint にした上
で、 operator policy / provider registry が何を決めるかを明確にします。
...
- `shape` — resource の semantic contract (例: `worker@v1`)
- `spec` — shape validator 済み desired state (例: `{ artifact, routes }`)
...
- `catalogRelease` — shape / provider / template release pin (digest / release
  id)
