...
本ページは `takosumi-api` role が mount する reference route inventory です。
public contract は [Installer API](./installer-api.md) を参照。internal control
plane、runtime-agent control RPC、probe route は operator runtime surface です。

> 実装は reference Takosumi の Hono router 群です。role ごとに mount される route
...
すべての endpoint は Takosumi の base URL に対する相対 path です。credential は
Takosumi が保持せず、operator が env 経由で inject します。

## 認証 {#authentication}

Takosumi は v1 で 2 種類の credential を区別し、credential ごとに作用範囲を
完全に分離します。
...
- `TAKOSUMI_INSTALLER_TOKEN` が unset の間、Installer API route は
...
asset route は operator extension surface です。mount する operator は
...
asset URL や token は Deployment outputs に出さず、operator evidence /
...
dev/operator-internal convenience surfaces; core conformance is the
Installer API above.
...
public installer のエラーレスポンスの正本は [Installer API](./installer-api.md)
です。Installer API では、syntax / local reference / cycle は
`400 invalid_argument`、well-formed だが Space で未採用 / 不可視 / unavailable
な kind・出力の形式・注入モード・プラットフォームサービスは
`409 failed_precondition`、採用済み kind の定義や extension に対する binding が
無い場合は `501 not_implemented` です。reference Takosumi の internal / probe
route は同じレスポンス shape と code set を使い、route 種別に応じて次の code を
返します。
...
- [Build service handoff](./build-spec.md) — build service と prepared source の
  handoff
- [Takosumi Kind Catalog Specification](./type-catalog.md) — official
  kind schema and output type vocabulary
- [Reference Runtime-Agent Execution Surface](./runtime-agent-api.md)
