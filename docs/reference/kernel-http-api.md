...
> このページでわかること: kernel の HTTP API エンドポイント一覧。
...
`provenance` は upstream deploy client が供給する optional な JSON object で、
kernel は opaque な audit evidence として扱います。 例えば `takosumi-git push`
は `kind: "takosumi-git.deployment-provenance@v1"` に workflow run id、 git
commit metadata、 artifact URI、 step log digest を 入れて送ります。 kernel
は値が JSON object であること、 `kind` がある場合は
文字列であることだけを検証し、 workflow を実行したり workflow file を読ん だり
build log を parse したり git semantics を解釈したりはしません。

operator / account plane の依存は kernel deploy 前に namespace export / account
API / OIDC discovery / BillingPort contract で解決します。 kernel deploy route
が受け取るのは compile 済 Shape manifest のみです。

public deploy scope は single token です。 `TAKOSUMI_DEPLOY_TOKEN` が operator
設定の 1 つの public deploy Space / tenant scope に対応し、 既定 は
`takosumi-deploy`、 設定は `TAKOSUMI_DEPLOY_SPACE_ID`。 actor 単位の multi-Space
auth、 entitlement check、 policy-gated Space routing は internal control plane
の責務で、 public manifest body から推論しません。
...
DesiredSnapshot / OperationPlan digest model と一致する deterministic な値 で、
request は副作用なし・ WAL 書込なしです。

`mode="plan"` / `mode="apply"` では、 `spec.artifact.size` を宣言する manifest
resource を WAL 書込や provider 呼出の前に検査します。 値は 非負整数 byte 数で、
登録された artifact-kind の `maxSize` を超えてはい けません。 未登録 kind は
`TAKOSUMI_ARTIFACT_MAX_BYTES` (既定 50 MiB) に fallback。 超過時は 413
`resource_exhausted`。

`mode="apply"` / `mode="destroy"` では、 内部で同じ public OperationPlan shape
を導出し、 provider 副作用の前後で `takosumi_operation_journal_entries`
を書きます。 provider 呼出前は `prepare` / `pre-commit` / `commit`、 成功時 は
`post-commit` / `observe` / `finalize`、 失敗時は `abort` を追記します。
これらは public surface の durable な replay 証跡で、 provider 呼出には WAL
idempotency tuple を fencing token として渡します。 `provenance` が指定さ
れた場合、 OperationPlan digest を導出する前に resolved resource ごとに
`metadata.takosumiDeployProvenance = { kind: "takosumi.deploy-provenance-digest@v1", digest }`
を attach し、 各 public WAL effect detail に provenance object 全体を持た
せます。 これで replay identity を artifact provenance に紐付けつつ、 kernel は
upstream workflow を理解しない構成を保てます。

該当 deployment の最新 public WAL が terminal (`finalize` / `abort` / `skip`)
でない場合、 新規 `apply` / `destroy` は provider 呼出前に 409
`failed_precondition` で fail-closed します。 `recoveryMode: "inspect"` は新規
journal entry を書かず、 provider も呼ばず に persist 済 public WAL を返します。
`recoveryMode: "continue"` は同じ `mode` で OperationPlan digest が一致する
manifest のときだけ同 operation を replay。 phase / digest が違う場合は 409 で
fail-closed。 `recoveryMode: "compensate"` は `commit` 以降に到達した一致 WAL
に対して terminal `abort` を追記し、 `activation-rollback` の RevokeDebt
を開きます (provider は呼ばない)。
...
この route は public query parameter を受け付けません。 pagination、 `kind`、
`since`、 cross-Space `space=*` filter は internal status / control-plane
surface 側に予約されています。
...
現在は以下の署名付き internal route のみ mount されています。
...
`permission_denied` を返します。route helper は `TAKOSUMI_INTERNAL_API_SECRET`
を internal HMAC secret として読みます。
...
これらの route を追加するには、 対応する route 実装、 authorization test、
必要に応じた OpenAPI / capabilities 更新、 本 reference 更新が要ります。

## Workflow / trigger / hook の境界

kernel は workflow / trigger / schedule / declarable hook の HTTP route を
持ちません。 workflow 形の automation は kernel の上位 `takosumi-git` の
責務で、 `.takosumi/manifest.yml`、 `.takosumi/workflows/*.yml`、
`resources[i].workflowRef`、 build 実行、 webhook / cron 連携、 artifact URI
discovery を所有します。 resolve 後、 `takosumi-git` は private な `workflowRef`
拡張を剥がして plain な Takosumi v1 manifest を `POST /v1/deployments`
に送ります。

The current kernel exposes no workflow, trigger, schedule, or declarable hook
HTTP route.

v1 では kernel に public / internal の trigger 登録 API は存在せず、 本
reference でも trigger endpoint schema を予約しません。 kernel に workflow route
を持たせるには、
...
[Public Spec Source Map](/reference/public-spec-source-map) の更新が必要に
なります。

WAL では kernel 所有の CatalogRelease 署名再検証は行いますが、 実行可能な hook
package を load することはしません。
