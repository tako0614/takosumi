...
the stage-polling flow are spec-reserved. :::
...
- Space は quota / audit chain / secret partition / observation set の 独立 unit
  で、current v1 では cross-Space の連携を許可しない。SpaceExportShare は future
  RFC vocabulary。
...
- approval queue / RevokeDebt queue は空。cross-Space share state は reserved /
  future RFC なので current v1 provisioning state では作らない。
