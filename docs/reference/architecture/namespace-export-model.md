...
prepared source snapshot は Installer API の source input です。DataAsset は
operator extension が保存する content-addressed blob です。DataAsset の可視性は
Space scope であり、operator の DataAsset policy が明示的に共有を許可した場合
のみ scope を超えます。
...
prepared source snapshot を受け取る。operator-owned DataAsset upload は別の
extension surface です。
...
service、CI、automation で実行されます。Kernel v1 は git source または prepared
source を受け取り、source digest を検証し、AppSpec を parse して Deployment
evidence を記録します。Transform approval が必要な場合、それは build service /
operator policy で扱います。
