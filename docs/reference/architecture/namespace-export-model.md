...
kernel v1 は任意のユーザーシェル build を直接実行しない。source build は
BuildSpec を読む build service または CI が担当し、kernel は digest-pinned
...
`GET /v1/artifacts/kinds` から discover できる。Takosumi AppSpec は artifact
kind catalog を持たない。
...
- Artifact-backed connector は `acceptedKinds` ベクトルを宣言する。source-backed
  connector は prepared source locator と kind-specific `spec` を読む。
...
`listen` の `as: target` で表現され、別の top-level `expose` object
で表現しない。Exposure は Link ではない。
