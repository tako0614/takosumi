...
BuildSpec を読む build service または CI が担当し、kernel は resolved AppSpec
bundle 内の digest-pinned artifact reference を受け取る。
...
し、返ってきた digest を AppSpec または provider-specific artifact material に
渡す。
...
.takosumi.yml spec.artifact path -> build service -> resolved spec.artifact.hash
