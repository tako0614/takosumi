...
この section は Takosumi kernel の internal architecture notes です。public contract
は [AppSpec](../app-spec.md) と [Installer API](../installer-api.md) を参照して
ください。
...
component の公開 field は `kind`、`spec`、`publish`、`listen`、`build` です。
provider id、route、artifact、custom domain などの詳細は kind の `spec` または
