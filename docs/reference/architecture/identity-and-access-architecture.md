...
AppInstallation / use edge / permission grant に関する identity & access は
operator の account plane が所有する (reference 実装: `takosumi-cloud/` の
Takosumi Accounts)。 takosumi kernel は generic AppSpec installer engine
として動作し、 account-plane identity を所有しない。
