...
owner) が責務を持ちます。Installed Takos は Takosumi Accounts の OIDC を
`AUTH_DRIVER=oidc` として consume するだけ で、kernel 自身は OAuth/OIDC
を発行しません。
...
bundled app distribution / default app set / preinstall は Takos product と
Takosumi Accounts / takosumi-git の install-layer concern です。kernel は
...
`/auth/*` 系の OAuth issuer endpoint を **持ちません**。Installed Takos は OIDC
consumer として Takosumi Accounts に redirect し、callback path
...
`takos.api-key` catalog metadata や top-level `bindings[]` env injection を
current manifest contract として扱いません。
