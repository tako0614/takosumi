...
kernel control はそれらの session state や callback/JWKS verification
を所有しません。
...
- **kernel API process** — compiled manifest / Deployment / GroupHead /
  ...
  trigger。 OAuth/OIDC issuer、OIDC consumer callback、Stripe webhook、Takos
  chat UI は 処理しない
...
- **runtime host process** — legacy Takos compatibility の
  `takos-runtime-service` container を host / forward する backend-specific
  adapter process role。current service id ではない
- **executor host process** — agent run の executor container を host する Takos
  product process role (`/architecture/container-hosts`)。kernel compute control
  の正本ではない
...
この節は Takos product の runtime-service compatibility path
を説明する参考情報です。 kernel control の ownership ではありません。operator
workstation の Takos CLI から runtime-service の CLI-proxy endpoint
(`/cli-proxy/*`) に到達する flow:

1. CLI が Takosumi Accounts bearer 付きで Takos app/API gateway に HTTPS POST
   する
2. Takos app/API gateway が Accounts bearer を issuer / introspection
...
[Takosumi Accounts](https://github.com/tako0614/takosumi-cloud/blob/master/docs/architecture/takosumi-accounts.md)、[AppInstallation 台帳](https://github.com/tako0614/takos-ecosystem/blob/master/docs/platform/app-installation.md)、
...
Takos product features such as Agent / Chat, Git hosting, Storage, Store,
app-local sessions, and app-local workflows are not kernel schema ownership.
They live in their owning Takos product roots / services and may consume kernel
outputs through published APIs or AppInstallation bindings.
...
compiled Shape manifest の `resources[]` に適用される。docs, excel, slide,
computer, yurucommu, user-defined workloads は同じ resource / provider model で
deploy できる。Takos の agent / chat / Git hosting / storage / Store は Takos
product 側の service / app-layer 機能であり、takosumi kernel に統合しない。
...
takosumi-git / operator / Takos app gateway
  → takosumi-kernel-api (Deployment API)
     ...
     → legacy takos-runtime-host (backend adapter / runtime-service compatibility host)
```
...
#### Legacy `takos-runtime-host` (Workers compatibility)

`runtime-host` は旧 compatibility 実装名であり、current service id
ではありません。残る文脈は 2 つです。

- backend-specific runtime で Worker `worker-bundle` を扱う deployment adapter
  名
- legacy `takos-runtime-service` container を host / forward する Cloudflare
  worker
...
- ②: container mode では (3) のように service binding 経由で legacy runtime-host
  を呼ぶ。これは worker bundle / runtime-service host 経路であり、container
  workload の materialization そのものではない。通常の direct HTTP mode では
  kernel から runtime-service の `/cli-proxy/*` を直接呼ぶ (loopback bypass は
  使わない)
- ③ → ④: `RUNTIME_HOST` service binding は Cloudflare worker binding
  なので、kernel → legacy runtime-host は net hop を経由しない
- ④: legacy `runtime-host` は container 内の deno process に loopback で到達する
...
- ⑤ → ⑥: runtime-service が実接続元を `127.0.0.1` / `::1` / `::ffff:127.0.0.1`
  ...
  の照合で space 分離が保たれる
- ⑦ → ⑨: legacy runtime-service は repo 操作を直接実行せず、session の proxy
  token で legacy runtime-host の `/forward/cli-proxy/*` に戻す。legacy
  runtime-host が token を検証し、Takos app/API gateway の `/api/repos/:id/*`
  に中継する
- `X-Forwarded-For` の spoof 防止は **ingress 側の責務**: CF Container は
  外部から渡された header を丸ごと渡すので、kernel / legacy runtime-host で
