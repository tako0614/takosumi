...
implementation binding は operator が Takosumi に attach する implementation であり、コントロールプレーンの一部ではありません。コントロールプレーンは kind-agnostic な状態管理に専念し、kind ごとのリソースの作成・更新は binding に委譲します。
