...
Takosumi kernel control の視点では、control plane は compiled Shape manifest を
Deployment として record し、`applied` 遷移と GroupHead 進行で route projection
を materialize する process role の集合。Group は primitive を任意に 束ねる
state scope であり、runtime backend や resource provider ではない。 current
manifest では workload / database / domain などを `resources[]` の `shape` /
`name` / `provider` / `spec` で固定し、resource 間配線は `${ref:...}` /
...
kernel control が提供する API。account / OAuth issuer / billing API、 consumer
application の browser/API gateway (例: Takos app の browser/API gateway)、 OIDC
consumer callback は本ページの対象外で、それらは
...
および各 consumer application 側 (例: Takos app/API gateway) の正本を参照。
...
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| Spaces       | compute namespace / Space id reference。membership と app-local profile は consumer application (例: Takos app) / Takosumi Accounts 側 |
...
Accounts が AppInstallation を所有し、 consumer application / installer layer
(例: reference Takos distribution) は installation id に紐づく app metadata
を管理します。kernel control は metadata が参照する workload URL / route output
を resource output として返すだけです。
...
consumer application service / app feature (例: reference Takos distribution の
service set) であり、takosumi kernel feature ではありません。
...
2. kernel が起動する（compiled manifest API / provider adapters / routing が
   ready）
3. bundled app distribution / preinstall は consumer application distribution と
   Accounts install layer が扱う (例: reference Takos distribution が ship する
   bundled InstallableApp 群)。kernel は default app list を持たず、 compiled
   manifest apply の準備ができていればよい。
...
```text
takosumi-git / operator / consumer application gateway (例: Takos app gateway)
...
Browser-facing consumer application routes (reference Takos distribution の
場合は `/_takosumi/launch`, `/auth/oidc/callback`, `/settings`, chat SPA, public
product API 等) は kernel API ではありません。OIDC issuer / login UI / consent
screen は
[Takosumi Accounts](https://github.com/tako0614/takosumi-cloud/blob/master/docs/architecture/takosumi-accounts.md)
が serve し、OIDC consumer session は installed consumer application (例:
Installed Takos distribution) が扱います。
...
backend 実装によっては SQL resource に対して schema change を実行できる。public
manifest contract は schema 変更の実行手順を記述しない。resource API / runtime
binding と schema change は backend 側で管理する。

### Rollback と schema change

group deployment record には schema-change 状態が含まれる。rollback 時、kernel
は forward-only schema change のみサポートする。schema
を巻き戻す必要がある場合は、新しい schema change として書く。

schema change が失敗した場合、deploy 全体が fail する。group の worker
は起動しない。
...
> このセクションは **reference distribution** materialization (例: Takos
> distribution が Cloudflare Workers backend で running している 場合) に固有の
> materialization detail。 kernel contract そのものではなく、 tracked reference
> の具体的な worker 配置・binding・cache 階層を扱う。 Core 用語との対応は
...
Cloudflare worker と Container DO に展開される。 reference distribution (例:
Takos distribution) の consumer application app/API gateway worker は 別 product
surface として同じ distribution に同居し得るが、kernel control role ではない。
...
consumer application 側の browser-facing surface (reference Takos distribution
の場合は Takos chat SPA、`/api/*` product gateway、`/_takosumi/launch`,
`/auth/oidc/callback`, `/settings`, setup UI 等) は installed consumer
application app/API gateway worker の責務です。
