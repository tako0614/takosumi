...
- **executor host process** — agent run の executor container を host する Takos
  product process role。kernel compute control の正本ではない
...
この節は Takos product の runtime-service path を説明する参考情報です。 kernel
control の ownership ではありません。operator workstation の Takos CLI から
