...
5. operator implementation config change
   The operator-provided PlatformService / binding visibility used by the Space has changed.
...
- **fix-hint 生成 stage**: WAL の `prepare` stage で `safeFix[]` (resolved target state 上で自動補正可能な提案) が、`pre-commit` stage で `requiresPolicyReview[]` (approval が要る昇格) が、`commit` / `post-commit` / `observe` / `finalize` stage で `operatorFix[]` (operator 介入が要る) が生成されます。詳細 stage 定義は [Operation Plan & Write-Ahead Journal](./runtime-deployment-model.md#operation-plan--write-ahead-journal) 。
