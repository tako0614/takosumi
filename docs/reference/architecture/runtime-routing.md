...
このページは reference architecture の internal routing model です。Takosumi v1 の public Source authoring は manifestless です。`RoutingPointer` や `TrafficSnapshot` は operator-selected routing state を記録する internal evidence であり、public core entity ではありません。
