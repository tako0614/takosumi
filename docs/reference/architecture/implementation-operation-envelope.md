...
[Runtime-Agent API](../runtime-agent-api.md) と
...
に存在し、そのような操作が許されるかどうかを判断する ものは kernel に存在する。
...
/ operator UI で 判定され、runtime-agent は報告するだけである。意図される配置は
...
ない限り、別 Space の object、secret、DataAsset、grant、namespace export を読み
