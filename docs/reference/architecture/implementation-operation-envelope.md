...
kernel / runtime-agent 境界と、両者が交換する operation envelope を記録する。
public concepts は AppSpec / Installation / Deployment であり、public Installer
API は 5 endpoint です。runtime-agent は operator execution surface
です。wire-level の field 表は
...
kernel core/control-plane role は state を orchestrate する。Space、
ResolutionSnapshot、DesiredSnapshot、 OperationPlan、Write-Ahead Journal、policy
評価、Approval 簿記、RevokeDebt を所有する。kernel core は cloud / OS credential
を保持せず、resource を materialize する side-effecting な外部 I/O も実行しな
い。
...
に存在し、そのような操作が許されるかどうかを判断するものは kernel に存在する。

## Packaging の自由度

reference Takosumi runtime-agent topology では、kernel と runtime-agent の間で
...
role は kernel core/control-plane role から分けて扱う。production 推奨 topology
...
contract は AppSpec と Installer API、そして Installation / Deployment の
...
- `succeeded` —承認済み effect がすべて適用された。kernel は WAL を `commit`
  ...
  Deployment に紐付ける。`ResolutionSnapshot` は immutable のまま変更しない。
...
比較し、`commit` への遷移を許可するか判断する。`actualEffects` は `commit` /
...
connector module はその pair の DataAsset / handle shape と lifecycle operation
...
に到達できるか」に答える。 health は `LifecycleVerifyResult.ok` をもとに kernel
...
kernel → runtime-agent 方向の認証方式は operator distribution の internal
...
object、secret、DataAsset、grant、external publication を読み
...
- **Debt.** effect が適用されたが desired state と reconcile できないとき
  ...
  historical `approvedEffects` は遡って変更しない。Debt は kernel が「何が
