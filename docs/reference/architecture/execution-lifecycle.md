...
OperationPlan、dry materialization、Approval record、snapshot、journal は
internal / operator の実行概念です。
...
kind の定義や platform service を再解決しません。re-resolve が必要な場合は
...
このセクションは operator / reference kernel の approval 実装を記述します。core
の dry-run response は `changes[]` と `expected` に留まります。account layer の
approval prompt、prediction digest、approval token、risk summary は operator
extension field または account layer API record です。

resolution が `require-approval` Risk を surface したとき、reference Takosumi /
operator の approval 設定は **dry-materialized prediction** を生成し、承認者に
同意を求める effect digest を捕捉する。
...
の間に [Approval invalidation trigger](./approval-model.md) の
