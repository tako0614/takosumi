...
と、将来の port-level readiness design を分けて定義する。 liveness probe
との分離もここで明示する。
...
- `internalApiSecret`: `takosumi-api` / `takosumi-runtime-agent` role では
  ...
  (`TAKOSUMI_INTERNAL_SERVICE_SECRET` は deprecated compatibility alias)
...
response は design-reserved であり、current HTTP output ではない。
...
## Design-Reserved Dependency DAG
...
## Design-Reserved Observation propagation
...
## Design-Reserved Failure propagation
...
- `docs/design/operational-hardening-checklist.md` — readiness を運用 signal
  として活用する checklist
