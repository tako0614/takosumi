...
Takosumi Core は Form Package が 0 個でも plain Stack flow を実行できます。OSS の generic `Offering` /
`OfferingSelection` は open な subject type の noncommercial な availability と exact selection を所有します。
Takosumi Cloud の closed `CommercialOfferingBinding`、price、billing、official capacity はこの OSS host model の
外側にあります。
...
Offering は Service Form 専用 catalog ではありません。OSS operator は open な subject `type + ref + version + digest`、
要件、audience、profile、region、maturity を immutable な catalog version に公開します。host に明示的に install された
subject resolver が、subject と要件を exact に再確認します。Form-backed Offering の resolver は exact FormRef / package /
FormActivation を要件として再確認しますが、Capsule distribution や non-Form service も別の resolver を install して同じ
engine を使えます。

`OfferingSelection` は catalog id/version、Offering id/version、exact subject、要件、resolver id、resolution fingerprint を
固定した OSS の選択結果です。unknown subject type、resolver 不在、inactive catalog/Offering、audience 不一致、stale な要件は
安全側に停止します。price、SKU、payment、manager、credential、raw capacity、SLA、support は OSS Offering に入りません。
Takosumi Cloud はこの exact selection にだけ closed `CommercialOfferingBinding` を付け、implementation / manager / capacity /
SKU / PriceCatalog / payment evidence を固定します。Cloud が別の availability/selection engine を持つことはありません。
...
Takosumi/operator は implementation と generic FormActivation を明示します。operator は generic Offering resolver で exact
subject と要件を再確認して `OfferingSelection` を返します。Cloud が販売・公式運用する場合は、その exact selection に
closed `CommercialOfferingBinding` を付けます。標準/compat surface
...
Cloud の sellable model は OSS の exact `OfferingSelection` に閉じた `CommercialOfferingBinding` と versioned
`PriceCatalog` を組み合わせます。preview は Resource desired-state digest、Offering selection / resolution fingerprint、
commercial binding / PriceCatalog version、SKU line items、currency、estimated total、expiry、quote digest を固定した
`DeploymentQuote` を返します。
