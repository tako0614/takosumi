...
各 event は kernel buildVersion / schemaVersion / hostname / pid を含む。 Event schema は [Audit Events](./audit-events.md) に従う。
...
- Stage 6 の `kernel-bootstrap-completed` event が audit chain に存在すれば bootstrap は **skip** される
...
Bootstrap は **new install 専用**。 Schema migration の cross-version semantics、 kernel ↔ runtime-agent skew、 rollback gate は [Schema Evolution](./migration-upgrade.md) で扱う。
