...
> Stability: stable
> Audience: operator
> See also: [CLI](/reference/cli), [Environment Variables](/reference/env-vars), [Storage Schema](/reference/storage-schema), [Secret Partitions](/reference/secret-partitions), [Cross-Process Locks](/reference/cross-process-locks), [Catalog Release Trust](/reference/catalog-release-trust), [Audit Events](/reference/audit-events), [Migration / Upgrade](/reference/migration-upgrade), [Readiness Probes](/reference/readiness-probes)

Takosumi kernel の **初回起動 (new install)** における bootstrap 手順の
正本。本 reference は new install のみを対象とし、既存 install の
upgrade は [Migration / Upgrade](/reference/migration-upgrade) が扱う。
...
Bootstrap は kernel の **初回 1 回のみ** 実行される。完了後の再起動では
audit chain の genesis event を確認し、bootstrap 段階を skip する。
...
各 stage 完了で audit event を書く (後述)。Stage 5–7 は単一の
cross-process lock 下で直列化される (multi-pod bootstrap 参照)。
...
- Storage に既存 data がある場合は bootstrap を **skip** し、Migration
  / Upgrade path に切り替わる ([Migration / Upgrade](/reference/migration-upgrade))
...
Auto-generate の場合、kernel は 256-bit ランダム値を生成し、
secret partition の sealed envelope に保存する。Auto-generate は
**explicit confirm flag** (`TAKOSUMI_BOOTSTRAP_ALLOW_AUTO_KEY=1`) が
ある場合のみ許可される。Default は禁止。
...
- 初期化後 `bootstrap` 名義で TTL 60s の lock を取得し、stage 5–7 を
  serialize する
...
- 既に default operator account が存在する場合は bootstrap 全体が
  abort される (re-init 防止)

CLI 経由 (`takosumi init`) で起動すると、token は CLI 側にも copy される
ので scrolloff 後の取り戻しも可能。HTTP server を直接起動した場合は
stdout のみ。
...
1. `--catalog <path>` flag (`takosumi init`) で operator-supplied
   CatalogRelease を渡す
...
[Catalog Release Trust](/reference/catalog-release-trust) に従う。
検証失敗で stage abort。

Embedded default catalog は kernel build 時の signing key で署名済。
Trust anchor として default key が enrolled される。
...
- Opt out の場合 stage 6 は skip され、operator が後で
  `takosumi space create` を叩く必要がある
...
する唯一の entry で、後続の audit chain はすべてこれを root とする
hash chain で連鎖する。
...
各 event は kernel buildVersion / schemaVersion / hostname / pid を
含む。Event schema は [Audit Events](/reference/audit-events) に従う。

## Stage 8 — listener-open

Public deploy / internal control / runtime-agent / discovery ports を
open する。Bootstrap 完了前は `/readyz` は 503 を返し続ける。
...
- Stage 7 の `kernel-bootstrap-completed` event が audit chain に
  存在すれば bootstrap は **skip** される
- 中途 abort された install (例えば stage 5 で失敗し再起動) では、
  audit chain に `kernel-bootstrap-completed` がないので bootstrap が
  再走する
- Stage 1–6 は個別に idempotent (storage migration は up step が再走
  しても no-op、secret partition は既存 envelope を尊重、lock store
  は既存 row を upsert、default operator account / catalog release /
  default space は existence check で skip)

## `takosumi init` CLI

Bootstrap を起動する operator-facing CLI コマンド。

```bash
takosumi init \
  --storage-path ./data \
  --catalog ./catalog-release.yaml \
  --no-default-space    # optional
```

挙動:

- 既に bootstrap 完了済 storage に対して叩くと exit code 64
  (`already-bootstrapped`) で abort
- 完了時に default operator token を stdout に 1 行 print し、
  CLI config file (`~/.config/takosumi/config.toml`) に保存する
- `--non-interactive` flag で確認 prompt を全 skip

詳細フラグは [CLI](/reference/cli)。
...
- Lock を取れなかった pod は genesis event の出現まで poll する
  (poll 間隔 1s, max 10 min)
- Genesis event を観測した pod は stage 8 (listener-open) に進む
- 10 min の timeout を超えた pod は exit code 75
  (`bootstrap-wait-timeout`) で abort

Lock holder pod が途中 crash した場合、TTL 60s 経過後に他 pod が
lock を引き継ぐ。引き継いだ pod は stage 4 から再走する (idempotency
保証下で no-op or 続行)。
...
`kernel-bootstrap-completed` の payload に bootstrap 完了 wall clock /
duration を含める。
