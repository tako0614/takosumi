...
Operator profiles may carry approval prompts, approval records, and account
layer policy state outside the core Installer API.

## Trigger 6 値

この reference profile での新 trigger 追加は `CONVENTIONS.md` §6 RFC を要する。

1. **digest change** — TargetState digest または OperationPlan digest の
   変更。
2. **effect-detail change** — `approvedEffects` / `effectDetailsDigest` /
   generated authorization access / network egress shape の変更。
3. **implementation change** —選択された provider implementation / connector
   binding の変更。
4. **external freshness change** — operator-owned PlatformServiceDeclaration
   の freshness
...
- **発火条件**: prepare stage 中に再計算した `desiredSnapshotDigest` または
  `operationPlanDigest` が、approval record にバインドされた digest
  と一致しない。
- **検出 timing**: `prepare` stage の WAL append 直前。reference Takosumi が
  operator approval profile の binding を最後に validate する点。
...
- **発火条件**: prepare で resolve された `approvedEffects` /
  `effectDetailsDigest` / 個別 generated authorization access mode / network
  egress allow set が approval record の binding と一致しない。
...
- **発火条件**: operator-owned PlatformServiceDeclaration の freshness state
...
- **検出 timing**: operator implementation config / Space visibility 操作の
  commit 完了直後。reference kernel / operator approval profile は当該 Space
...
- **発火条件**: operator account layer membership version、scoped installer
  context、policy pack の変更。
- **検出 timing**: 該当 mutation の commit 完了直後。reference kernel / operator
  approval profile は operator から渡された authorization-context digest
  に紐づく approval を walk する。
...
- Trigger 1, 2 (digest 系) は **短絡 invalidate**: 他 binding を再評価せず即時
  `invalidated` 確定。
- Trigger 3-6 は plan を保ったまま発火し、reference Takosumi / operator approval
  profile は **影響範囲を minimum approval set に絞って propagate** する。Space
  全体や全 plan を巻き込まない。
...
`reviewing` は client UX のソフト状態で、reference kernel / operator approval
profile は永続化しない。

reference profile 側 state machine が永続化する server state は
...
これらの field は approval decision 時に固定され、勝手に書き換わらない。binding
...
- `docs/reference/architecture/approval-model.md` — approval
