# Takosumi 公式型カタログ {#type-catalog}

Takosumi core は kind-agnostic です。reusable な kind の定義 / output type
の語彙は、公式型カタログや operator distribution が供給します。この catalog は
...
identity、output type name、projection-family name、JSON-LD catalog metadata format
を定義します。kind の定義の repository source は kind package
ごとに置きます。公式 vocabulary を TypeScript で扱う code は
`@takos/takosumi-contract/type-catalog` を使えます。
...
Official type catalog の範囲:

- `https://takosumi.com/kinds/v1/*` 配下の kind の定義 identity
- `spec`、output slot、output vocabulary、expected な出力の型を説明する kind
  の定義 metadata field
...
platform service path、OIDC issuer operation、billing behavior、account layer
record、backend provisioning、dashboard API は、その vocabulary を採用する
operator distribution spec に置きます。
...
| Output type          | `http-endpoint`                                 | output slot が offer する material の型。                   |
| Injection mode       | `env`, `secret-env`, `upstream`, `config-mount` | resolved output material を consumer に渡す形式。           |
| Platform output type | `identity.oidc@v1`                              | platform service 用の reusable な出力の型。                 |
| Access metadata      | `invoke-only`, `restricted`                     | platform service material の access / projection metadata。 |

manifest は `kind` と `connect` / `listen` の `inject` を string として記録します。operator resolution が kind の定義 semantics を接続し、Space で見える catalog entry を選び、それを実現する binding を選びます。
...
| Suggested alias | Kind URI                                      | Package                              | Typical output slot                        |
| --------------- | --------------------------------------------- | ------------------------------------ | ------------------------------------------ |
...
| `sqlite`        | `https://takosumi.com/kinds/v1/sqlite`        | `@takos/takosumi-kind-sqlite`        | `connection` as `service-binding`          |
| `object-store`  | `https://takosumi.com/kinds/v1/object-store`  | `@takos/takosumi-kind-object-store`  | `bucket` as `object-store`                 |
| `kv-store`      | `https://takosumi.com/kinds/v1/kv-store`      | `@takos/takosumi-kind-kv-store`      | `store` as `service-binding`               |
| `message-queue` | `https://takosumi.com/kinds/v1/message-queue` | `@takos/takosumi-kind-message-queue` | `producer` / `consumer` as `event-channel` |
| `vector-store`  | `https://takosumi.com/kinds/v1/vector-store`  | `@takos/takosumi-kind-vector-store`  | `index` as `service-binding`               |
...
`takosumi-plugins/packages/kind-*` に置きます。そこでは descriptor と reference
plugin binding が同じ package に入りますが、plugin は AppSpec core 仕様ではなく
reference implementation の配線です。native kind の package 一覧は
...
ですが、公式型カタログの型は typo や未定義入力を許さないことを優先します。
...
Output type は component output slot または platform service declaration が offer
する出力データの portable shape を定義します。service path、backend
resource、dashboard route、account layer lifecycle は、その出力データを offer
する operator / product distribution spec に置きます。

Official output type は closed shape です。operator-specific field
を追加したい場合は、別の output type / catalog extension
...
path exposure と materialization result の property です。たとえば `web.http` は upstream HTTP
output、`public.public` は gateway / ingress component が作る public endpoint
output になれます。
...
`identity.oidc@v1` と `billing.port@v1` は platform service 用の neutral
な出力の型です。catalog は workload が受け取れる field を名付けます。issuer
operation、 client lifecycle、redirect policy、billing account lifecycle、usage
authorization、payment-provider integration、concrete platform service path
...
Injection mode は output material を consumer component
に提示する形式です。 manifest は selected family を `connect.<binding>.inject` または `listen.<binding>.inject`
に記録し、selected な kind の定義と operator policy が compatibility
をリソースの作成・更新前に検証します。
...
したい場合は、別の env-safe output type または explicit operator distribution
extension を定義します。
...
platform service や manifest 外の external service は `listen.<binding>.path`
で operator-defined service path を参照します。base URL のような public config
として渡す場合は `inject: env`、gateway / router の upstream binding
として渡す場合は `inject: upstream` を使います。secret-bearing output type は上の compatibility table
が優先されるため、`service-binding` / `object-store` / `identity.oidc@v1` /
`billing.port@v1` を plain `env` に落とすことは valid ではありません。
...
| `listens.<slot>.projectionMatrix`         | output type ごとに有効な injection mode の machine-readable intersection。    |
...
を組み合わせます。manifest は access-mode field を持ちません。`connect.output` / `listen.path` と
`inject` を選び、kind の定義 metadata、output slot declaration、policy
から access を解決します。
...
type を plain `env` に落とさないという型の安全性は変えません。backend が追加の
...
- `routes[].to` は local `connect` binding key。output type、`listen.path`、URL ではない。
...
gateway `public` output slot は `http-endpoint` output type を使います。
...
## Platform service output type

platform service は same-manifest component output と同じ output type
を使います。`identity.oidc@v1` と `billing.port@v1` はこの catalog の official
output type です。operator / product distribution spec が、それらを Space で
offer する concrete platform service path を定義します。
...
Catalog の kind の定義 metadata は package-owned `spec/kind.jsonld` と、公開時の
...
`outputSlots.<name>.contract` は component output slot が使う output type
...
が受け入れる injection mode を列挙します。manifest では same-manifest output
を `connect` で、platform / external service path を `listen.path` でその slot
へ接続します。runtime injection detail、environment
...
document、この specification page、`@takos/takosumi-contract/type-catalog` の
