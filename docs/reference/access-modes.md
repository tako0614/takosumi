...
access mode は 公式型カタログの語彙です。component output や platform service
entry を consuming component
へ渡すとき、どの強さの権限で出力データを注入するかを表します。

manifest author は `connect.output` または `listen.path` で source
を選び、`inject` で注入モードを選びます。resolved access mode は operator
policy、output slot または platform service entry の
