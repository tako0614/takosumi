...
> apply / destroy を直列化する apply fence として `takosumi_deploy_locks`
> を使う。 この table は `(tenant_id, name)` を primary key とし、 `owner_token`
> と `locked_until` で lease を管理する compact shape。 release は `owner_token`
