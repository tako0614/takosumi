...
> このページでわかること: cross-process lock の v1 仕様 — acquisition / heartbeat / TTL / recovery / deadlock 予防 / SQL backed store 必須化。
...
| `public-deploy:<tenantId>:<name>`     | `POST /v1/deployments` apply / destroy apply fence                |
...
> apply / destroy を直列化する apply fence として `takosumi_deploy_locks` を使う。
> この table は `(tenant_id, name)` を primary key とし、 `owner_token` と
> `locked_until` で lease を管理する compact shape。 release は `owner_token`
> 一致時だけ row を削除するため、 lease takeover 後の新 holder を stale holder
> が消すことはできない。 side-effecting stage 自体は
...
`TakosumiDeploymentRecordStore.acquireLock` の public deploy lock は同じ
`tenantId` / deployment `name` への concurrent apply を直列化する。 lease が
取れるまで poll / wait する。 これは `public-deploy:<tenantId>:<name>` scope
限定の挙動で、 full lifecycle operation lock は上記の fail-closed busy semantics
に従う。
...
lease。 下流 provider へ fencing token は渡さない。 長時間の public deploy
...
- in-memory store は per-process Promise chain しか持たない。 複数 pod で race
  を起こす。 dev / unit test 専用
- in-memory store を production で inject すると kernel boot 時に warning が出る。
  warning を見逃さない運用が前提
