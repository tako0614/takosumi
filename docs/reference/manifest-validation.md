...
Top-level `template` is retired authoring shorthand and is rejected by current
public kernel deploy validation. Template expansion, if used by a tool, must run
before the kernel request and submit only expanded `resources[]`.
...
### Removed service import fields

`services[]` / `imports[]` / `serviceResolvers[]` /
`metadata.takosumiServiceImports` は current v1 envelope に含まれない。これらは
schema phase で unknown field として reject される。外部 / operator-owned
capability は Space-scoped namespace export と explicit grant / account API で
表現し、kernel は anchor fetch や signed `ServiceDescriptor` verification を
行わない。
