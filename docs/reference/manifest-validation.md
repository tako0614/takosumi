...
の 10 値で、未知 top-level field を含む manifest は schema phase で reject
される。 `@context` は optional な JSON-LD context で、推奨値は
...
`@context / apiVersion / kind / namespace / metadata / template / services /
imports / serviceResolvers / resources`
以外の field が現れた時点で `invalid_argument` で reject される (warning に
降格しない)。
...
```text
template | inputs | ref (deprecated compatibility only)
```

Compatibility note: the current public deploy resolver still accepts
`template.ref` as a deprecated alias for `template.template` when the value
matches `<id>@<version>`. New manifests and docs must use `template.template`;
`template.ref` exists only to keep early public-route clients working through
the v1 deprecation window.

CLI local mode may additionally accept `template.name` as an authoring
convenience before publish, but remote kernel manifests do not accept it.
...
### Cross-instance service fields

`namespace` / `services[]` / `imports[]` / `serviceResolvers[]` は
cross-instance service binding 用の optional top-level field。

- provider manifest は `namespace` と `services[]` を使い、service identifier
  (`takosumi.account.auth@v1` など) で export する service set を宣言する
- consumer manifest は `imports[]` と `serviceResolvers[]` を使い、endpoint URL
  を直書きせず anchor から provider-signed ServiceDescriptor を resolve する
- `imports[]` が 1 件以上ある manifest は `serviceResolvers[]` が必須
- `services[]` を使う provider manifest は `namespace` が必須

これらは現行 v1 envelope の closed vocabulary に含まれる。legacy の
`services: { app: ... }` map とは別物で、現行 `services[]` は provider 側の
cross-instance service export 配列だけを意味する。

Legacy authoring shape の top-level / component-level field は v1 envelope では
reject される:
...
の実値は plan / apply phase で初めて埋まる。Reference resolution phase
は「埋まる予定があること」だけを検証する。

## Phase 4 — Catalog binding

Manifest が参照する Shape / Provider / Template は CatalogRelease に bind
されていなければならない。
