...
> 本ドキュメントの説明する curated 4 kind catalog (= `worker` / `postgres` /
> `object-store` / `custom-domain`) は Wave N で **完全廃止予定** (= kernel pure
> contract executor 化、 specific kind は **operator distribution が JSON-LD +
> plugin で持ち込む** model に移行)。 `Component.build` も削除され、 build は別
> `kind: build` component に移管します。 詳細 design は
> [RFC 0001](../rfc/0001-kernel-kind-agnostic.md) を参照。 **現状 (Wave L
> まで)** の code 動作は本ドキュメント通り curated catalog ベース (= URI は
> `https://takosumi.com/kinds/v1/<name>`)。 **Wave N planned** では
> takosumi-cloud が **1 つの reference operator distribution** として 6 kind (=
> worker / postgres / object-store / custom-domain / build / oidc) を
> `https://cloud.takosumi.com/kinds/v1/` 系で publish 予定 (= 「公式 / blessed」
> ではなく alternative operator distribution が 同 contract で 置き換え可能)。
...
**現状 (Wave L まで)**: 本 section は curated 4-kind catalog (= `worker` /
`postgres` / `object-store` / `custom-domain`) の spec / outputs / publishes /
listens を 説明する。

::: info Wave N planned 本 section が説明する curated 4-kind catalog 自体が Wave
N で **物理削除予定** (= takosumi kernel core は kind 名を 一切知らない pure
contract executor に 純化)。 全 kind は operator distribution が JSON-LD +
plugin で 持ち込む model に進化する。 takosumi-cloud reference distribution は
`https://cloud.takosumi.com/kinds/v1/<name>` で 6 kind (= worker / postgres /
object-store / custom-domain / build / oidc) を publish 予定 (= 「公式 /
blessed」 ではなく **alternative operator distribution も 同 contract で
置き換え可能**、 RFC 0001 §4.6)。 詳細 design は
...
**URI domain の 2-layer narrative**:

- **Wave L 現状** (= 本 table 中央列): `https://takosumi.com/kinds/v1/<name>` が
  canonical 正本 URI。 takosumi kernel core が curated 4 kind を 認識する。
- **Wave N planned** (= 本 table 右列): takosumi-cloud reference operator
  distribution が `https://cloud.takosumi.com/kinds/v1/<name>` で同 kind を
  publish (= 「公式 / blessed」 ではなく **alternative operator distribution
  も同 contract で 置き換え可能**)。 旧 `https://takosumi.com/kinds/v1/<name>`
  URI は Wave N 完了後 **deprecated** (= alternative operator が新 URI で
  publish した時点で takosumi.com URI は redirect / 廃止される、 RFC 0001
  §4.6)。

| `kind` (alias)  | URI (= 現状 Wave L まで、 canonical 正本)     | Wave N planned (= takosumi-cloud reference、 旧 URI deprecated) | 用途                                           |
| --------------- | --------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------- |
| `worker`        | `https://takosumi.com/kinds/v1/worker`        | `https://cloud.takosumi.com/kinds/v1/worker`                    | Serverless HTTP service (= JS bundle artifact) |
| `postgres`      | `https://takosumi.com/kinds/v1/postgres`      | `https://cloud.takosumi.com/kinds/v1/postgres`                  | Managed PostgreSQL                             |
| `object-store`  | `https://takosumi.com/kinds/v1/object-store`  | `https://cloud.takosumi.com/kinds/v1/object-store`              | S3-compatible bucket                           |
| `custom-domain` | `https://takosumi.com/kinds/v1/custom-domain` | `https://cloud.takosumi.com/kinds/v1/custom-domain`             | Public domain + TLS termination                |

> Footnote: 現状 (Wave L まで) の URI は takosumi.com domain で publish された
> curated catalog (= takosumi kernel が hardcoded 認識)。 Wave N planned で
> curated catalog 自体が物理削除され、 takosumi-cloud が
> `cloud.takosumi.com/kinds/v1/` 系で **1 つの reference operator distribution
> として** publish (= 「公式」 ではなく alternative も同 contract で 同列、 RFC
> 0001 §4.6 + §7.4)。 旧 takosumi.com URI は Wave N 完了後 deprecated。 RFC 0001
> 完了後は本 table を 「operator が自分の catalog を 作るときの reference
> convention」 として reframe 予定。
...
```
https://takosumi.com/contexts/v1.jsonld          ← root vocabulary (wire URL)
https://takosumi.com/kinds/v1/<name>             ← @id (= canonical identifier、 拡張子なし)
```

operator が自前 kind を立てるときも同じ形を採用してください:

```
https://operator.example.com/contexts/v1.jsonld          ← operator が root を引く場合 (wire URL)
https://operator.example.com/kinds/lambda                ← @id (= canonical identifier)
```

> **Footnote (= canonical URI vs wire URL の区別)**: RFC 0001 §4.3 で canonical
> URI form は `https://<operator-domain>/kinds/<version>/<name>` (= 拡張子
> なし、 例 `https://takosumi.com/kinds/v1/worker`)。 一方 actual な HTTP serve
> 上の **wire URL** は `https://takosumi.com/contexts/kinds/v1/<name>.jsonld` (=
> `.jsonld` 拡張子付き、 `contexts/` overlay 配下) で配信される。 つまり 1 個の
> kind は 2 形 (canonical URI = identifier、 wire URL = serve location) を
> 持つが、 kernel / parser は **canonical URI を identifier として完全一致
> 比較** するため、 wire URL の path prefix (`contexts/`) や 拡張子 (`.jsonld`)
> は kind URI matching には参加しない。 operator が自前 catalog を 立てる場合
> も、 canonical URI と wire URL は別形を 採用してよい (= `@id` を 拡張子なしで
> declare し、 実 serve は `.jsonld` 拡張子付き URL から resolve する慣習)。
> 詳細 design は
> [RFC 0001 §4.3](../rfc/0001-kernel-kind-agnostic.md#43-kind-uri-scheme)
> を参照。
