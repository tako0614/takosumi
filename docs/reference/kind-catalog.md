...
> します。
>
> **2-layer note (= Wave N planned)**: 本 section は **現状 (Wave L まで)** の
> curated 4-kind catalog (= `worker` / `postgres` / `object-store` /
> `custom-domain` を `takosumi.com/kinds/v1/` で publish) を説明する。 **Wave N
> planned** では curated catalog 自体が物理削除され、 全 kind は operator
> distribution が JSON-LD + plugin で 持ち込む (= takosumi-cloud reference は
> `cloud.takosumi.com/kinds/v1/` 系で publish、 alternative operator
> distribution も同 contract で 置き換え可能、 RFC 0001 §4.6)。
...
| `kind` (alias)  | URI (= 現状 Wave L まで)                      | Wave N planned (= takosumi-cloud reference)         | 用途                                           |
| --------------- | --------------------------------------------- | --------------------------------------------------- | ---------------------------------------------- |
...
> curated catalog。 Wave N planned で物理削除され、 takosumi-cloud が
> `cloud.takosumi.com/kinds/v1/` 系で **1 つの reference operator distribution
> として** publish (= 「公式」 ではなく alternative も同 contract で 同列、 RFC
> 0001 §4.6 + §7.4)。 RFC 0001 完了後は本 table を 「operator が自分の catalog
> を 作るときの reference convention」 として reframe 予定。
...
https://takosumi.com/kinds/v1/<name>             ← @id (= canonical identifier、 wire URL は .jsonld 拡張子付き serve)
```
...
> Footnote (= canonical URI vs wire URL): RFC 0001 §4.3 で canonical URI は
> `https://<operator-domain>/kinds/<version>/<name>` (= 拡張子なし)。 実際の
> wire URL は HTTP serve 時に `.jsonld` 拡張子を付ける慣習が available (= 例:
> takosumi.com で `https://takosumi.com/kinds/v1/worker.jsonld` を 配信)。
> kernel / parser は **canonical URI を identifier として完全一致 比較**
> するため、 wire URL の `.jsonld` 拡張子は kind URI matching には参加しない。
