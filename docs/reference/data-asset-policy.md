...
asset は operator が optional extension として有効化できる content-addressed blob です。diagnostic bundle、large runtime input、 operator-owned generated payload のように、source handoff とは別の blob storage と lifecycle policy で扱いたいデータに使います。Takosumi core Installer API の source handoff は `git` / `prepared` / `local` source descriptor を使い、 prepared source archive が build 後 source tree を運びます。

manifest authoring では runtime intent と runtime file path を kind-specific `spec`、source handoff を prepared source archive で表します。build 後の source tree や worker bundle は asset に分けず、prepared source archive として Installer API に渡します。source-backed connector は resolved source view と kind-specific `spec` を読みます。

## 互換名 {#compatibility-names}

asset は operator extension の概念名です。reference implementation の optional asset extension は `artifact` という名前で current compatibility wire を公開します。これらの名前は current reference extension wire であり、manifest field でも Installer API source kind でも Takosumi core conformance requirement でもありません:
...
asset-backed lifecycle request は asset descriptor を明示します。 current compatibility wire の descriptor は `kind` と `hash` または `uri` を持ちます。この `kind` は asset metadata value であり、manifest の component `kind` ではありません。dispatcher はその metadata value を connector の `acceptedArtifactKinds` と照合します。`acceptedArtifactKinds` が空の source-backed connector は asset descriptor を受け取らず、resolved source view と kind-specific `spec` を読みます。
...
asset size policy は上記の optional operator extension route と connector に適用されます。
...
asset metadata value は operator-owned open metadata です。各 connector は受け付けるものを current compatibility field `acceptedArtifactKinds` で宣言します。例:

- asset-backed custom connector: 明示的に登録された operator-owned metadata value
- source-backed worker / web-service connector: `acceptedArtifactKinds: []` と resolved source view + kind-specific `spec`

runtime-agent は connector code が動く前に mismatch を reject します。reference component kind level の validation はより厳しいことがあります。Takosumi official type catalog の `worker` descriptor を採用した operator profile は `spec.entrypoint` を要求し、asset descriptor は要求しません。

reference connector package が現在 operator discovery 用に登録している bundled asset metadata の例:
...
これらの値は asset metadata value であり、manifest component kind でも Takosumi Kind Catalog descriptor identity でもありません。

reference extension の compatibility wire は `ArtifactReference = string | Artifact` を受け付けます。string 形式は operator-local な省略形です。asset-backed connector へ dispatch する前に operator resolver が `{ kind, hash | uri, metadata? }` descriptor へ正規化します。正規化できない asset reference は `invalid_argument` です。
