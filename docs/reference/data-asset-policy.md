...
runtime-agent は connector code が動く前に mismatch を reject します。reference component kind level の validation はより厳しいことがあります。Takosumi 公式カタログの `worker` descriptor を採用した operator distribution は `spec.entrypoint` を要求し、asset descriptor は要求しません。
...
これらの値は asset metadata value であり、manifest component kind でも Takosumi 公式カタログ descriptor identity でもありません。
