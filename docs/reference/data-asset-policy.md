...
runtime-agent は connector code が動く前に mismatch を reject します。reference component kind level の validation はより厳しいことがあります。Takosumi 公式型カタログの `worker` descriptor を採用した operator distribution は `spec.entrypoint` を要求し、asset descriptor は要求しません。
...
これらの値は asset metadata value であり、manifest component kind でも Takosumi 公式型カタログ descriptor identity でもありません。

reference extension の reader-facing wire は明示的な `{ kind, hash | uri, metadata? }` descriptor です。asset-backed connector へ dispatch する前に operator resolver がこの descriptor を検証します。正規化できない asset reference は `invalid_argument` です。operator-local な省略形を受け付ける場合でも、それはその operator profile の入力規約であり、portable asset reference shape ではありません。
