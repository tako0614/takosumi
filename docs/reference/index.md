...
Takosumi docs は、Takosumi 本体仕様と Takosumi 公式カタログ仕様を同じ docs site に置きます。ただし章は分けます。operator distribution の具体的な account layer 仕様は、その distribution の docs を正本にします。

Component kind の schema と出力データの vocabulary は Takosumi 公式カタログが扱います。Cloud account layer の具体仕様は `takosumi-cloud/docs/` 側に置きます。

## Takosumi 本体仕様

- [仕様境界](./spec-boundaries.md) — Takosumi core、公式カタログ、operator distribution の責務分離。
...
- [プラットフォームサービス](./platform-services.md) — manifest 外の publication が提供する出力データを `listen.path` / `listen.kind` から受け取る model。
- [Installer API](./installer-api.md) — Installation / Deployment を作成・更新・ rollback する public Installer API。
- [HTTP 公開](./http-exposure.md) — public app endpoint を workload component output、gateway output、root publish による Installation output publication で表現する model。
- [用語集](./glossary.md) — manifest / Installation / Deployment と隣接する用語の短い定義。

## Takosumi 公式カタログ仕様

- [Takosumi 公式カタログ](./catalog.md) — Takosumi が公開する kind の定義の語彙、material kind、注入モード、JSON-LD catalog metadata。
...
公式カタログを reference kernel に接続するための実装資料です。AppSpec core 仕様として読む章ではありません。
