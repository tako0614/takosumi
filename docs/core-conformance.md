...
| first-party OpenTofu Capsule module catalog                                         | conformant     | Active bundled-HCL catalog in `firstPartyModuleFilesByTemplateId` matches `cloudflare-hello-worker`, `cloudflare-static-site`, `core`, and `takosumi-ai-endpoint`; each is a plain child module called from the generated root. Storage examples use existing providers or S3-compatible endpoints through the Stack flow instead of first-party starter modules. |
...
| Resource Shape API                                                                  | partial        | Resource object routes and current default v1alpha1 shapes exist for EdgeWorker and AIEndpoint. Object storage is handled by existing providers or scoped S3-compatible compatibility profiles, not a default Resource Shape. Events/refresh/import and full UI remain incomplete.       |
| Resource Shape contract types                                                       | partial        | `takosumi.dev/v1alpha1` exposes default public shapes for EdgeWorker and AIEndpoint. Storage compatibility is reported through `compat.s3.v1` capability when an operator intentionally enables it.                                                                                         |
