...
The CLI packs the local directory into a `tar`(zstd) archive, sends it to `POST /api/spaces/:id/uploads` (stored in
R2_SOURCE and recorded as an **upload-origin SourceSnapshot**), then asks `POST /api/deploy` to pin that snapshot,
