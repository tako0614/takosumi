...
The Kind Catalog is Takosumi-published vocabulary adjacent to the core
spec. The core manifest structure stays small; kind definitions, output type
contracts, injection modes, and JSON-LD catalog documents live in catalog
docs. Operator configurations such as Takosumi Cloud adopt the core and catalog
...
Core parses kind names, output type names, injection mode names, and
platform service paths as strings, then records resolution details.
Catalogs define kind, output type, and projection vocabulary. Operator or product
distribution specs define the inventory of platform service paths.

Core compatibility is based on manifest structure, Installer API behavior, source and
digest guards, Deployment records, and publish/listen resolution rules. If a
manifest uses Takosumi official catalog terms, catalog compatibility also
applies. If it uses operator-provided platform service paths or account management
APIs, compatibility with that operator configuration also applies.

## Kind Catalog Specification {#official-type-catalog-specification}

The Kind Catalog answers: which vocabulary does Takosumi publish for
...
- kind definition metadata for `spec`, output vocabulary, and expected output
  format
- output types such as `http-endpoint`, `service-binding`,
  `identity.oidc@v1`, and `billing.port@v1`
...
A catalog entry describes reusable vocabulary and JSON-LD metadata.
Publisher roots, account management URLs, billing owners, identity issuer policy, and
dashboard behavior belong to the operator or product distribution that provides
those services or APIs.

## Operator Configuration Specifications {#operator-distribution-specifications}

An operator configuration answers: around the Takosumi core installer, which
concrete account management, provider, runtime, and admin behavior does this operator
provide?

Takosumi Cloud is one operator configuration. Its normative docs live in
...
An operator configuration can adopt output types from the official type
catalog. The catalog defines reusable output formats. The operator docs define
concrete platform service paths, account management lifecycle, approval flows, and runtime
delivery behavior.
...
3. [Kind Catalog](./type-catalog.md) when you need concrete kind or
   output type vocabulary
4. [Platform Services](./external-publications.md) when a manifest consumes
   output from outside the same manifest
5. Operator configuration docs when you use operator-provided account management
