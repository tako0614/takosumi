...
`identity.oidc@v1` and `billing.port@v1` are neutral output formats for platform services. The catalog names the fields a workload can receive. Issuer operation, client lifecycle, redirect policy, billing ownership, usage authorization, payment-provider integration, and concrete platform service paths belong to the operator or product distribution that offers the output.
...
Consumer slot metadata and operator policy can make a syntactically valid combination invalid for a particular component.
