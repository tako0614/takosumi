...
| Mutation redelivery safety | Apply/Destroy provider dispatch is durably fenced at most once; only a stable semantic match in the provably pre-dispatch phase may resume with a freshly verified credential, while ambiguous post-dispatch transport loss is terminally indeterminate unless the exact immutable state/output target can be adopted | Durable Object tests cover crash/restart before and after the durable dispatch transition, re-minted token/JTI equivalence, authority/input mismatch, concurrent claims, secret-free evidence, one provider POST, and safe Plan/pre-dispatch retry; deploy-control tests prove the typed outcome is not retry-classified |
