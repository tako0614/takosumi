...
| Mutation redelivery safety | Apply/Destroy provider dispatch is durably fenced at most once; ambiguous post-dispatch transport loss is terminally indeterminate unless the exact immutable state/output target can be adopted | Durable Object tests cover restart/redelivery, request-digest drift, secret-free evidence, one provider POST, and safe Plan/pre-dispatch retry; deploy-control tests prove the typed outcome is not retry-classified |
