...
| Public endpoint and Capsule identity | Hostname/DNS/endpoint behavior is ordinary Git-owned OpenTofu/provider work; OSS neither reserves managed hostnames nor auto-registers Capsule OIDC clients | Contract/OpenAPI/UI scans and Capsule tests cover the absence of managed-host input/synthesis; generic Accounts OIDC live-grant and Interface authorization regressions remain required |
