...
The source proof now has a create-only report generator and a distinct
protected-main signing candidate lane. It runs the real in-process Takosumi
HTTP Resource API twice against a stateful deterministic conformance Adapter,
requires byte-identical output, and binds every report to the exact Takoform
`canonical` / `reject-invalid-semantics` fixture name and raw file digest. Its
fixed subject is `host:https://in-process.takosumi.test`. The resulting
`takoform.standard-runner-report@v1` files remain OSS reference-host/source
conformance only: they do not claim Takosumi Cloud, a real provider backend,
managed capacity, or a production deployment. The workflow creates a closed
Actions artifact and performs no release, tag, activation, or deployment
mutation.
