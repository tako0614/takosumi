...
| Typed provider                 | Portable statically typed form provider; separate Takosumi legacy/admin release custody                            | Independently versioned third-party provider and verified unchanged network mirror                                                | No dynamic schema, TargetPool/admin object in form provider, private manager/price selection, old-version overwrite, package-version-coupled binary, or address/type/API rename in one step                                                            |
