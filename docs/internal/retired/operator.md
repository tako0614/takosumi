...
| `TAKOSUMI_RESOURCE_OBSERVATION_BATCH`            | `8`    | `1`–`32`       | 1 tick で claim する最大 Resource 数         |
| `TAKOSUMI_RESOURCE_OBSERVATION_CONCURRENCY`      | `4`    | `1`–`8`        | 同時に実行する最大観測数                 |
| `TAKOSUMI_RESOURCE_OBSERVATION_INTERVAL_SECONDS` | `3600` | `300`–`604800` | 同一 Resource の最小観測間隔               |
| `TAKOSUMI_RESOURCE_OBSERVATION_LEASE_SECONDS`    | `900`  | `600`–`7200`   | abandoned claim を再取得できるまでの時間  |

範囲外または不正な値は安全な既定値に戻ります。batch を無制限に増やす用途ではなく、runner pool の容量に合わせて
小さく保ってください。観測結果は `takosumi_resource_observation_count` metric の `outcome` label で確認できます。

## 運用証跡

Takosumi には ecosystem-wide / OSS-wide の GA engineering gate はありません。
operator は実際に運用する capability ごとに、次の証跡を維持します。

| 領域               | 必要な証跡                                                                                                         |
| ------------------ | ------------------------------------------------------------------------------------------------------------------ |
| Website/docs       | ドキュメントビルド、公開する場合はカスタムドメイン/TLS                                                             |
| Runner             | 非本番環境での OpenTofu plan/apply/destroy の実証                                                                  |
| Release activation | webhook/materializer の実証、terminal success gate、失敗時の state/output 保持と Run/Capsule/Interface 非 Ready 証跡 |
...
この表は運用証跡の checklist であり、release authority や単一の maturity 判定では
ありません。公式 managed target、hosted compatibility profile、billing、abuse 対策、
support、usage metering、deprovision の証跡は、影響する release surface の Cloud
operator policy が所有します。
