...
Takosumi has no ecosystem-wide or OSS-wide GA engineering gate. Each operator
selects the capabilities it runs and keeps current evidence for those
capabilities:
...
This table is an operating-evidence checklist, not release authority and not a
single maturity verdict. Official managed targets, hosted compatibility
profiles, billing, abuse controls, support, usage metering, and deprovision
evidence belong to the Cloud operator policy for the affected release surface.
