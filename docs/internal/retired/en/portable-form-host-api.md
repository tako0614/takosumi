...
The current `terraform-provider-takoform` candidate follows same-origin
discovery to `endpoints.api`, loads exact Form availability, and uses the
versioned contract. Historical provider/state clients can explicitly select
the advertised `compatibility_api`; the host never silently downgrades a
versioned client to `/v1`. Publication of the candidate and the standard Form
packages is still a separate release/admission gate.
