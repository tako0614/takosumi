...
`kind: worker` は takosumi.com reference kind alias の例です。operator
distribution は alias map で URI に解決し、worker implementation binding を
選びます。AppSpec component field は `kind`、`spec`、`publish`、`listen`
です。source を build する operator は
[Build service handoff](/reference/build-spec) の convention で prepared source
を作れます。`entrypoint` は prepared source snapshot 内で worker が読む path
です。
