...
[Quickstart](/getting-started/quickstart) で `takosumi deploy` を 1
回通したあとに読むのがおすすめ。
...
が欲しい」「object-store が欲しい」という抽象だけで、どの cloud で動くかは Shape
には書かない。
...
新しい Shape の追加は breaking change で、RFC が必要 (cf.
...
Provider は Shape の具体実装。 同じ `web-service@v1` でも、 `@takos/aws-fargate`
/ `@takos/cloudflare-container` / `@takos/kubernetes-deployment` /
`@takos/selfhost-systemd` がそれぞれ違う cloud で同じ shape を提供する。
...
Provider は manifest 側 `provider:` field で陽に指定する。 同 Shape の複数
provider が register されていても、 selection は `provider:` と `requires:` を
superset で満たすかで決まる。
...
kernel `POST /v1/deployments` は展開済みの `resources[]` だけを受け取る。
template expansion は installer/compiler layer で kernel request 前に実行する。
...
`${ref:db.connectionString}` のような参照は kernel 側で DAG edge
に変換され、topological order で apply される。DAG は [Manifest](/manifest#dag)
参照。
