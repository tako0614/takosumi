...
Takosumi は `.takosumi.yml` という manifest (宣言ファイル) を読んで、アプリを丸ごとデプロイする PaaS です。 Docker Compose に似ていますが、大きな違いが 1 つあります。manifest には「何が必要か」だけを書き、「どこで動かすか」は書きません。実行先は operator (運用者) が決めるので、同じ manifest が Cloudflare でも AWS でも動きます。
