...
アプリが使う DB / OIDC / bucket / queue などは operator catalog の PlatformService として扱います。OpenTofu や
cloud provider API で何を作るかは operator distribution の責務です。Takosumi は inventory を読み、どの
...
OpenTofu は resource graph と state management に強い IaC です。Takosumi は OpenTofu を置き換えません。

推奨境界:

- OpenTofu/Helm/Pulumi: operator-owned infra materialization と state。
