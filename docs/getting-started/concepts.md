...
新しい Shape の追加は breaking change として扱われ、RFC が必要です (cf.
...
`@takos/aws-fargate` / `@takos/cloudflare-container` /
`@takos/kubernetes-deployment` / `@takos/selfhost-systemd` がそれぞれ違う cloud
で同じ shape を提供します。
...
複数 provider が register されていても、selection は manifest 側 `provider:` と
`requires:` を superset で満たすかで決まります。
...
- **runtime-agent** は cloud credential を持ち、`@takos/aws-fargate` の場合は
  ECS API、`@takos/selfhost-systemd` の場合は `systemctl enable --now`
  を実行する。
