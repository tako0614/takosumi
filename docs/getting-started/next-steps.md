...
`host` を省略すると operator が自動でホスト名を割り当てます。ローカル開発環境では `host: hello.takosumi.test` のように明示することもできます。
...
前の Deployment に戻す場合は retained Deployment id を指定します。rollback は新しい Deployment を作らず、current pointer を過去の `succeeded` Deployment に戻します。
