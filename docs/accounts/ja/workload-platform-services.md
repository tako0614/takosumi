...
state であり、Takosumi の source authoring vocabulary ではありません。
...
`POST /internal/workload-platform-services/resolve` route でこれらの path を解決します。Takosumi Installer API は選ばれた binding snapshot を受け取り、Deployment に記録します。
...
です。Takosumi は resolved binding snapshot だけを記録します。
