...
::: warning 必須項目
production deploy で以下のいずれかが欠けると、kernel の boot 時
fail-closed か silent な persistence loss / plaintext secret に直結します。
:::
...
kernel は production / staging で adapter port が 1 つでも未 wire だと
boot 時に拒否します (`STRICT_RUNTIME_KERNEL_PORTS` enforcement、source:
...
::: warning 見落とすと
operator が `TAKOSUMI_DATABASE_URL` を export し忘れたまま `takosumi server`
を local 起動すると、CLI からの deploy は成功するように見えますが、kernel
を再起動した瞬間に **deployment record が空になります**。production に
押し上げる前にこの warning が出ていないことを必ず確認してください。
:::
...
deployment 状態を再構成できる** よう作られています (since
...
runtime-agent process を restart しても、describe() は host から live
状態を query して正しい outputs を返します。
...
`@takos/selfhost-systemd` が render する unit ファイルには
`# X-Takos-HostPort=<n>` / `# X-Takos-InternalPort=<n>` の marker が
入ります。 operator が手で書いた unit ファイルにこの marker が無いと、
describe() は status だけを返して outputs (URL / port) を空で返します。
:::

---

## Cross-process apply lock の gap

`POST /v1/deployments` は **per-(tenant, name) lock** で同 deployment
への並行 apply を直列化しています。

| scope            | 保証                                                                        |
| ---------------- | --------------------------------------------------------------------------- |
| **In-process**   | per-(tenant, name) Promise chain で同 process 内の race を防ぐ ✅           |
| **Cross-process**| **保証なし** ❌                                                            |

source: `packages/kernel/src/domains/deploy/takosumi_deployment_record_store_sql.ts`

::: warning multi-pod kernel
Postgres `pg_advisory_lock` は **session-scoped** で、pooled SqlClient
経由では successive `acquireLock` / `releaseLock` 呼び出しが別 connection
にルーティングされます。 結果として lock が原 connection に leak し、
別 pod から同 deployment への 2nd apply が race します。
:::

multi-pod 構成で fence したい場合の選択肢:

1. **single-writer apply tier** — `POST /v1/deployments` を受ける pod を 1 台
   だけにする (active/passive)。 これが一番単純。
2. **`SELECT ... FOR UPDATE` を使う custom SqlClient を inject** —
   `acquireLock` / `releaseLock` が同じ connection を pin するように
   `SqlClient` をオーバライドする。`SqlTakosumiDeploymentRecordStore` を
   subclass して `acquireLock` / `releaseLock` を上書きする方法も同じ
   コメントで案内されています。
...
公開 deploy route (`POST /v1/deployments`) は **shared bearer token** モデルです。

- token は `TAKOSUMI_DEPLOY_TOKEN` の **1 つだけ**。
- `tenant_id` は kernel 側で hardcoded `"takosumi-deploy"` (cf.
  `packages/kernel/src/api/deploy_public_routes.ts`)。
- 1 つの token を共有する operator 同士は同じ namespace を共有します。

::: warning 複数 operator / org 環境
独立した namespace が必要な場合は **kernel instance を分離する** か、
`TAKOSUMI_DEPLOY_TOKEN` を strict に運用 (1 org = 1 kernel) してください。
public route は当面 single-tenant です。
:::

deployment record の tenant 列は将来の multi-tenant 拡張のため SQL schema
には存在しますが、route 側ではまだ static 値が入ります。
...
partition の override が無ければ `global` passphrase に partition label を
mix した HKDF-style salt から派生されます。 そのため **AWS partition の
secret を漏らしても他 cloud の ciphertext は復号できません**
(`MultiCloudSecretBoundaryCrypto`、Phase 18.2 H14)。

`additionalData (AAD)` に partition label が bind されているため、payload
の partition tag を swap すると open() が失敗します。
...
過ぎても自動で seal されない)。 operator は `rotationStatus()` を
監視して新 version を put し直してください。

`runVersionGc()` は keep-latest-N + last-accessed-N-days で旧 version を
GC します (defaults: keepLatest=5, accessedWithinDays=90)。
...
::: warning Prometheus / OTLP は未実装
- `/metrics` (Prometheus exposition) は提供しない
- OpenTelemetry OTLP exporter も未実装

external 観測が必要な場合の選択肢:
- `TAKOSUMI_AUDIT_REPLICATION_KIND=s3` で audit log を S3 に replicate (Object Lock + retention 対応)
- `/readyz` を外部 monitoring から scrape
- application logs を Deno stdout から拾う

これらは future work で、提供 issue が立っていない場合は plugin として外側で
書くのが現実的です。
:::
