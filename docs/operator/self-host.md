...
各 env の意味は [Environment Variables](../reference/env-vars.md) 参照。
...
## 本番: kernel と agent を分離 {#production-kernel-と-agent-を分離}

multi-host setup や credential 隔離が必要な場合、 agent を別 host で立てて
kernel から HTTP で叩く。

### Agent host (cloud credential を保持する host) {#agent-host-cloud-credential-を持つ-host}
...
### Kernel host (credential を保持しない) {#kernel-host-credential-を持たない}
...
## Artifact storage の hygiene {#artifact-storage-hygiene}
...
### 読み取り専用 artifact fetch token (agent ↔ kernel scope separation) {#read-only-artifact-fetch-token-agent--kernel-scope-separation}
...
## In-memory fallback の警告 (dev mode) {#in-memory-fallback-の警告-dev-mode}
...
## Selfhost connector の再起動耐性 {#selfhost-connector-の-restart-survival}
...
## プロセス間 apply ロック {#cross-process-apply-lock}
...
ロック詳細は [Lifecycle Protocol](../reference/lifecycle.md) も参照。

---

## マルチテナンシーの境界 {#multi-tenancy-の境界}
...
## Observability の現状 {#observability-の現状}
...
- [Observability Stack](../reference/observability-stack.md) の SLI / SLO table
  を alert rules と on-call routing の初期 contract として使う。
