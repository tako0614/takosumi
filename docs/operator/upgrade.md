...
operator distribution が持つ backend implementation、connector、inventory importer は operator の release bundle / image / IaC repo で管理します。
...
operator distribution は、有効にしている OpenTofu stack、service image、runtime-agent image、connector wiring、PlatformService inventory importer を同一のテスト済みリリースバンドルに揃えるべきである。有効にする backend implementation を減らせば検証対象も減らせるが、Takosumi npm package とは別管理である。
