...
の対象であり、 Takosumi public conformance surface は Manifest / Installation /
Deployment と [Installer API](../reference/installer-api.md) です。 **各 package はまだ pre-1.0 で、
...
package であり、core Manifest / Installer API conformance surface
ではありません。
...
Takosumi は schema migration を `takosumi server` 起動時に自動実行します。
schema change を伴う upgrade では Takosumi を一度停止し、新 version の Takosumi を
...
Takosumi の SQL state は `storage_migrations` ledger で管理される。 production
...
production では Takosumi と runtime-agent を同じ release bundle から更新する。
