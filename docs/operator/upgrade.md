...
operator distribution は、有効にしている `@takosjp/takosumi-plugins` subpath implementation・kernel・runtime-agent を同一のテスト済みリリースバンドルに揃えるべきである。有効にする backend implementation を減らせば検証対象も減らせるが、npm package は `@takosjp/takosumi-plugins` 1 つである。
