...
reference Takosumi publish layout は **core/runtime/tooling 6 package (`@takos/takosumi` umbrella を含む) + official catalog/helper 1 package + provider / adapter 11 package = 計 18 package** です。provider / adapter packages、 runtime-agent、official catalog/helper package は release-bundle alignment の対象であり、 Takosumi public conformance surface は Manifest / Installation / Deployment と [Installer API](../reference/installer-api.md) です。 **各 package はまだ pre-1.0 で、 minor bump も breaking change を含む可能性があります。** upgrade 前に必ず CHANGELOG を確認してください。
...
### Provider / adapter (別 install)

operator は必要な外部 system だけを別 install する。provider / adapter packages はすべて pre-1.0 で運用される。
...
local / external adapter behavior は [Operator-managed 運用](./operator-managed.md) を production traffic 移行前に検証すること。
...
- [Operator-managed 運用](./operator-managed.md) — production 配信前の checklist
