...
operator は Takosumi kernel と runtime-agent を起動し、provider implementation、
credential、storage、network boundary を選びます。account / billing / signup
flow は operator account-plane の資料で扱います。OIDC issuer など外部 surface
は、 必要な場合だけ namespace export として接続します。

## 読む順序

1. [Bootstrap](./bootstrap.md) — reference kernel に provider implementation
   adapter を attach する。
2. [セルフホスト運用](./self-host.md) — production 起動前の
   env、secret、optional DataAsset retention を確認する。
