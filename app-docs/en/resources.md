...
the Takosumi hosted service. The selected provider creates them. Takosumi supplies the Run
...
- the Takosumi hosted service Takoform Host after it is released.

The Takosumi hosted service does not infer accounts, regions, secrets, or prices from a
...
The authenticated Takosumi catalog, provided by the hosted-service owner as an
...
the hosted-service owner's contract.

These facts are checked independently:

- the provider or protocol is released;
- the hosted-service backend and capacity are configured;
...
Takoform owns the portable contract and OpenTofu/Terraform provider. The
Takosumi hosted service intends to provide the official Host, but an unpublished candidate is
