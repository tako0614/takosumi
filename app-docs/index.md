...
ownership と certificate が current の間だけ route を有効にします。review 済みの live launch evidence が揃うまでは
...
recovery、tenant isolation、Dashboard、review 済み live launch evidence が含まれます。self-test、descriptor、
...
[Takosumi Cloud pricing](./pricing.md) にまとめています。PriceCatalog の公開、
決済事業者との同期、margin guard、突合処理は公開 contract ではなく運用の範囲です。
