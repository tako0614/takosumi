...
課金アカウントを検証できる必要があります。Workers route は、同じ Workspace に属する
profile-owned Ready `EdgeWorker` と呼び出し Principal を検証してから canonical Interface /
...
immutable quote を作り、reviewed apply で reserve してから backend を呼びます。compatibility
control request も先に `EdgeWorker` / `ObjectBucket` request へ変換され、同じ lifecycle を
...
Takosumi Cloud の Stable Cloudflare view は `cloudflare_workers_script`、system-host route、
...
`compat.cloudflare.workers.v1` の Cloudflare v4 形式サブセットです。既存の
`cloudflare/cloudflare` OpenTofu/Terraform provider は `base_url` を変えて利用できますが、
これは protocol adapter であり、Cloudflare account の複製でも Resource lifecycle authority
でもありません。

Stable control-plane subset は次に限定します。
...
クライアントはこの値を生成せず、取得してから任意の route を別操作で作成します。
...
route pattern は scheme を含めず、発見した system hostname と明示 path を使います。
...
成功時の `id` は canonical Interface id、`etag` は更新・削除 CAS 用の strong ETag です。
...
1 Worker 1 route、明示 path、末尾 wildcard 0/1 個だけが Stable です。host-only、複数・
...
Stable offering として少なくとも次を表示します。

- Edge Worker
- Object Storage

Preview の service form は active offering がある場合だけ同じ一覧に状態を表示します。
compatibility handler の仮想 inventory や独自 Resource 台帳は使いません。Dashboard、
`takosumi/takosumi` provider、direct Deploy API、対応する compatibility request はすべて
同じ Resource に収束します。`resource_shapes` capability は型付き Resource Shape API が
