...
`takosumi.com`. The site is intentionally low-tech: a single `index.html` with
system-font CSS and three sections (what it is, where to go, try it). Reference
docs live at `docs.takosumi.com` and the operator Accounts service at
`cloud.takosumi.com`.
...
The default Pages host `takosumi-site.pages.dev` stays available for preview
deploys.

## Iterating

This is a static drop-in. To switch to a real SSG (Astro, 11ty, Next static, …),
