...
// ---------------------------------------------------------------------------
// Generic Env Provider delivered TF_VAR variable declarations.
//
// A generic_env_provider Connection delivers each name as TF_VAR_<NAME>, but
...
test("generateGenericCapsuleRoot declares a sensitive variable for every generic env helper name", () => {
  const { files } = generateGenericCapsuleRoot({
    requiredProviders: ["registry.opentofu.org/vercel/vercel"],
    inputs: {
      project_name: "talk",
    ...
    genericEnvVarNames: ["VERCEL_API_TOKEN", "VERCEL_TEAM_ID"],
  });
  ...
  // Generic env helper credentials are static secrets, so they are NOT marked ephemeral
  // (unlike the per-alias provider-credential split).
  expect(main).not.toContain("  ephemeral = true");
  // No provider block is materialized for the generic env helper — the value is a
...
test("generateGenericCapsuleRoot dedupes repeated generic env helper names and rejects non-identifiers", () => {
  const { files } = generateGenericCapsuleRoot({
    requiredProviders: ["registry.opentofu.org/vercel/vercel"],
    inputs: { project_name: "talk" },
    outputAllowlist: {},
    genericEnvVarNames: ["VERCEL_API_TOKEN", "VERCEL_API_TOKEN"],
  });
  ...
  expect(() =>
    generateGenericCapsuleRoot({
      ...
      inputs: {},
      outputAllowlist: {},
      genericEnvVarNames: ["not a valid identifier"],
    }),
  ).toThrow();
});

test("generateInstallationRoot declares generic env helper variables alongside provider aliases", () => {
  const { files } = generateInstallationRoot({
    template: R2_TEMPLATE,
    inputs: { bucketName: "b", accountId: "a", location: "" },
    installType: "opentofu_module",
    providerEnvBindings: [{ provider: "cloudflare", alias: "main" }],
    genericEnvVarNames: ["EXTRA_PROVIDER_TOKEN"],
  });
  const main = files["main.tf"]!;
  // The per-alias provider credential split is unchanged...
  expect(main).toContain('variable "cloudflare_main_api_token" {');
  expect(main).toContain("  ephemeral = true");
  // ...and the generic env helper name gets its own sensitive (non-ephemeral)
  // declaration.
  expect(main).toContain(
    [
  ...
  // Generic env helper names produce no provider block of their own.
  expect(main).not.toContain('provider "EXTRA_PROVIDER_TOKEN"');
});

test("generateInstallationRoot with no generic env helper names is byte-stable vs the existing root", () => {
  const inputs = { bucketName: "b", accountId: "a", location: "" };
