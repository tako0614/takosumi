...
import { SqlRevokeDebtStore } from "../../../../core/domains/deploy-records/revoke_debt_store_sql.ts";
...
class FakeSqlClient implements SqlClient {
  ...
  #getById(params: readonly unknown[]): SqlQueryResult<FakeRow> {
    ...
    const row = this.rows.find(
      (entry) => entry.owner_space_id === ownerSpaceId && entry.id === id,
    );
    return row
      ? { rows: [{ ...row }], rowCount: 1 }
      : { rows: [], rowCount: 0 };
  ...
  #update(params: readonly unknown[]): SqlQueryResult<FakeRow> {
    ...
    const row = this.rows.find(
      (entry) =>
        entry.owner_space_id === ownerSpaceId &&
        entry.id === id &&
  ...
  #listOwnerStatus(params: readonly unknown[]): SqlQueryResult<FakeRow> {
    ...
    const rows = this.rows.filter(
      (row) => row.owner_space_id === ownerSpaceId && row.status === status,
    );
    return { rows: rows.map((row) => ({ ...row })), rowCount: rows.length };
  }
  ...
  #listOpenOwnerSpaces(): SqlQueryResult<{ owner_space_id: string }> {
    const rows = Array.from(
      ...
      .sort()
      .map((ownerSpaceId) => ({ owner_space_id: ownerSpaceId }));
