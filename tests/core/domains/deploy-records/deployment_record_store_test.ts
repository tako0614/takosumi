...
test("upsert keys tenant and deployment name as a tuple", async () => {
  ...
  assert.equal(
    (await store.get("tenant a", "app"))?.sourceEvidence.name,
    "first",
  );
  assert.equal(
    (await store.get("tenant", "a app"))?.sourceEvidence.name,
...
test("recordsFromAppliedResources copies shape from sourceEvidence", () => {
  const records = recordsFromAppliedResources(
    [
      {
        name: "logs",
        providerId: "@operator/filesystem-object-store",
        handle: "h_1",
        outputs: { ok: true },
      },
    ],
    [
      {
        shape: "object-store@v1",
        name: "logs",
        provider: "@operator/filesystem-object-store",
        spec: {},
      },
    ],
    NOW_1,
  ...
  assert.equal(records[0].providerId, "@operator/filesystem-object-store");
...
test("listReferencedArtifactHashes finds artifact.hash in sourceEvidence tree", async () => {
  const store = new InMemoryTakosumiDeploymentRecordStore();
  await store.upsert({
    tenantId: TENANT,
    name: "worker-app",
...
test("listReferencedArtifactHashes unions across many records", async () => {
  const store = new InMemoryTakosumiDeploymentRecordStore();
  await store.upsert({
    tenantId: TENANT,
    name: "a",
  ...
  await store.upsert({
    tenantId: TENANT,
    name: "b",
    sourceEvidence: { resources: [{ spec: { artifact: { hash: HASH_B } } }] },
    appliedResources: [],
    status: "applied",
    now: NOW_1,
  });
...
test("listReferencedArtifactHashes preserves hash on destroyed records", async () => {
  const store = new InMemoryTakosumiDeploymentRecordStore();
  await store.upsert({
    tenantId: TENANT,
    name: "a",
  ...
  assert.equal(
    hashes.size,
    1,
    "destroyed records still pin their sourceEvidence's artifacts " +
      "(audit + race-protection)",
  );
});

test("listReferencedArtifactHashes ignores non-hash strings", async () => {
  const store = new InMemoryTakosumiDeploymentRecordStore();
  await store.upsert({
    tenantId: TENANT,
    name: "a",
...
test("listReferencedArtifactHashes scans applied resource outputs too", async () => {
  const store = new InMemoryTakosumiDeploymentRecordStore();
  await store.upsert({
    tenantId: TENANT,
    name: "a",
