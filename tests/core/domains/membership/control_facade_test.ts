 ...
 *
 * It drives the account-plane `/api/v1/workspaces/:id/members` surface
...
} from "../../../../core/domains/membership/mod.ts";
import { createMembershipControlFacade } from "../../../../core/domains/membership/control_facade.ts";
...
test("real path: owner bootstraps the first member on a brand-new Space", async () => {
  ...
  const list0 = await h.call("GET", `/api/v1/workspaces/${SPACE}/members`, {
    subject: OWNER,
  });
  ...
  const add = await h.call("POST", `/api/v1/workspaces/${SPACE}/members`, {
    subject: OWNER,
  ...
  const list1 = await h.call("GET", `/api/v1/workspaces/${SPACE}/members`, {
    subject: OWNER,
  });
...
test("real path: add then role-change then remove all succeed", async () => {
  ...
  expect(
    (
      await h.call("POST", `/api/v1/workspaces/${SPACE}/members`, {
        subject: OWNER,
  ...
  const promote = await h.call(
    "PATCH",
    `/api/v1/workspaces/${SPACE}/members/tsub_bob`,
  ...
  const list = await h.call("GET", `/api/v1/workspaces/${SPACE}/members`, {
    subject: OWNER,
  });
...
test("access control: a non-owner non-member cannot mutate on the real path", async () => {
  ...
  const memberAdd = await h.call(
    "POST",
    `/api/v1/workspaces/${SPACE}/members`,
...
test("access control: last-owner cannot be demoted or removed on the real path", async () => {
  ...
  const demote = await h.call(
    "PATCH",
    `/api/v1/workspaces/${SPACE}/members/${OWNER}`,
