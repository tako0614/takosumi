...
} from "../../../core/api/runtime_agent_routes.ts";
...
test("runtime agent routes enroll, heartbeat, lease, complete, and drain", async () => {
  ...
  const enroll = await app.request(TAKOSUMI_RUNTIME_AGENT_PATHS.enroll, {
    method: "POST",
    body: JSON.stringify({
      agentId: "agent_1",
      provider: "local",
...
test("runtime agent routes return auth and registry errors", async () => {
  ...
  const invalidEnroll = await app.request(TAKOSUMI_RUNTIME_AGENT_PATHS.enroll, {
    method: "POST",
    body: JSON.stringify({}),
  });
...
function agentPath(
  ...
  return TAKOSUMI_RUNTIME_AGENT_PATHS[kind].replace(":agentId", agentId);
}
