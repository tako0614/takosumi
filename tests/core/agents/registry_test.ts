...
import { InMemoryRuntimeAgentRegistry, InMemoryWorkLedger } from "../../../core/agents/mod.ts";
import { DomainError } from "../../../core/shared/errors.ts";
...
// ---------------------------------------------------------------------------
// Phase 18 / C5 — service restart resumability via the persistent work ledger.
// ---------------------------------------------------------------------------
test("runtime agent registry resumes in-flight long-running work after a service restart (C5)",
  async () => {
    const ledger = new InMemoryWorkLedger();
    // Phase 1: a service boots, an agent registers + leases a long-running
    // op, then the service crashes.
    {
      const registry = new InMemoryRuntimeAgentRegistry({
        clock: fixedClock("2026-04-30T00:00:00.000Z"),
      ...
      // Service crashes here — the in-memory `registry` is dropped on the
      // floor with the work item still in `leased` state.
    }

    // Phase 2: service restart. A fresh registry is hydrated from the
...
test("runtime agent registry rehydration requeues stale leases on service boot (C5)",
  async () => {
    const ledger = new InMemoryWorkLedger();
    {
      const registry = new InMemoryRuntimeAgentRegistry({
      ...
      assert.ok(lease);
      // Lease expires at 00:00:30. Service "crashes" at 00:00:10.
    }
