...
} from "../../../../core/domains/security/mod.ts";
...
test("acceptance P3: NetworkServiceGrant is required before runtime internal service calls", async () => {
...
// minimization). Queue subscription canary semantics are no longer service
...
function createRuntimeSecurityAcceptanceHarness(options?: {
  ...
  const securityStores: WorkerAuthzStores = {
    ...
    serviceGrants: new InMemoryNetworkServiceGrantStore(),
