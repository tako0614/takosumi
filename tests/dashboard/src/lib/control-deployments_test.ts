 ...
 *
 *   GET  /api/v1/capsules/:id/state-versions       -> { deployments: [...] }
 *   GET  /api/v1/state-versions/:id                -> { deployment: {...} }
...
describe("listDeployments", () => {
  test("GETs the installation deployments route and unwraps `deployments`", async () => {
    ...
    expect(req.url).toBe("/api/v1/capsules/inst%201/state-versions");
...
describe("getDeployment", () => {
  test("GETs the deployment route and unwraps `deployment`", async () => {
    ...
    const req = captured();
    expect(req.method).toBe("GET");
    expect(req.url).toBe("/api/v1/state-versions/dep%201");
...
describe("createDeploymentRollbackPlan", () => {
  test("POSTs the rollback-plan route", async () => {
    ...
    expect(req.url).toBe("/api/v1/state-versions/dep%201/rollback-plan");
