...
describe("listInstallConfigsCached", () => {
  test("shares in-flight InstallConfig requests per Workspace and view", async () => {
    ...
    const [a, b] = await Promise.all([
      listInstallConfigsCached("space_1", { view: STORE_VIEW }),
      listInstallConfigsCached("space_1", { view: STORE_VIEW }),
    ]);

    expect(calls()).toEqual([
      `/api/v1/capsule-configs?workspaceId=space_1&view=${STORE_VIEW}`,
    ]);
    expect(a).toEqual(b);
    expect(a[0]?.id).toBe("cfg_1");
  });

  test("separates all-config and store view cache entries", async () => {
    const calls = stubInstallConfigFetch();

    await listInstallConfigsCached("space_1");
    await listInstallConfigsCached("space_1", { view: STORE_VIEW });
