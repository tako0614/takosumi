...
import { aggregateBillingUsage } from "../../../../accounts/service/src/billing-usage.ts";
import type { BillingUsageRecord } from "../../../../accounts/service/src/store.ts";
...
test("aggregateBillingUsage partitions rollups across billing accounts when no filter is set", () => {
  ...
  const bill1Seconds = rollups.find(
    (r) => r.billingAccountId === "bill_1" && r.unit === "seconds",
  );
  const bill1Minutes = rollups.find(
    (r) => r.billingAccountId === "bill_1" && r.unit === "minutes",
  );
