...
test("accounts handler exposes service graph services and rotates event ingest tokens", async () => {
  ...
  const catalog = await handler(
    new Request(
      `${testIssuer}${TAKOSUMI_ACCOUNTS_SERVICE_GRAPH_SERVICES_PATH}`,
      {
        headers: accountSessionHeaders(sessionId),
      },
    ),
  ...
  const aiGatewayBefore = servicesBeforeBody.services.find(
    (service: { id: string }) =>
      service.id === TAKOSUMI_ACCOUNTS_PLATFORM_SERVICE_AI_GATEWAY,
  );
  ...
  expect(eventsBefore.rotate_token_url).toContain(
    "/v1/installation-projections/inst_services/services/takosumi.events.webhook/rotate-token",
  );
  expect(aiGatewayBefore.status).toEqual("not_configured");
  expect(aiGatewayBefore.endpoint).toEqual(
    "https://accounts.example.test/gateway/ai/v1",
  );
  expect(aiGatewayBefore.material.baseUrl).toEqual(
    "https://accounts.example.test/gateway/ai/v1",
  );
  expect(aiGatewayBefore.material.apiKeyEnv).toEqual("OPENAI_API_KEY");
  expect(aiGatewayBefore.material.baseUrlEnv).toEqual("OPENAI_BASE_URL");
  expect(aiGatewayBefore.material.modelEnv).toEqual("OPENAI_MODEL");
  expect(aiGatewayBefore.material.defaultModel).toEqual("takosumi/default");
  expect(aiGatewayBefore.material.compatibleProtocols).toEqual([
    "openai.chat_completions",
    "openai.embeddings",
  ]);
  expect(aiGatewayBefore.material.runtimeConfigured).toEqual(false);
  expect(aiGatewayBefore.rotate_token_url).toContain(
    "/v1/installation-projections/inst_services/services/takosumi.ai.gateway/rotate-token",
  );

  const rotateAiGateway = await handler(
    new Request(
      ...
      )}`,
      {
        method: "POST",
        headers: accountSessionHeaders(sessionId),
        body: JSON.stringify({ scopes: ["ai.models.read", "ai.chat"] }),
      },
    ),
  );
  expect(rotateAiGateway.status).toEqual(200);
  const rotatedAiGateway = await rotateAiGateway.json();
  expect(rotatedAiGateway.token).toStartWith("taksrv_");
  expect(rotatedAiGateway.service.status).toEqual("not_configured");
  expect(rotatedAiGateway.service.material.runtimeConfigured).toEqual(false);
  expect(rotatedAiGateway.service.secret_ref).toEqual(undefined);
  expect(typeof rotatedAiGateway.service.token_expires_at).toEqual("string");
  expect((await store.findAccessToken(rotatedAiGateway.token))?.scope).toEqual(
    "ai.model ai.models.read ai.chat",
  );
  ...
  const invalidAiGatewayScopes = await handler(
    new Request(
      ...
      )}`,
      {
        method: "POST",
        headers: accountSessionHeaders(sessionId),
        body: JSON.stringify({ scopes: ["ai.chat", "control.api"] }),
      },
    ),
  );
  expect(invalidAiGatewayScopes.status).toEqual(400);
  ...
  expect(rotated.service.secret_ref).toEqual(undefined);
  expect(typeof rotated.service.token_expires_at).toEqual("string");
  expect(JSON.stringify(rotated)).not.toContain("tokenHash");
  expect(JSON.stringify(rotated)).not.toContain("secretRef");
  expect(JSON.stringify(rotated)).not.toContain("secret_ref");
  ...
  const eventLogAfterRotateBody = await eventLogAfterRotate.json();
  const eventLogAfterRotateText = JSON.stringify(eventLogAfterRotateBody);
  expect(eventLogAfterRotateText).not.toContain("tokenHash");
  expect(eventLogAfterRotateText).not.toContain("secretRef");
  expect(eventLogAfterRotateText).not.toContain("secret_ref");
      ...
      )}`,
      {
        ...
        body: JSON.stringify({
          ...
          payload: {
            ok: true,
            runId: "run_1",
            apiKey: "sk-service-event-raw",
            message:
              "worker returned Authorization: Bearer service-event-token",
            nested: {
              authorization: "Bearer service-event-nested-token",
              databaseUrl: "postgres://user:servicepass@db.example/takos",
            },
          },
        }),
      },
  ...
  expect(ingested.event.type).toEqual("service_event.ingested");
  expect(ingested.event.payload.type).toEqual("service.job.finished");
  expect(ingested.event.payload.payload.apiKey).toEqual("[REDACTED]");
  expect(ingested.event.payload.payload.nested.authorization).toEqual(
    "[REDACTED]",
  );
  expect(ingested.event.payload.payload.nested.databaseUrl).toEqual(
    "[REDACTED]",
  );
  expect(ingested.event.payload.payload.message).toContain("Bearer [REDACTED]");
  const ingestedText = JSON.stringify(ingested);
  expect(ingestedText).not.toContain("sk-service-event-raw");
  expect(ingestedText).not.toContain("service-event-token");
  expect(ingestedText).not.toContain("servicepass");

  const eventLogAfterIngest = await handler(
    new Request(
      `${testIssuer}${takosumiAccountsInstallationEventsPath("inst_services")}`,
      { headers: accountSessionHeaders(sessionId) },
    ),
  );
  expect(eventLogAfterIngest.status).toEqual(200);
  const eventLogAfterIngestText = JSON.stringify(
    await eventLogAfterIngest.json(),
  );
  expect(eventLogAfterIngestText).not.toContain("sk-service-event-raw");
  expect(eventLogAfterIngestText).not.toContain("service-event-token");
  expect(eventLogAfterIngestText).not.toContain("servicepass");
...
test("accounts handler marks AI Gateway service ready only when host runtime is configured", async () => {
  const store = new InMemoryAccountsStore();
  const handler = createAccountsHandler({
    store,
    serviceGraphRuntimeAvailability: { aiGatewayConfigured: true },
  });
  const now = Date.now();
  seedOwnedSpace(
    store,
    "tsub_owner",
    "acc_ai_gateway_ready",
    "space_ai_gateway_ready",
  );
  const sessionId = seedAccountSession(store, "tsub_owner", "sess_ai_gateway");
  store.saveAppInstallation({
    installationId: "inst_ai_gateway_ready",
    accountId: "acc_ai_gateway_ready",
    spaceId: "space_ai_gateway_ready",
    appId: "example.ai",
    sourceGitUrl: "https://github.com/example/ai",
    sourceRef: "main",
    sourceCommit: "abc123",
    planDigest: "sha256:manifest",
    mode: "shared-cell",
  ...
  const rotate = await handler(
    new Request(
      `${testIssuer}${takosumiAccountsInstallationServiceRotateTokenPath(
        "inst_ai_gateway_ready",
        TAKOSUMI_ACCOUNTS_PLATFORM_SERVICE_AI_GATEWAY,
      )}`,
      {
        method: "POST",
        headers: accountSessionHeaders(sessionId),
        body: JSON.stringify({ scopes: ["ai.chat"] }),
      },
    ),
  );
...
test("accounts handler accepts rotated billing service tokens without ServiceGrantMaterial storage", async () => {
  ...
  expect(
    store.listServiceGrantMaterialsForInstallation("inst_billing_service"),
  ).toEqual([]);
...
test("accounts handler accepts same-space service graph control tokens for scoped operations", async () => {
  const store = new InMemoryAccountsStore();
  const handler = createAccountsHandler({ store });
  const now = Date.now();
  seedOwnedSpace(store, "tsub_owner", "acct_1", "space_1");
  ...
  const invalidRotate = await handler(
    new Request(
      `${testIssuer}${takosumiAccountsInstallationServiceRotateTokenPath(
        "inst_control",
        TAKOSUMI_ACCOUNTS_PLATFORM_SERVICE_CONTROL_API,
      )}`,
      {
        method: "POST",
        headers: accountSessionHeaders(sessionId),
        body: JSON.stringify({ scopes: ["provider.credentials.manage"] }),
      },
    ),
  );
  expect(invalidRotate.status).toEqual(400);
  expect((await invalidRotate.json()).error.code).toEqual("invalid_request");
  ...
  const rotate = await handler(
    new Request(
      ...
      )}`,
      {
        ...
        body: JSON.stringify({
          ttlSeconds: 300,
          scopes: TAKOSUMI_ACCOUNTS_CONTROL_API_PERMISSIONS,
        }),
      },
    ),
  );
  expect(rotate.status).toEqual(200);
  const rotated = await rotate.json();
  expect(store.findAccessToken(rotated.token)?.scope).toEqual(
    ["control.api", ...TAKOSUMI_ACCOUNTS_CONTROL_API_PERMISSIONS].join(" "),
  );
  const controlHeaders = { authorization: `Bearer ${rotated.token}` };

  const list = await handler(
    new Request(`${testIssuer}/v1/installation-projections?space_id=space_1`, {
  ...
  expect(deploymentPlan.status).toEqual(401);
  expect((await deploymentPlan.json()).error).toEqual("invalid_token");

  const materialize = await handler(
    new Request(
  ...
  expect(materialize.status).toEqual(401);
  expect((await materialize.json()).error).toEqual("invalid_token");
  ...
  expect((await crossSpace.json()).error.code).toEqual(
    "installation_not_found",
  );

  const listOnlyRotate = await handler(
    new Request(
      `${testIssuer}${takosumiAccountsInstallationServiceRotateTokenPath(
        "inst_control",
        TAKOSUMI_ACCOUNTS_PLATFORM_SERVICE_CONTROL_API,
      )}`,
      {
        ...
        body: JSON.stringify({
          ttlSeconds: 300,
          scopes: ["installations.list.same-space"],
        }),
      },
    ),
  );
  expect(listOnlyRotate.status).toEqual(200);
  const listOnly = await listOnlyRotate.json();
  expect(store.findAccessToken(listOnly.token)?.scope).toEqual(
    "control.api installations.list.same-space",
  );
  const listOnlyHeaders = { authorization: `Bearer ${listOnly.token}` };

  const listOnlyList = await handler(
    new Request(`${testIssuer}/v1/installation-projections?space_id=space_1`, {
      headers: listOnlyHeaders,
    }),
  );
  expect(listOnlyList.status).toEqual(200);

  const listOnlyDetail = await handler(
    new Request(
      `${testIssuer}${takosumiAccountsInstallationPath("inst_control_target")}`,
      { headers: listOnlyHeaders },
    ),
  );
  expect(listOnlyDetail.status).toEqual(403);
  expect((await listOnlyDetail.json()).error.code).toEqual(
    "insufficient_scope",
  );
