...
} from "../../../../accounts/service/src/runtime.ts";
...
test("sharedCellRuntimeBinding creates per-installation namespace targets", () => {
  ...
  expect(binding.targetId).toEqual(
    "shared-cell://tokyo-cell-01/namespaces/inst_1",
  );
});
...
test("InMemorySharedCellWarmPool allocates warm capacity once per installation", () => {
  ...
  expect(first?.targetId).toEqual(
    "shared-cell://tokyo-cell-01/namespaces/inst_a",
  );
  expect(repeat?.targetId).toEqual(first?.targetId);
  expect(second?.targetId).toEqual(
    "shared-cell://tokyo-cell-02/namespaces/inst_b",
  );
