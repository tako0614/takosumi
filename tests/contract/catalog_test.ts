...
test("catalog guards pin official names", () => {
  ...
  assert.equal(isOfficialMaterialKindName("mcp-server@v1"), false);
...
test("catalog projection matrix matches official output material contracts", () => {
  assert.deepEqual(
...
test("catalog accepts valid official material samples", () => {
  ...
  assert.deepEqual(validateOfficialMaterial("service-binding", service), []);
  assert.deepEqual(
  ...
  assert.deepEqual(validateOfficialMaterial("billing.usage", billing), []);
  ...
  assert.deepEqual(
    validateOfficialMaterial("identity.oidc", {
...
test("catalog rejects invalid official material URL values", () => {
  ...
  assert.deepEqual(
    validateOfficialMaterial("billing.usage", {
      billingSubjectRef: "acct_123",
