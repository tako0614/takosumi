...
} from "../../contract/implementation.ts";
import type { Component } from "../../contract/implementation.ts";
import type {
  Deployment,
  Installation,
...
test("OperatorImplementation is a plain-array shape: name + provides + apply suffice", () => {
  const implementation: OperatorImplementation = {
    name: "@operator/cloudflare-workers",
    version: "1.0.0",
  ...
  assert.equal(implementation.name, "@operator/cloudflare-workers");
  assert.deepEqual(
  ...
  assert.equal(implementation.applyBinding, undefined);
});
...
test("OperatorImplementation.applyBinding returns an EnvInjection", async () => {
  const component: Component = {
    kind: "worker",
    connect: {
      db: { output: "database.connection", inject: "env", prefix: "DB" },
  ...
  const injection = await implementation.applyBinding!({
    installationId: "ins_1",
    componentName: "web",
    component,
    bindingName: "db",
