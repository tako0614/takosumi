...
test("predeployed verification accepts only the exact current head ledger", async () => {
  ...
  await extra
    .prepare(
      `insert into schema_migrations (version, name, checksum, applied_at)
       values (${latestD1OpenTofuSchemaMigration.version + 1}, 'unexpected', ?, '2026-08-05T00:00:00.000Z')`,
    )
