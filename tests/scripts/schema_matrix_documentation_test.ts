...
test("JP and EN schema matrices follow current source authorities", () => {
  ...
  const accountsD1Source = read("accounts/service/src/d1-migrations.ts");
  const accountsD1Versions = numericVersions(accountsD1Source);
  const accountsD1Latest = maximum(accountsD1Versions, "Accounts D1 catalog");
  ...
  const publishedRepositorySchemas = readdirSync(join(root, "docs/public/schemas"))
    ...
    .filter((value): value is string => value !== undefined)
    .sort();
  expect(publishedRepositorySchemas).toEqual([
    "v2.1",
    "v2.2",
    "v2.3",
    "v2.4",
  ...
  const requiredTokens = [
    ...
    String(accountsD1Versions.length),
  ...
  for (const matrix of matrices) {
    ...
    expect(matrix).not.toContain("install.takosumi.com/v1alpha1");
  }
...
test("the public contract README does not advertise a retired manifest lane", () => {
  ...
  for (const version of ["v1", "v2", "v2.1", "v2.2", "v2.3", "v2.4"]) {
