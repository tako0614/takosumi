...
describe("provider release source", () => {
  test("uses an independent 1.1.4 candidate lane and immutable historical quarantine", async () => {
    ...
    expect(descriptor.version).toBe("1.1.4");
    expect(descriptor.version).not.toBe(packageJson.version);
    expect(descriptor.tag).toBe("provider/v1.1.4");
  ...
  test("pins the quarantine manifest digest and rejects public source drift", async () => {
    ...
    expect(result.providerVersion).toBe("1.1.4");
      ...
      const prepublication = await verifyProviderPrepublication({
        ...
        fetchImpl: async (url) => {
          expect(String(url)).toBe(
            "https://app.takosumi.com/opentofu/providers/registry.opentofu.org/takosjp/takosumi/1.1.4.json",
          );
...
async function makeTaggedSourceFixture({ wrongGoDigest = false } = {}) {
  ...
  runGit(repoRoot, ["tag", "provider/v1.1.4"]);
  return {
    repoRoot,
