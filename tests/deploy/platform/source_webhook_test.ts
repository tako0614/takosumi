...
function makeWebhookOps(
  ...
  const ops: SourceWebhookOperations = {
    ...
    createSourceReconciliationSyncs: (sourceId) => {
      syncCalls.push({ sourceId, dedupe: true });
...
// (a JSON array of opaque
// `{ basePath, handlerKey, requiredScopes?, requestScopeRules? }`
...
test("platformExtensionRoutes merges duplicate capability descriptors", () => {
  expect(
    platformExtensionRoutes({
      TAKOSUMI_PLATFORM_EXTENSIONS: JSON.stringify([
        {
          basePath: "/extensions/example/v1",
          handlerKey: "TEST_EXTENSION",
          capabilities: ["example.storage.v1"],
        },
        {
          basePath: "/extensions/example/v1",
          handlerKey: "TEST_EXTENSION",
          capabilities: ["example.kv.v1"],
        },
      ]),
    }),
  ).toEqual([
    {
      basePath: "/extensions/example/v1",
      handlerKey: "TEST_EXTENSION",
      capabilities: ["example.storage.v1", "example.kv.v1"],
    },
  ]);
});
...
test("platform worker exposes product discovery before accounts handler", async () => {
  ...
  expect(capabilitiesBody).not.toHaveProperty("compat");
...
test("platform worker product discovery exposes only configured extension capabilities", async () => {
  const worker = (await import("../../../deploy/platform/worker.ts")).default;
  ...
  const capabilities = await worker.fetch(
    ...
    ),
    {
      TAKOSUMI_PLATFORM_EXTENSIONS: JSON.stringify([
        ...
        {
          basePath: "/extensions/provider/v1",
          handlerKey: "TEST_PROVIDER_EXTENSION",
          capabilities: ["provider.gateway.v1"],
        },
        {
  ...
  expect(body).not.toHaveProperty("compat");
  const discovery = await worker.fetch(
    new Request(`https://app.takosumi.com${TAKOSUMI_WELL_KNOWN_PATH}`),
    {
      TAKOSUMI_PLATFORM_EXTENSIONS: JSON.stringify([
  ...
  expect(discovery.status).toBe(200);
  const discoveryBody = await discovery.json();
  expect(discoveryBody.features).not.toHaveProperty("resource_shapes");
  expect(discoveryBody.features).not.toHaveProperty("compat_framework");
  expect(discoveryBody.features).not.toHaveProperty("compatibility_profiles");
  expect(discoveryBody.endpoints.extensions["provider.gateway.v1"]).toBe(
    "https://app.takosumi.com/extensions/provider/v1",
  );
});
...
test("platform keeps retired Takoform paths ahead of the SPA fallback", async () => {
  ...
  expect(platformDiscovery.features).not.toHaveProperty("resource_shapes");
  expect(platformDiscovery.features).not.toHaveProperty("compat_framework");
  expect(platformDiscovery.features).not.toHaveProperty("compatibility_profiles");
...
test("handler-auth platform extensions preserve protocol auth and strip spoofed context", async () => {
  const forwarded: {
    readonly authorization: string | null;
  ...
  const response = await handlePlatformExtensionRouteRequest(
    new Request("https://app.takosumi.com/extensions/storage/v1/assets/object.txt", {
      method: "PUT",
      headers: {
        authorization:
    ...
    {
      basePath: "/extensions/storage/v1",
...
test("platform extension request scope rules select one permission per exact AI path", async () => {
  ...
  let presentedScopes: readonly string[] = [];
  const verifier = async (
    _request: Parameters<typeof verifyPlatformExtensionSession>[0],
    _env: Parameters<typeof verifyPlatformExtensionSession>[1],
    effectiveRoute: Parameters<typeof verifyPlatformExtensionSession>[2],
  ) => {
    calls.push(
      `${effectiveRoute?.basePath}:${effectiveRoute?.requiredScopes?.join(",") ?? ""}`,
    );
    return {
      authenticated: true as const,
  ...
  const preflightAuthorization: string[] = [];
  const binding = {
    TEST_AI_EXTENSION: {
      fetch: async (request: Request) => {
        if (request.method === "OPTIONS") {
          preflightAuthorization.push(
  ...
  const preflight = await handlePlatformExtensionRouteRequest(
    ...
    route,
    async () => {
      throw new Error("platform auth must not run for preflight");
    },
  );
  expect(preflight.status).toBe(204);
  expect(preflightAuthorization).toEqual(["none"]);
  ...
  expect(deniedChat.status).toBe(401);
...
test("an exact inventory leaf narrows PAT access without closing the Marketplace subtree", async () => {
  const routes = platformExtensionRoutes({
    TAKOSUMI_PLATFORM_EXTENSIONS: JSON.stringify([
      ...
      {
        basePath: "/extensions/hosted/marketplace/resources",
        matchMode: "exact",
        handlerKey: "HOSTED",
  ...
  const walletRequest = new Request(
    "https://app.takosumi.com/extensions/hosted/marketplace/wallet",
  );
  const walletRoute = matchPlatformExtensionRoute(
    new URL(walletRequest.url).pathname,
    routes,
  );
  expect(walletRoute?.basePath).toBe("/extensions/hosted/marketplace");
  ...
  const resourcesRequest = new Request(
    "https://app.takosumi.com/extensions/hosted/marketplace/resources",
  );
  ...
  expect(resourcesRoute?.basePath).toBe(
    "/extensions/hosted/marketplace/resources",
  );
...
test("platform extension authenticates delegated OAuth access claims without prefix routing", async () => {
  const context = await verifyPlatformExtensionBearerToken(
    new Request(
      "https://app.takosumi.com/api/v1/interfaces?workspaceId=space_oauth",
      {
...
test("platform Interface API enforces delegated OAuth Capsule scopes", async () => {
  ...
  const request = () =>
    new Request(
      "https://app.takosumi.com/api/v1/interfaces?workspaceId=space_oauth&permission=mcp.invoke",
      { headers: { authorization: "Bearer takat_runtime" } },
    );

  const denied = await handlePlatformInterfaceApiRequest(
    request(),
    env,
    async () => ({
      authenticated: true,
  ...
  const allowed = await handlePlatformInterfaceApiRequest(
    request(),
    env,
    async () => ({
      authenticated: true,
  ...
  const mutationDenied = await handlePlatformInterfaceApiRequest(
    new Request("https://app.takosumi.com/api/v1/interfaces", {
      method: "POST",
      headers: {
  ...
  const tokenIssueReachedRuntimeBoundary =
    await handlePlatformInterfaceApiRequest(
...
test("platform Interface token issuance treats an exact Workspace PAT as a runtime Principal", async () => {
  ...
  const response = await handlePlatformInterfaceApiRequest(
    new Request("https://app.takosumi.com/api/v1/interfaces/if_missing/token", {
      method: "POST",
      headers: {
  ...
  const response = await handlePlatformInterfaceApiRequest(
    new Request(
      "https://app.takosumi.com/v1/resources/EdgeWorker/takos/artifacts?space=workspace_artifact",
      {
        ...
        body: JSON.stringify({ payload: "retired" }),
      },
    ),
    env,
    async () => ({
...
test("platform Interface API binds delegated OAuth requests to their Workspace", async () => {
  ...
  const crossCreate = await handlePlatformInterfaceApiRequest(
    new Request("https://app.takosumi.com/api/v1/interfaces", {
      method: "POST",
      headers: {
        authorization: "Bearer takat_runtime",
  ...
  const seeded = await handlePlatformInterfaceApiRequest(
    new Request("https://app.takosumi.com/api/v1/interfaces", {
      method: "POST",
      headers: {
  ...
  const crossRead = await handlePlatformInterfaceApiRequest(
    new Request(
      `https://app.takosumi.com/api/v1/interfaces/${seededId}?permission=mcp.invoke`,
      {
        headers: { authorization: "Bearer takat_runtime" },
      },
    ),
...
test("platform Interface ingress separates control and runtime credentials", async () => {
  ...
  const url =
    "https://app.takosumi.com/api/v1/interfaces?workspaceId=workspace_auth";
  ...
  for (const mutation of [
    { method: "POST", path: "/api/v1/interfaces", body: "{}" },
    { method: "PATCH", path: "/api/v1/interfaces/if_auth", body: "{}" },
    { method: "DELETE", path: "/api/v1/interfaces/if_auth" },
    {
      method: "POST",
      path: "/api/v1/interfaces/if_auth/bindings",
      body: "{}",
    },
    {
      method: "DELETE",
      path: "/api/v1/interfaces/if_auth/bindings/binding_auth",
    },
  ]) {
    const readOnlyPatWrite = await handlePlatformInterfaceApiRequest(
  ...
  const readOnlyPatRead = await handlePlatformInterfaceApiRequest(
    new Request(url, {
  ...
  for (const method of ["GET", "POST"]) {
    const serviceToken = await handlePlatformInterfaceApiRequest(
      new Request(url, {
        method,
        headers: {
...
test("platform extension request scope rules preserve the Interface OAuth audience", async () => {
  ...
  const effectiveRoute = resolvePlatformExtensionRequestScopeRoute(
    request,
    route,
  );
