...
Deno.test("takosumi framework ships no kind source modules", async () => {
  ...
  const kindImports = Object.keys(manifest.imports ?? {})
    .filter((specifier) =>
      ...
      specifier.startsWith("@takosjp/takosumi-kind-")
    );
  assert.deepEqual(
    kindImports,
    [],
    "the framework import map must not self-map any per-kind implementation alias",
  );
});
...
Deno.test("takosumi scripts use narrow contract subpaths", async () => {
  ...
  for (const file of files) {
    if (relativeToRoot(file) === "scripts/catalog-plugin-boundary_test.ts") {
      continue;
    }
    const source = await Deno.readTextFile(file);
    assert.equal(
...
Deno.test("reference compat does not expose legacy provider bridge", async () => {
  ...
  const manifest = await readJson<DenoManifest>(new URL("deno.json", ROOT));
  assert.equal(
    manifest.imports?.["takosumi-contract/internal/provider-plugin"],
...
Deno.test("kind alias docs state that the resolved URI owns the schema", async () => {
  const docs = [
    ...
    "docs/reference/reference-plugin-exports.md",
    "docs/en/reference/reference-plugin-exports.md",
  ...
  assert.match(
    await Deno.readTextFile(
      new URL("docs/en/reference/reference-plugin-exports.md", ROOT),
    ),
