...
export const RUNNER_IMAGE_RELEASE_CONTRACT_SURFACE = {
  ...
  covers: [
    ...
    "scripts/lib/runner-image-native-proof.ts",
    "scripts/runner-image-native-candidate.ts",
  ...
  obligations: {
    ...
    "failure-handling":
      "the executing account is CLOUDFLARE_ACCOUNT_ID and must equal the realized prior-image repository, whose exact target and environment select one external locator bound to the same physical machine identity and exact journal inode; a complete machine/PID-namespace/boot/PID-start/file-identity lock record is fsynced in a private pending inode before atomic no-overwrite publication, foreign host/namespace/boot locks are never reclaimed, and the locked operation holds and revalidates the same locator and journal descriptors immediately before push; the journal and containing directory are fsynced before push; a PID namespace change after restart does not rewrite or migrate the durable v3 locator, while a missing or replaced journal fails closed; the explicit reviewed recover-journal authority operation may bind one replacement inode only by publishing a no-overwrite v4 sidecar under the same scope lock after validating the complete observed journal and relations, sealing its prefix length and digest, and retaining the exact v3 bytes digest, expected old identity, current identity, recovery source authority, and continuity unknown; it never rewrites v3, calls a provider, resolves an unseen attempt, or claims complete history, and partial v4 persistence remains fail-closed and explicitly recoverable; this local state does not claim anti-rollback against a same-owner deletion of v4 plus restoration of the exact v3-bound inode; every observed unresolved unknown blocks every later build until exact read-only reconciliation proves the exact recorded tag present with matching image bytes or authoritatively absent; this is deliberately local-machine authority, not a distributed lock, so shared state across operator hosts remains unsupported even when machine IDs are cloned; bounded redacted evidence distinguishes pre-mutation failure from post-mutation unknown; verification performs no Worker mutation and fails on platform evidence, config, serving Version, application identity, image, rollout, or health ambiguity",
    "independent-review":
      "executing build, journal recovery, or verification requires a named --review value; recovery additionally requires --execute and records the exact recovery source authority while verification consumes the platform release reviewer and sealed plan evidence",
    "no-overwrite":
      "the source/content/nonce-bound mutable transport tag is never a version identity or consumer input and is not protected by a racy check-then-push claim; the published identity is the content-addressed sha256 descriptor digest, whose exact manifest bytes are bound through local/remote descriptor-digest equality, and verification consumes exactly one matching immutable-digest build record; continuity-unknown recovery does not prove a fresh transport target has a distinct descriptor, and distinct transport refs may lawfully resolve to the same immutable consumer digest",
  },
} as const;
