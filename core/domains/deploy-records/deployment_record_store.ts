...
/**
 * Persisted apply evidence for artifact retention and revoke cleanup.
 *
 * OpenTofu PlanRun / ApplyRun records are the canonical deployment ledger.
 * This store only keeps materialized resource handles and artifact hashes that
 * cleanup workers need after an apply has completed.
 */
export interface TakosumiDeploymentRecord {
  ...
  /** Deployment name selected by the deployment controller. */
  readonly name: string;
  /** Submitted source evidence retained for artifact reachability checks. */
  readonly sourceEvidence: JsonObject;
  /** Per-resource state derived from a successful materialization. */
...
export interface TakosumiAppliedResourceRecord {
  ...
  /**
   * Stable implementation-supplied fingerprint for idempotent materialization.
   * Takosumi stores it as opaque evidence; the public v1 contract does not
   * define how implementations compute it.
   */
...
export interface TakosumiDeploymentRecordStore {
   ...
   * primitive) so separate service pods share the same fence.
   */
  ...
  /**
   * Walk every persisted record's source evidence and per-resource `outputs` and
   ...
   * but their source evidence is preserved for audit; we still scan it
   * because a future re-apply may republish the same artifact
 ...
 * sees to the accumulator. We accept common evidence shapes:
 *  - `{ artifact: { hash: "sha256:..." } }` — DataAsset-backed handler
 *    evidence used by optional operator extensions.
 *  - bare `sha256:...` strings anywhere in the tree.
 *
...
/**
 * Helper used by callers that have materialized resource handles and source
 * resource metadata. Looks up each applied entry's shape from the
 * resources array so the persisted record carries the shape id alongside
 * the provider id. The `specFingerprint` is propagated through verbatim as
...
export function recordsFromAppliedResources(
  ...
  resources: readonly DeploymentRecordResource[],
  now: string,
...
export interface DeploymentRecordResource {
  readonly name: string;
  readonly shape: string;
}
