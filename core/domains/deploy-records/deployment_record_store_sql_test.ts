...
import { SqlTakosumiDeploymentRecordStore } from "./deployment_record_store_sql.ts";
...
test("SqlStore.listReferencedArtifactHashes returns hashes from source_evidence_json",
  async () => {
    const { store } = createStore();
    await store.upsert({
      tenantId: TENANT,
