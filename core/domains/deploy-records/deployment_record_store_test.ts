...
test("upsert keys tenant and deployment name as a tuple", async () => {
  ...
  assert.equal((await store.get("tenant a", "app"))?.sourceEvidence.name, "first");
  assert.equal((await store.get("tenant", "a app"))?.sourceEvidence.name, "second");
});
...
test("listReferencedArtifactHashes finds artifact.hash in sourceEvidence tree",
  async () => {
    const store = new InMemoryTakosumiDeploymentRecordStore();
    await store.upsert({
      tenantId: TENANT,
