 ...
 * the service's `objectStorage` adapter; digest is computed and verified
 ...
 * the `resource_exhausted` code instead of OOM-ing the service.
 *
 ...
 * AWS S3, GCS) and configure the service's `objectStorage` adapter to
 ...
 * buffering through service RAM. Until that lands, raise
 ...
 * without letting a single multipart request OOM the service process.
 */
...
export function registerArtifactRoutes(
   ...
   * deployed service knows about. CLIs use this to surface valid `--kind`
...
function tooLargeMessage(
  ...
  return (
    ...
    `uploads — the service buffers the whole multipart body before ` +
