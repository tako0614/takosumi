...
export interface RegisterRuntimeAgentRoutesOptions {
  ...
  /**
   * Issuer the service uses to mint a signed gateway manifest for the
   ...
   * sign every successful response with the service-trusted Ed25519 key so
...
export function registerRuntimeAgentRoutes(
  ...
  if (signer) {
    ...
    // Wrap every runtime-agent response with the service-trusted Ed25519
  ...
  app.post(TAKOSUMI_RUNTIME_AGENT_PATHS.enroll, async (c) => {
  ...
  app.post(TAKOSUMI_RUNTIME_AGENT_PATHS.heartbeat, async (c) => {
    const auth = await authenticate(c.req.raw);
    if (!auth.ok) return runtimeAgentAuthError(c, auth);
    const request = await readJsonObject(c.req.raw);
  ...
  app.post(TAKOSUMI_RUNTIME_AGENT_PATHS.lease, async (c) => {
    const auth = await authenticate(c.req.raw);
    if (!auth.ok) return runtimeAgentAuthError(c, auth);
    const request = await readJsonObject(c.req.raw);
    const subject = authorizeRuntimeAgentPathSubject({
  ...
  app.post(TAKOSUMI_RUNTIME_AGENT_PATHS.report, async (c) => {
    const auth = await authenticate(c.req.raw);
    if (!auth.ok) return runtimeAgentAuthError(c, auth);
    const request = await readJsonObject(c.req.raw);
    const subject = authorizeRuntimeAgentPathSubject({
  ...
  app.post(TAKOSUMI_RUNTIME_AGENT_PATHS.drain, async (c) => {
    const auth = await authenticate(c.req.raw);
    if (!auth.ok) return runtimeAgentAuthError(c, auth);
    const request = await readJsonObject(c.req.raw);
    const authorization = await authorizeRuntimeAgentMutation({
  ...
  app.post(TAKOSUMI_RUNTIME_AGENT_PATHS.gatewayManifest, async (c) => {
    const auth = await authenticate(c.req.raw);
    if (!auth.ok) return runtimeAgentAuthError(c, auth);
