...
export interface CreateTakosumiServiceOptions extends AppContextOptions {
  ...
  /**
   * Operator-selected public Git source for first-party store listings. The
   * default tracks the public development mirror, while hosted deployments can
   * point at an built-in release mirror without changing OSS source.
   */
...
export interface TakosumiOperations {
  ...
  /**
   * Space membership facade backing the account-plane `/api/v1/workspaces/:id/
   ...
   * /internal/v1/state-versions/:id/rollback-plan`): re-plans the Deployment's Installation
...
export async function createTakosumiService(
  ...
  await bootstrapInstallConfigs(sharedOpenTofuStore, {
