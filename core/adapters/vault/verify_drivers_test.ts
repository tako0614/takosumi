...
// --- provider_env_set (structural) ----------------------------------------

test("provider_env_set: all declared envNames present ⇒ verified", async () => {
  const result = await verifyProviderEnvSet({
    connection: connection({
...
test("provider_env_set: a declared envName missing ⇒ pending", async () => {
  const result = await verifyProviderEnvSet({
    connection: connection({
      kind: "provider_env_set",
      provider: "datadog",
...
test("provider_env_set: no declared envNames ⇒ pending", async () => {
  const result = await verifyProviderEnvSet({
    connection: connection({ kind: "provider_env_set", envNames: [] }),
    values: {},
    fetch: noFetch,
  });
  expect(result.ok).toBe(false);
...
test("gcp: reserved ⇒ structural verified, no fetch", async () => {
...
test("verifyDriverForKind routes each verifiable kind and is undefined otherwise", () => {
  ...
  expect(verifyDriverForKind("provider_env_set")).toBe(verifyProviderEnvSet);
