...
- Release builds are explicit: a clean `provider/v<version>` tag and its exact
  ...
  byte-for-byte clean before any build begins. Candidate and promotion jobs
  install the same SHA-pinned OpenTofu action and exact CLI version used by the
  canonical quality workflow, then verify its JSON version/platform identity.
