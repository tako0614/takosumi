/**
 * One small primitive for the dashboard's hand-rolled async-action machine.
 *
 * Almost every mutation path in the dashboard was the same shape: a `busy`
 * signal, an `error` signal, an optional `result` signal, and a
 * `setBusy(true) / setError(null) / try { ... } catch { setError(msg) }
 * finally { setBusy(false) }` block, with `(e as ApiError).message` copied
 * inline ~a dozen times. {@link createAction} captures that shape so the call
 * sites only describe the async work + its success side effects.
 *
 ...
 */
import { createSignal, type Accessor, Show, type JSX } from "solid-js";
