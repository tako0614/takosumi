/**
 * `DockerComposeConnector` — Docker Compose web-service driven by `docker run`.
 *
...
import { parseExternalWebServiceSpec } from "../_spec.ts";
...
export class DockerComposeConnector implements Connector {
  readonly provider = "@takos/docker-compose-web-service";
  ...
  async apply(
    ...
    const spec = parseExternalWebServiceSpec(req.spec);
