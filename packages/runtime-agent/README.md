...
lifecycle envelopes (apply / destroy / describe / verify) from the kernel
over HTTP and dispatches to the right per-provider connector, which makes
the actual cloud REST API call (SigV4 / OAuth / Cloudflare API token /
Azure ARM / Kubernetes / etc) or local OS call (`docker`, `systemd`,
filesystem).

The runtime-agent is the **only place cloud credentials live**. The kernel
never sees them.
...
A cloud's connectors are skipped (not registered) when its required env
vars are missing — operator can `takosumi runtime-agent verify` to confirm
which connectors are live.

## Implementation note

All cloud REST calls are made via `fetch()` + `crypto.subtle` (Web Crypto).
No npm SDK packages are pulled in; SigV4 / OAuth bearer / Service-account
JWT signing is internal. This keeps the agent Deno-runtime-pure and the
install surface small.

## See also

- [`@takos/takosumi-kernel`](https://jsr.io/@takos/takosumi-kernel) —
  control plane that talks to this agent
