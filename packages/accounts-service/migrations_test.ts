...
test("app grant capability catalog migration restricts grants", async () => {
  ...
  const migration = await readMigration(
    "009_app_grant_capability_catalog.sql",
  );
...
test("accounts boundary cleanup drops retired descriptor/import storage", async () => {
  ...
  expect(migration).toContain("DROP TABLE IF EXISTS accounts_v1.service_descriptors");
  expect(migration).toContain("DROP COLUMN IF EXISTS service_imports_json");
  expect(migration).toContain("DROP CONSTRAINT IF EXISTS app_installations_runtime_binding_id_fkey");
  expect(migration).toContain("event_sequence");
  expect(migration).toContain("installation_events_one_root_per_installation_idx");
...
test("personal access token migration stores Accounts PATs without raw secrets", async () => {
  ...
  expect(migration).toContain("CREATE TABLE IF NOT EXISTS accounts_v1.personal_access_tokens");
  ...
  expect(migration).toContain("scopes <@ ARRAY['read', 'write', 'admin']::text[]");
...
test("billing usage migration records metered AppInstallation usage", async () => {
  ...
  expect(migration).toContain("CREATE TABLE IF NOT EXISTS accounts_v1.billing_usage_records");
  expect(migration).toContain("REFERENCES installation_v1.app_installations");
  ...
  expect(migration).toContain("billing_usage_records_installation_idempotency_idx");
});
...
test("Wave 6 v1 contract reset drops binding/grant/runtime-binding tables", async () => {
  ...
  expect(migration).toContain("DROP TABLE IF EXISTS installation_v1.app_grants");
  expect(migration).toContain("DROP TABLE IF EXISTS installation_v1.app_bindings");
  expect(migration).toContain("DROP TABLE IF EXISTS installation_v1.runtime_bindings");
...
test("F7 event chain lock migration creates per-installation row lock", async () => {
  ...
  expect(migration).toContain("CREATE TABLE IF NOT EXISTS installation_v1.installation_event_chain_locks");
  expect(migration).toContain("installation_id text PRIMARY KEY");
  expect(migration).toContain("REFERENCES installation_v1.app_installations(installation_id)");
...
test("F30 refresh chain migration persists rotation links and code reuse markers", async () => {
  ...
  expect(migration).toContain("parent_token_hash text PRIMARY KEY CHECK (parent_token_hash LIKE 'sha256:%')");
  expect(migration).toContain("root_token_hash text NOT NULL");
  expect(migration).toContain("code_hash text PRIMARY KEY CHECK (code_hash LIKE 'sha256:%')");
...
test("auth_code_token_links sentinel migration replaces the NULL-in-PK columns", async () => {
  ...
  expect(migration).toContain("CHECK (refresh_root_hash = '' OR refresh_root_hash LIKE 'sha256:%')");
  expect(migration).toContain("PRIMARY KEY (code_hash, access_token_hash, refresh_root_hash)");
});
...
test("passkey challenge migration persists single-shot WebAuthn challenges", async () => {
  ...
  expect(migration).toContain("CREATE TABLE IF NOT EXISTS accounts_v1.passkey_challenges");
