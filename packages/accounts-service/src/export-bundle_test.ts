...
test("installation export bundle import plan rewrites OIDC issuer", () => {
  ...
  expect(bundle.kind).toEqual(TAKOSUMI_ACCOUNTS_INSTALLATION_EXPORT_BUNDLE_KIND);
  expect(bundle.source.commit).toEqual("0123456789abcdef0123456789abcdef01234567");
  ...
  expect(bundle.oidcClient?.useEdge).toEqual("auth");
  expect(bundle.oidcClient?.servicePath).toEqual("identity.primary.oidc");
  expect(bundle.oidcClient && !("namespacePath" in bundle.oidcClient)).toBeTruthy();
  expect(bundle.useEdges.map((useEdge) => useEdge.useEdgeId)).toEqual([
    "bind_auth",
    "bind_domain",
  ]);
  expect(bundle.permissionScopes.map((scope) => scope.permissionScopeId)).toEqual(["grant_logs", "grant_threads"]);
  ...
  const request = plan.request as {
    ...
    permissionScopes: readonly Record<string, unknown>[];
  };
  ...
  expect(request.oidcClients[0].useEdge).toEqual("auth");
  expect(request.oidcClients[0].servicePath).toEqual("identity.primary.oidc");
  expect(!("namespacePath" in request.oidcClients[0])).toBeTruthy();
  expect(request.permissionScopes.map((scope) => scope.permissionScopeId)).toEqual([
      "grant_threads",
    ]);

  const authBinding = request.useEdges.find((useEdge) =>
    useEdge.name === "auth"
  );
...
test("installation export archive writer emits canonical bundle payload", async () => {
  ...
  expect(files.map((file) => file.path)).toEqual([
      ...
      "takos-export/use-edges/template.yml",
      "takos-export/oidc/use-edge-template.json",
      "takos-export/docs/restore.md",
    ]);
  expect(files[0].content as string).toContain(TAKOSUMI_ACCOUNTS_INSTALLATION_EXPORT_BUNDLE_KIND);
  expect(files.at(-1)?.content as string).toContain("takosumi accounts installations import");
  const oidcTemplate = files.find((file) =>
    file.path === "takos-export/oidc/use-edge-template.json"
  );
  ...
  try {
    ...
    expect(parsed.source.commit).toEqual("0123456789abcdef0123456789abcdef01234567");
  } finally {
...
test("installation export archive writer embeds artifact descriptor content", async () => {
  ...
  const artifactDescriptor = files.find((file) =>
    file.path === "takos-export/artifact.yml"
  );
...
test("installation export archive writer includes deterministic provider data files", async () => {
  ...
  const manifest = files.find((file) =>
    file.path === "takos-export/data/manifest.json"
  );
  expect(manifest).toBeTruthy();
  expect(typeof manifest.content).toEqual("string");
  const parsedManifest = JSON.parse(manifest.content as string);
  expect(parsedManifest.kind).toEqual("takosumi.accounts.installation-export-data-manifest@v1");
...
test("metadata-only export worker writes archive and returns download URL", async () => {
  ...
  try {
    ...
    expect(result.downloadUrl).toEqual("https://downloads.example.test/accounts/exports/takos-export-op_worker.tar.zst");
    ...
    const manifest = await commandOutputText(
      command("tar", {
        args: [
          "--use-compress-program=zstd",
          "-xOf",
          join(root, "takos-export-op_worker_data.tar.zst"),
          "takos-export/data/manifest.json",
        ],
      }),
    );
...
test("metadata-only export worker keeps prefixed static archive paths readable", async () => {
  ...
  try {
    ...
    expect(result.downloadUrl).toEqual("https://downloads.example.test/accounts/exports/takos-export-op_static_prefix.tar.zst");
  ...
  try {
    ...
    const worker = createMetadataOnlyInstallationExportWorker({
      ...
      uploader: async (input) => {
        ...
        return {
          downloadUrl:
            `https://object-store.example.test/signed/${input.objectKey}?sig=test`,
    ...
    expect(result.downloadUrl).toEqual("https://object-store.example.test/signed/accounts/exports/takos-export-op_upload.tar.zst.age?sig=test");
    expect(result.downloadExpiresAt).toEqual("2026-05-09T00:00:45.000Z");
    expect(uploads.length).toEqual(1);
    expect(uploads[0].objectKey).toEqual("accounts/exports/takos-export-op_upload.tar.zst.age");
...
test("metadata-only export worker encrypts archive with age recipients", async () => {
  ...
  try {
    ...
    expect(result.downloadUrl).toEqual("https://downloads.example.test/accounts/exports/takos-export-op_worker_age.tar.zst.age");
