...
Takosumi の turnkey umbrella package。root export は public contract types で、
kernel / plugins / installer / runtime-agent / cli は sub-export から使います。
cloud-backed provider は **別 install** (=
...
- `jsr:@takos/takosumi/plugins` — reference adapter entry (official catalog
  descriptor helpers + reference adapter helpers)
- `jsr:@takos/takosumi/kinds` — Takosumi official catalog kind descriptors
...
- `jsr:@takos/takosumi-plugins` — official catalog helpers and reference adapter
  helpers
