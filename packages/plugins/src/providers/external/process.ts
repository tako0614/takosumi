/**
 * Subprocess lifecycle helper for the external profile.
 *
 ...
 *   - `status(name)` — fetch a structured `ExternalProcessStatus`
 *   - `stop(name)`   — graceful stop (SIGTERM → SIGKILL after `gracePeriodMs`)
 *   - `restart(spec)`/`logs(name)` — convenience wrappers
 *
 * Operators inject a `ExternalProcessRunner` that knows how to actually
...
export interface ExternalProcessStatus {
  readonly name: string;
  readonly state: ExternalProcessState;
  readonly pid?: number;
...
export interface ExternalProcessRunner {
  ...
  logs?(input: {
    ...
    readonly entries: readonly ExternalProcessLogEntry[];
    readonly nextCursor?: string;
  }>;
}

export interface ExternalProcessLogEntry {
  readonly stream: "stdout" | "stderr";
  readonly timestamp: string;
  readonly message: string;
}

export type ExternalProcessConditionType =
  | "ProcessReady"
  | "ProcessStopped"
  | "ProcessRestarted"
  | "ProcessFailed";

export interface ExternalProcessCondition {
  readonly type: ExternalProcessConditionType;
...
export type ExternalProcessConditionSink = (
  condition: ExternalProcessCondition,
) => void;

export interface ExternalProcessControllerOptions {
  readonly runner: ExternalProcessRunner;
  readonly clock?: () => Date;
  readonly defaultGracePeriodMs?: number;
  readonly readinessTimeoutMs?: number;
  readonly readinessIntervalMs?: number;
  readonly sleep?: (ms: number) => Promise<void>;
  readonly conditionSink?: ExternalProcessConditionSink;
}

export class ExternalProcessController {
  readonly #runner: ExternalProcessRunner;
  readonly #clock: () => Date;
  ...
  readonly #sleep: (ms: number) => Promise<void>;
  readonly #sink?: ExternalProcessConditionSink;

  constructor(options: ExternalProcessControllerOptions) {
   ...
   */
  async ensure(spec: ExternalProcessSpec): Promise<ExternalProcessStatus> {
  ...
  async status(name: string): Promise<ExternalProcessStatus | undefined> {
  ...
  async restart(spec: ExternalProcessSpec): Promise<ExternalProcessStatus> {
  ...
  async list(): Promise<readonly ExternalProcessStatus[]> {
  ...
  #emit(
    type: ExternalProcessConditionType,
