/**
 * Production-grade Postgres helpers for the external profile.
 *
 * Wraps the operator-injected `ExternalPostgresPoolClient` (see
...
import type { ExternalPostgresPoolClient } from "./injected_clients.ts";
import type { ExternalSqlClient, ExternalSqlValue } from "./sql.ts";

export type ExternalPostgresErrorCode =
  | "connection-refused"
...
export interface ExternalPostgresErrorOptions {
...
export class ExternalPostgresError extends Error {
  readonly code: ExternalPostgresErrorCode;
  readonly retryable: boolean;
  readonly sqlState?: string;
  readonly details?: Record<string, unknown>;

  constructor(
    code: ExternalPostgresErrorCode,
    message: string,
    options: ExternalPostgresErrorOptions = {},
  ) {
...
export interface ExternalPostgresMigration {
  readonly id: string;
  readonly description?: string;
  readonly statements: readonly string[];
}

export interface ExternalPostgresMigrationRecord {
  readonly id: string;
...
export interface ExternalPostgresMigrationOptions {
  readonly tableName?: string;
  readonly clock?: () => Date;
  readonly dryRun?: boolean;
}

export interface ExternalPostgresMigrationOutcome {
  readonly applied: readonly ExternalPostgresMigrationRecord[];
 ...
 */
export class ExternalPostgresMigrationRunner {
  readonly #client: ExternalSqlClient;
  readonly #tableName: string;
  readonly #clock: () => Date;
  readonly #dryRun: boolean;

  constructor(
    client: ExternalSqlClient,
    options: ExternalPostgresMigrationOptions = {},
  ) {
    this.#client = client;
  ...
  async listApplied(): Promise<readonly ExternalPostgresMigrationRecord[]> {
  ...
  async apply(
    migrations: readonly ExternalPostgresMigration[],
  ): Promise<ExternalPostgresMigrationOutcome> {
    await this.ensureLedger();
    const applied: ExternalPostgresMigrationRecord[] = [];
    ...
    for (const migration of migrations) {
      ...
      if (existing) {
        if (existing.checksum !== checksum) {
          throw new ExternalPostgresError(
            "unknown",
  ...
  async #runMigration(
    migration: ExternalPostgresMigration,
    checksum: string,
  ): Promise<void> {
    const exec = async (client: ExternalSqlClient) => {
...
export async function probePostgresHealth(
  client: ExternalPostgresPoolClient,
...
/**
 * Retry helper specialised for the external SQL surface. Maps unknown
 * errors into `ExternalPostgresError` and applies exponential backoff with
...
export async function retryPostgres<T>(
  ...
  options: {
    ...
    readonly classify?: (error: unknown) => ExternalPostgresError;
  } = {},
): Promise<T> {
  ...
  let lastError: ExternalPostgresError | undefined;
  ...
  throw lastError ??
    new ExternalPostgresError("unknown", "retry exhausted with no error");
}

function defaultClassifyError(error: unknown): ExternalPostgresError {
  if (error instanceof ExternalPostgresError) return error;
  const message = error instanceof Error ? error.message : String(error);
  if (/timeout/i.test(message)) {
    return new ExternalPostgresError("timeout", message, {
      cause: error,
      retryable: true,
    });
  }
  if (/serialization|deadlock/i.test(message)) {
    return new ExternalPostgresError("serialization-failure", message, {
      cause: error,
      retryable: true,
    });
  }
  if (/unique|duplicate key/i.test(message)) {
    return new ExternalPostgresError("unique-violation", message, {
      cause: error,
      retryable: false,
    });
  }
  if (/permission denied|not authorised|not authorized/i.test(message)) {
    return new ExternalPostgresError("permission-denied", message, {
      cause: error,
      retryable: false,
    });
  }
  if (/connect.*refused|ECONNREFUSED|connection terminated/i.test(message)) {
    return new ExternalPostgresError("connection-refused", message, {
      cause: error,
      retryable: true,
    });
  }
  return new ExternalPostgresError("unknown", message, {
    cause: error,
    retryable: false,
  });
}

function defaultRetryable(code: ExternalPostgresErrorCode): boolean {
  switch (code) {
...
export type _ExternalPostgresParameter = ExternalSqlValue; // re-export hint
