/**
 * Router config reload + health surface for the external profile.
 *
 * The base `ExternalRouterConfigAdapter` (router.ts) renders Caddy / Traefik
...
import type { ExternalRouterKind } from "./router.ts";

export interface ExternalRouterReloader {
  reload(input: {
    readonly kind: ExternalRouterKind;
    readonly path?: string;
  }): Promise<{ readonly reloadedAt: string; readonly revision?: string }>;
}

export interface ExternalRouterHealthProbe {
  probe(input: {
...
export type ExternalRouterConditionType =
  | "RouterValidated"
  | "RouterReloaded"
  | "RouterHealthy";

export interface ExternalRouterCondition {
  readonly type: ExternalRouterConditionType;
...
export type ExternalRouterConditionSink = (
  condition: ExternalRouterCondition,
) => void;

export interface ExternalRouterControllerOptions {
  readonly reloader?: ExternalRouterReloader;
  readonly healthProbe?: ExternalRouterHealthProbe;
  readonly clock?: () => Date;
  readonly conditionSink?: ExternalRouterConditionSink;
...
export class ExternalRouterController {
  readonly #reloader?: ExternalRouterReloader;
  readonly #probe?: ExternalRouterHealthProbe;
  readonly #clock: () => Date;
  readonly #sink?: ExternalRouterConditionSink;
  readonly #maxAttempts: number;
  readonly #sleep: (ms: number) => Promise<void>;
  readonly #initialBackoffMs: number;
  readonly #maxBackoffMs: number;

  constructor(options: ExternalRouterControllerOptions = {}) {
   ...
   */
  validate(content: string, kind: ExternalRouterKind): readonly string[] {
    const issues: string[] = [];
  ...
  #emit(
    type: ExternalRouterConditionType,
