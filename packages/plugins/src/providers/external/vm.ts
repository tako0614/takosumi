/**
 * VM / bare-metal provider materializer for the external profile.
 *
 * The container materializer (`ExternalContainerProviderMaterializer` in
 ...
 * to an operator-injected `ExternalVmRunner`. The runner can wrap:
 *
...
export type ExternalVmApplyMode = "cloud-init" | "ssh-runcmd";

export interface ExternalVmRunner {
  apply(input: ExternalVmApplyInput): Promise<ExternalVmApplyResult>;
  remove?(input: ExternalVmRemoveInput): Promise<ExternalVmRemoveResult>;
}

export interface ExternalVmApplyInput {
  readonly host: string;
  readonly mode: ExternalVmApplyMode;
...
export interface ExternalVmApplyResult {
...
export interface ExternalVmRemoveInput {
  readonly host: string;
  readonly activationId: string;
  readonly metadata?: Record<string, string>;
}

export interface ExternalVmRemoveResult {
  readonly status: "succeeded" | "failed";
  readonly code?: number;
  readonly stdout?: string;
  readonly stderr?: string;
...
export interface ExternalVmProviderOptions {
  readonly runner: ExternalVmRunner;
 ...
 */
export class ExternalVmProviderMaterializer
  ...
  constructor(options: ExternalVmProviderOptions) {
  ...
  async materialize(
    ...
    const operation = freezeClone({
      ...
      kind: "external-vm-cloud-init" as const,
      provider: "external",
      desiredStateId: desiredState.id,
  ...
  #composeSpec(desiredState: RuntimeDesiredState): CloudInitSpec {
    ...
    return {
      ...
      systemdUnits: units,
      finalMessage:
        `takos external-vm activation ${desiredState.activationId} ready`,
    };
  }
