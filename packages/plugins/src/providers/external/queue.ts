...
import {
  createDocumentStore,
  type ExternalDocumentStore,
  type ExternalSqlClient,
} from "./sql.ts";

export interface ExternalPostgresQueueAdapterOptions {
...
export class ExternalPostgresQueueAdapter implements queue.QueuePort {
  readonly #client: ExternalSqlClient;
  readonly #clock: () => Date;
  readonly #idGenerator: () => string;
  readonly #defaultVisibilityTimeoutMs: number;
  ...
  constructor(options: ExternalPostgresQueueAdapterOptions) {
  ...
  async #withQueueTransaction<T>(
    fn: (documents: ExternalDocumentStore) => Promise<T>,
  ): Promise<T> {
