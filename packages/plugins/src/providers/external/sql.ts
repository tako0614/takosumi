...
export type ExternalSqlPrimitive = string | number | boolean | null;
export type ExternalSqlJson =
  | ExternalSqlPrimitive
  | { readonly [key: string]: ExternalSqlJson }
  | readonly ExternalSqlJson[];
export type ExternalSqlValue =
  | ExternalSqlPrimitive
  | ExternalSqlJson
  | Date;
export type ExternalSqlParameters =
  | Readonly<Record<string, ExternalSqlValue | undefined>>
  | readonly (ExternalSqlValue | undefined)[];

export interface ExternalSqlQueryResult<
...
export interface ExternalSqlClient {
  query<Row extends Record<string, unknown> = Record<string, unknown>>(
    sql: string,
    parameters?: ExternalSqlParameters,
  ): Promise<ExternalSqlQueryResult<Row>>;
  transaction?<T>(
    fn: (transaction: ExternalSqlTransaction) => T | Promise<T>,
  ): Promise<T>;
}

export interface ExternalSqlTransaction extends ExternalSqlClient {
  commit?(): Promise<void>;
  rollback?(): Promise<void>;
}

export interface ExternalSqlStorageDriverOptions {
  readonly client: ExternalSqlClient;
  readonly clock?: () => Date;
  readonly providerSupportReports?: readonly Record<string, unknown>[];
}

export class ExternalSqlStorageDriver implements storage.StorageDriver {
  readonly statements = storage.storageStatementCatalog;
  readonly #client: ExternalSqlClient;
  readonly #clock: () => Date;
  readonly #providerSupportReports: readonly Record<string, unknown>[];

  constructor(options: ExternalSqlStorageDriverOptions) {
  ...
  async transaction<T>(
    ...
    if (this.#client.transaction) {
      return await this.#client.transaction(async (transaction) => {
        ...
        return await fn(
          new ExternalSqlStorageTransaction(
            documents,
...
class ExternalSqlStorageTransaction implements storage.StorageTransaction {
  ...
  constructor(
    documents: ExternalDocumentStore,
...
export interface ExternalDocumentStore {
...
class SqlDocumentStore implements ExternalDocumentStore {
  #ready = false;

  constructor(
    private readonly client: ExternalSqlClient,
    private readonly clock: () => Date,
  ) {}
  ...
  async put<T>(collection: string, id: string, body: T): Promise<T> {
    ...
    await this.client.query(
      ...
      { collection, id, bodyJson: body as ExternalSqlValue, now },
    );
...
class JsonCollectionStore {
  constructor(
    private readonly documents: ExternalDocumentStore,
