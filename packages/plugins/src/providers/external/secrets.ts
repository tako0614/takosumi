...
export interface ExternalSecretClient {
  putSecret(input: {
...
export interface ExternalSecretStoreAdapterOptions {
  readonly client: ExternalSecretClient;
  readonly clock?: () => Date;
}

export class ExternalSecretStoreAdapter implements secretStore.SecretStorePort {
  readonly #client: ExternalSecretClient;
  readonly #clock: () => Date;

  constructor(options: ExternalSecretStoreAdapterOptions) {
...
export interface ExternalKmsClient {
...
export class ExternalKmsAdapter implements kms.KmsPort {
  constructor(private readonly client: ExternalKmsClient) {}
...
export interface ExternalLocalAesGcmKmsOptions {
...
export class ExternalLocalAesGcmKmsClient implements ExternalKmsClient {
  readonly #keyRef: kms.KmsKeyRefDto;
  readonly #keyPromise: Promise<CryptoKey>;
  readonly #clock: () => Date;

  constructor(options: ExternalLocalAesGcmKmsOptions) {
    this.#keyRef = {
      provider: "local-webcrypto",
      keyId: options.keyId ?? "external-local",
  ...
  async decrypt(input: kms.KmsDecryptInput): Promise<Uint8Array> {
    if (input.envelope.algorithm !== "AES-256-GCM") {
      throw new Error(
        `unsupported external KMS envelope algorithm: ${input.envelope.algorithm}`,
      );
    }
