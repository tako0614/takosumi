 ...
 *
 * Why an adapter? The shape-provider files in
 * `packages/plugins/src/shape-providers/` still expose the production
 * runtime-agent + in-memory lifecycle plumbing that the runtime-agent
 * smoke / batch tests depend on; we don't rewrite that surface for Wave 9
 * Phase D. Instead, the bundled factories wrap each provider so operators
 * see the new `KernelPlugin` shape on the outside while everything below
 * stays put.
 */
