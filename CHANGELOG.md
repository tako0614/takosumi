...
- The Cloudflare Compatibility Gateway, AI Gateway, managed resources, and
  Stripe-enforced billing move to the closed `takosumi-cloud/` delta (one-way
  Cloud → OSS, Seam A route proxy + Seam B composition ports). OSS billing is a
  Workspace/Organization-scoped showback-or-disabled ledger with no payment gate.
