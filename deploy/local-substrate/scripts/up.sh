...
INGRESS_HTTP_PORT="${TAKOSUMI_LOCAL_SUBSTRATE_INGRESS_HTTP_PORT:-80}"
INGRESS_HTTPS_PORT="${TAKOSUMI_LOCAL_SUBSTRATE_INGRESS_HTTPS_PORT:-443}"
export TAKOSUMI_LOCAL_SUBSTRATE_INGRESS_IP="$INGRESS_IP"
export TAKOSUMI_LOCAL_SUBSTRATE_DNS_HOST_BIND="$DNS_HOST_BIND"
export TAKOSUMI_LOCAL_SUBSTRATE_INGRESS_HTTP_PORT="$INGRESS_HTTP_PORT"
export TAKOSUMI_LOCAL_SUBSTRATE_INGRESS_HTTPS_PORT="$INGRESS_HTTPS_PORT"
...
if [[ -n "$PROFILE" ]]; then
	...
	for _ in $(seq 1 120); do
		...
		if curl -sk --cacert caddy/runtime/pebble-issuance-root.pem \
			--resolve "app.takosumi.test:${INGRESS_HTTPS_PORT}:127.0.0.1" \
			"https://app.takosumi.test:${INGRESS_HTTPS_PORT}/.well-known/openid-configuration" \
			>/dev/null 2>&1; then
			break
		fi
		sleep 2
...
==> INGRESS_IP=$INGRESS_IP, INGRESS_HOST_BIND=$INGRESS_HOST_BIND, HTTP_PORT=$INGRESS_HTTP_PORT, HTTPS_PORT=$INGRESS_HTTPS_PORT, DNS_HOST_BIND=$DNS_HOST_BIND, APP_UPSTREAM=$TAKOSUMI_LOCAL_APP_UPSTREAM
