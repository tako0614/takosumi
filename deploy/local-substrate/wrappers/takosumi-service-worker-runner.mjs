 ...
 *
 * Boots takosumi/deploy/cloudflare's Worker-first service scaffold in
...
if (process.env.LOCAL_SUBSTRATE_TEST_BED !== "1") {
  console.error(
    "[takosumi-service-worker] refusing to start: this runner is local-substrate-only.\n" +
...
const scriptPath =
  process.env.WORKER_SCRIPT ?? "/worker/takosumiflare-worker.mjs";
...
const mf = new Miniflare({
  ...
  d1Databases: { TAKOSUMI_CONTROL_DB: "takosumi-service" },
  d1Persist: "/data/d1",
...
console.log(`[takosumi-service-worker] miniflare serving at ${url}`);
