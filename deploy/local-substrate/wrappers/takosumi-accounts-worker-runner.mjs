...
if (process.env.LOCAL_SUBSTRATE_TEST_BED !== "1") {
  console.error(
    "[takosumi-worker] refusing to start: this runner is local-substrate-only.\n" +
...
const scriptPath = process.env.WORKER_SCRIPT ??
  "/worker/takosumi-accounts-worker.mjs";
...
const mf = new Miniflare({
  ...
  d1Databases: { TAKOSUMI_ACCOUNTS_DB: "takosumi-accounts" },
  d1Persist: "/data/d1",
...
console.log(`[takosumi-worker] miniflare serving at ${url}`);
