  ...
  if (env.AWS_ACCESS_KEY_ID && env.AWS_SECRET_ACCESS_KEY) {
    ...
    reg.register(
      new AwsFargateConnector({
        ...
        ...(env.AWS_FARGATE_SECURITY_GROUP_IDS
          ...
          : {}),
        ...(env.AWS_FARGATE_EXECUTION_ROLE_ARN
          ? { executionRoleArn: env.AWS_FARGATE_EXECUTION_ROLE_ARN }
          : {}),
        ...(env.AWS_FARGATE_TASK_ROLE_ARN
          ? { taskRoleArn: env.AWS_FARGATE_TASK_ROLE_ARN }
          : {}),
        ...(env.AWS_FARGATE_ASSIGN_PUBLIC_IP === "true"
          ? { assignPublicIp: true }
          : {}),
      }),
    );
    reg.register(
      new AwsRdsConnector({
        region,
        credentials,
        ...(env.AWS_RDS_SUBNET_GROUP
          ? { subnetGroupName: env.AWS_RDS_SUBNET_GROUP }
          : {}),
        ...(env.AWS_RDS_SECURITY_GROUP_IDS
          ? { securityGroupIds: parseList(env.AWS_RDS_SECURITY_GROUP_IDS) }
          : {}),
      }),
    );
  ...
  if (env.DENO_DEPLOY_ACCESS_TOKEN) {
    reg.register(
      new DenoDeployWorkersConnector({
        ...
        ...(env.DENO_DEPLOY_ORGANIZATION_ID
  ...
  if (
    ...
    reg.register(
      new K3sDeploymentConnector({
        ...
        ...(env.TAKOSUMI_KUBERNETES_CLUSTER_DOMAIN
          ? { clusterDomain: env.TAKOSUMI_KUBERNETES_CLUSTER_DOMAIN }
          : {}),
      }),
    );
  }
