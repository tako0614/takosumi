...
- An in-cluster service account with cluster-wide apply permissions (or
  namespace-scoped if the runtime-agent only deploys into one namespace).
- A Postgres database for kernel state (`TAKOSUMI_DATABASE_URL` — any in-cluster
  Postgres operator or external managed Postgres).
...
State lives in a separate Postgres instance (Zalando Postgres operator,
CrunchyData, or external managed Postgres).
