...
test("Cloudflare Accounts Worker parses env clients without a container proxy", async () => {
  const d1 = new InitOnlyD1Database();
  const worker = createCloudflareWorker();
  const response = await worker.fetch(
    new Request("https://accounts.example/.well-known/openid-configuration"),
...
test("Cloudflare Accounts Worker writes metadata exports to R2 with signed downloads", async () => {
  ...
  const createResponse = await routeWorker.fetch(
    new Request("https://accounts.example.test/v1/installations", {
      ...
      body: JSON.stringify({
        ...
        source: {
          gitUrl: "https://github.com/example/route-export",
          ref: "v1.0.0",
  ...
  const routedDownloadResponse = await routeWorker.fetch(
    new Request(exported.downloadUrl),
    routeEnv,
  );
...
test("managed-offering 'open' refuses an invalid issuer URL", async () => {
  const d1 = new InitOnlyD1Database();
  const worker = createCloudflareWorker();
  const env = createEnv(d1, {
