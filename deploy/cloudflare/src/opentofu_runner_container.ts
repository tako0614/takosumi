 ...
 * in the job. When ABSENT the DO falls back to the legacy TAKOS_ARTIFACTS
...
export class TakosumiOpenTofuRunner extends OpenTofuRunnerContainerBase<CloudflareWorkerEnv> {
  ...
  async #fetchWithDurablePlanArtifacts(
    ...
    // to the legacy TAKOS_ARTIFACTS state path so older jobs/tests keep working.
  ...
  async #persistPlanArtifact(
    ...
    const stored = await this.env.TAKOS_ARTIFACTS.put(
      encryptedKey(key),
      sealed.ciphertext,
      {
        httpMetadata: { contentType: ENCRYPTED_ARTIFACT_CONTENT_TYPE },
  ...
  async #persistPlanJsonArtifact(runId: string, baseUrl: URL): Promise<void> {
    ...
    const digest = await digestBytes(bytes);
    const sealed = await this.#stateCrypto().seal(bytes);
    await this.env.TAKOS_ARTIFACTS.put(
  ...
  async #readPlanArtifactPlaintext(
    ...
    const encrypted = await this.env.TAKOS_ARTIFACTS.get(encryptedKey(key));
    if (encrypted) {
