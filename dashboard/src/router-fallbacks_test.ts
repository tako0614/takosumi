 ...
 * visitor on a blank screen. The takosumi.com marketing site links its sign-up
 * / sign-in CTAs to `/signup` and `/login`, but this SPA only ships a single
 * `/sign-in` screen, and the SPA host serves a 200 fallback for unknown paths.
...
describe("dashboard router fallbacks (FATAL FIX 3)", () => {
  test("/signup redirects to /sign-in (website CTA target)", () => {
    expect(source).toMatch(
      /<Route\s+path="\/signup"\s+component=\{\(\)\s*=>\s*<Navigate\s+href="\/sign-in"\s*\/>\}\s*\/>/,
    );
  });

  test("/login redirects to /sign-in (website CTA target)", () => {
    expect(source).toMatch(
      /<Route\s+path="\/login"\s+component=\{\(\)\s*=>\s*<Navigate\s+href="\/sign-in"\s*\/>\}\s*\/>/,
    );
  });
