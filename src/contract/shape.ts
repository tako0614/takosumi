/**
 * Author-facing kind descriptor type and runtime-handler-local shape registry.
 *
 ...
 * separate, optional runtime-handler-local lookup. The default reference apply
 * facade resolves kinds through `OperatorImplementation`, not this registry, so it is
 * left empty unless a runtime handler explicitly uses descriptor-local validation.
 */
