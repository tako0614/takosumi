...
export interface LocalSourceInput {
  readonly kind: "local";
  /** Service-local source root path for dev / operator-local profiles. */
  readonly url: string;
  readonly ref?: never;
  readonly commit?: never;
  readonly digest?: never;
...
export type InstallerErrorHttpStatus =
  | 400
  | 401
  | 403
  | 404
...
export const INSTALLER_ERROR_CODES = [
  "invalid_argument",
  "unauthenticated",
  "permission_denied",
  "not_found",
...
export const INSTALLER_ERROR_HTTP_STATUS_BY_CODE = {
  ...
  not_implemented: 501,
  internal_error: 500,
} as const satisfies Record<InstallerErrorCode, InstallerErrorHttpStatus>;
