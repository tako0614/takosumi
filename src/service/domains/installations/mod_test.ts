...
import { InstallationsService } from "./mod.ts";
...
test("putDeploymentProfile validates the installation + matching space", async () => {
  const { store, service } = build();
  await seedAll(store);
  const installation = await service.createInstallation({
    spaceId: "space_1",
  ...
  const profile = await service.putDeploymentProfile({
    id: "dpf_1",
    spaceId: "space_1",
  ...
  const fetched = await service.getDeploymentProfileByInstallation(
    installation.id,
    "production",
  );
...
test("putDeploymentProfile rejects a spaceId mismatching the installation", async () => {
  const { store, service } = build();
  await seedAll(store);
  const installation = await service.createInstallation({
    spaceId: "space_1",
