...
// Space conformance tests for Deployment.resolution / Deployment.desired /
...
import { REFERENCE_DESCRIPTOR_CONFORMANCE_RECORDS } from "./descriptor_plan.ts";
import {
  DeploymentService,
...
const SOURCE_JS_MODULE_DESCRIPTOR_ID =
  "https://takosumi.com/reference/service/contracts/source/js-module/v1";
...
test("space conformance: resolveDeployment pins descriptor closure and resolved graph", async () => {
...
test("space conformance: descriptor closure uses docs JSON-LD body digests", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: source/runtime-input descriptors use current bundled kind names", () => {
...
test("space conformance: reference descriptor refs resolve in-tree", () => {
...
test("space conformance: source-backed worker pins source descriptor in closure and graph", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: public shorthand expansion is descriptor-traced", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
// Wave J removed the service `exposure-target` projection family that the
...
// service's reduced projection set.

test("space conformance: resource access paths emit policy decisions on resolution", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: public resource bindings become plan-visible resource access paths", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: removed service import bindings are rejected", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
    idFactory: () => "deployment_service_import_binding_1",
    clock: fixedClock("2026-04-27T00:00:00.000Z"),
  });
...
test("space conformance: blockers from authoring resolution surface as Deployment.conditions", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: approveDeployment stores approval without applying", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: activation envelope assignment invariants — primary maps to a component", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
// Wave J removed service-side `activation_envelope.route_assignments`; the
...
// because the service no longer emits route assignments.

test("space conformance: applyDeployment emits ActivationCommitted on success", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: rolledBack condition is appended to the previously current Deployment", async () => {
  const store = new InMemoryDeploymentStore();
  let counter = 0;
  const service = new DeploymentService({
    store,
...
test("space conformance: unsupported runtime type is rejected at resolution without rewriting Space contracts", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
  ...
  await assert.rejects(
    ...
    // Compiler rejects unknown runtime type before we get to Space projection.
    /unsupported|unknown|invalid/i,
  );
});
test("space conformance: provider capabilities reject unsupported runtime capability requirements", async () => {
...
test("space conformance: minInstances derives always-on container capability", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: provider capabilities reject unsupported resource access paths without rewriting access mode", async () => {
...
test("space conformance: unsupported route protocol is rejected at resolution", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
// state mutation.
test("space conformance: apply rejects descriptor closure drift without mutating activation", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: read-set validation uses current descriptor provider snapshots", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: missing read-set snapshots are stale", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
// Wave J removed the canary route-assignment chain and the service
...
// projections. Deleted because the service no longer emits either.
test("space conformance: unsupported route target is blocked at resolution", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
// Wave J removed service-side canary route assignment ownership and the
// `exposure-target` projection. The "route canaries must be
// activation-owned" test asserted on `desired.activation_envelope
// .route_assignments` / `exposure-target` projections that the service no
// longer emits. Deleted because the activation-ownership invariant has no
// surface on the routes-agnostic service.
test("space conformance: resource access path descriptor refs are carried onto Deployment.desired.bindings", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: runtime network policy can allow external resource access path", async () => {
...
test("space conformance: approval is recorded on Deployment.approval and is independent of apply", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
  ...
  // in Space plan" — it is stored on the Deployment record without applying.
  const approved = await service.approveDeployment({
    deploymentId: resolved.id,
    approval: {
      approved_by: "acct_owner",
...
test("space conformance: native raw binding requires policy approval before apply", async () => {
  const store = new InMemoryDeploymentStore();
  let counter = 0;
  const service = new DeploymentService({
    store,
...
test("space conformance: approval applied to a denied resolution does not unblock apply", async () => {
...
test("space conformance: required provider features deny unsupported runtime capabilities locally", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: pgvector native feature realization requires approval", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: cross-contract previousNames are denied at resolution", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: canary candidate-scoped egress requires approval", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: shadow side-effect manifests are denied before DB semantic write approval", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: advisory external access path records audit allow while enforced path denies", async () => {
...
test("space conformance: ambiguous resource binding shorthand is blocked at resolution", async () => {
...
test("space conformance: activation envelope route assignments use the canonical permille weight", async () => {
  const store = new InMemoryDeploymentStore();
  const service = new DeploymentService({
    store,
...
test("space conformance: applying a finalized Deployment never re-runs activation", async () => {
...
test("space conformance: provider observation is recorded separately from materialization", async () => {
...
test("space conformance: condition reasons are exposed from the catalog", () => {
  for (
    const reason of [
      "DescriptorChanged",
      "BindingCollision",
...
test("space conformance: reference descriptor conformance dataset is exposed", () => {
