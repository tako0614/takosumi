...
export interface ReferenceDeploySourcePayload extends Record<string, unknown> {
   ...
   * tooling, but the service pipeline drops it at `compileSourcePayloadToInternalDeploySpec`
   * (= post Wave J Component contract minimization, the InternalDeploySpec / service
...
/**
 * Vestigial route record — the service pipeline no longer compiles or
 * iterates routes (= post Wave J Component contract minimization,
 * routes are a worker-materializer convention, not a service concern).
...
export interface InternalDeploySpec {
  ...
  /**
   * Always empty after Wave J Component contract minimization — service
   ...
   * source payload text is identical (Space spec § 6 — closure determinism).
   *
