...
//
// This module supports the reference service's internal deploy conformance
...
// The closure is the reference-service record consumed by Apply — apply reuses
...
//      contract instance shape. (Space spec § 5.)
...
//   6. The shared reference-service JSON-LD context
...
//   - `id`        — reference-service URI
//                   (e.g. `https://takosumi.com/reference/service/contracts/runtime/oci-container/v1`)
...
//     `descriptor_plan.ts`) seeds the URI ↔ alias ↔ raw-digest mapping at startup by
 ...
 *  URI under `https://takosumi.com/reference/service/descriptors/unknown/<sanitised-alias>` when no
...
function canonicalUriFor(ref: string): { id: string; alias: string } {
  ...
  return {
    id:
      `https://takosumi.com/reference/service/descriptors/unknown/${sanitised}`,
    alias: ref,
  };
}
...
export async function buildDescriptorClosure(
  ...
  // descriptor body that referenced it (Space spec § 6 — JSON-LD context is a
...
function isImplementationOwnedDescriptorAlias(alias: string): boolean {
  ...
  if (
    alias.startsWith(
      "https://takosumi.com/reference/service/descriptors/unknown/",
    )
  ) {
    return false;
  }
