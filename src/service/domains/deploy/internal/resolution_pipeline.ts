...
export async function buildDeploymentArtifacts(input: {
  ...
  // `buildResolvedGraph` became async after the service switched from
  ...
  // them from raw descriptors (Space spec § 8).
  ...
  // an explicit `runtime_network_policy` egress allow rule (Space spec § 12
