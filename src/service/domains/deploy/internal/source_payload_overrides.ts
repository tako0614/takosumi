// Override-merge phase for the reference deploy source payload.
//
// `prepareReferenceDeploySourcePayload` runs first: it validates the top-level
// envelope, then if an `envName` is selected, merges the matching
// `overrides[envName]` block back over the base payload before the
// validation phase looks at the result.

import type { ReferenceDeploySourcePayload, PublicOutputSpec } from "../types.ts";
import {
  assertKnownFields,
  isRecord,
...
export function prepareReferenceDeploySourcePayload(
...
function validateTopLevelAndOverrides(manifest: ReferenceDeploySourcePayload): void {
  const candidate = manifest;
  for (const field of FORBIDDEN_PUBLIC_FIELDS) {
    if (field in candidate) {
      throw new TypeError(
        `reference deploy source payload must not include '${field}'`,
      );
    }
  }
  assertKnownFields(
    candidate,
    TOP_LEVEL_FIELDS,
    "reference deploy source payload",
  );
  if (!manifest.name || typeof manifest.name !== "string") {
    throw new TypeError(
      "reference deploy source payload requires string field 'name'",
    );
  }
  if (manifest.overrides !== undefined && !isRecord(manifest.overrides)) {
    throw new TypeError(
      "reference deploy source payload field 'overrides' must be object",
    );
  }
...
function mergeEnvironmentOverride(
  ...
  if (override.routes !== undefined) {
    output.routes = structuredClone(
      override.routes as ReferenceDeploySourcePayload["routes"],
    );
  }
