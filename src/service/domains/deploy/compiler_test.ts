...
// Wave J removed service-side route compilation; the
// "accepts default-app style routes and outputs" test that asserted on
// `deploySpec.routes[].to/path` projections was deleted because the service no
...
// Workflow/trigger authoring is owned above the service by installer clients.
...
// Wave J removed the service-side `routes`/`outputs` map projection that the
// "keeps route and outputs map" test exercised; deleted because the service no
...
// Wave J removed service-side route override projection; the "applies selected
// environment overrides" test asserted on `deploySpec.routes` produced by the
// `overrides.production.routes[]` chain. Deleted because the service no longer
...
// Wave J removed service-side route protocol normalization (HTTP/HTTPS/TCP/UDP/
// queue interface contract resolution); the "normalizes non-HTTP route
// protocols" test asserted on the removed `route.protocol` /
// `interfaceContractRef` projections. Deleted because the service no longer
