...
// Wave J removed service-side `exposure-target` projection and
...
// length being 1. Deleted because the service no longer emits those.
...
test("deploy: applying a denied resolution is rejected (status='failed')", async () => {
  ...
  // of "apply rejects Space plans that still require approval".
  const store = new InMemoryDeploymentStore();
  let counter = 0;
  const service = new DeploymentService({
    store,
...
test("deploy: rollback is a GroupHead pointer move (no new Deployment is created)", async () => {
  ...
  // Deployment count is unchanged; rollback is a pointer move per Space
...
test("apply preserves source snapshot validation hooks", async () => {
  ...
  // live provider contract performs (Space spec § 13 step 4).
  await assert.rejects(
    () =>
      service.applyDeployment({
        deploymentId: resolved.id,
...
test("rollback is blocked when retained Space artifacts are unavailable", async () => {
  const store = new InMemoryDeploymentStore();
  let counter = 0;
  const service = new DeploymentService({
    store,
