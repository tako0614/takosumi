...
export interface ApplyDeploymentInput {
  ...
  /**
   * Source-snapshot validation hook (Space spec § 13 step 4). Verifies the
...
export interface RollbackGroupInput {
  ...
  readonly descriptorClosureValidator?: ApplyPreflightValidator;
  /**
   * Retained Space artifact availability hook (Phase 17D). Verifies the
 ...
 * live provider snapshot — which the Takosumi service genuinely cannot probe. They
...
export class DeploymentService {
   ...
   *
   * Phase 10D semantics (Space spec § 13):
   ...
   *
   * Per Space spec § 15 a rollback is a strongly consistent pointer move on
  ...
  async #rollbackGroup(
    ...
    // canonical again with its original `applied` status (Space spec § 15
