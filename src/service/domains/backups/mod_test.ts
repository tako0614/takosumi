...
function decodeZstdRaw(bytes: Uint8Array): Uint8Array {
  ...
  while (input < bytes.byteLength && output < out.byteLength) {
    const header = bytes[input]! | (bytes[input + 1]! << 8) | (bytes[input + 2]! << 16);
    input += 3;
...
test("control bundle carries state-snapshot metadata + output projection only", async () => {
  ...
  expect(bundle.providerTemplates.map((entry: any) => entry.id)).toEqual([
    "cloudflare",
  ]);
  expect(bundle.deploymentProfiles.map((profile: any) => profile.id)).toEqual([
    "dp_1",
  ]);
