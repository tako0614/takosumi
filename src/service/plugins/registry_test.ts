...
test("registry exposes registered plugins in registration order", () => {
  ...
  const registry = createTakosumiPluginRegistry([a, b]);
...
test("registry resolves exact kind URI lookup", () => {
  ...
  const registry = createTakosumiPluginRegistry([workerPlugin]);
...
test("registry resolves operator-defined kind URI without normalization", () => {
  ...
  const registry = createTakosumiPluginRegistry([operatorPlugin]);
...
test("registry rejects duplicate plugin name", () => {
  ...
  assert.throws(
    () => createTakosumiPluginRegistry([a, b]),
    /service plugin already registered: @example\/dup/,
  );
});
...
test("registry rejects conflicting kind URI providers", () => {
  ...
  assert.throws(
    () => createTakosumiPluginRegistry([a, b]),
    /service plugin @example\/b conflicts on kind https:\/\/example\.test\/kinds\/v1\/x; @example\/a already provides it/,
  );
});
...
test("findPluginForKind returns undefined when no plugin matches", () => {
  const registry = createTakosumiPluginRegistry([]);
  assert.equal(findPluginForKind(registry, "worker"), undefined);
  assert.equal(
