...
export interface InMemoryRuntimeAgentRegistryOptions {
   ...
   * mirrors its agent + work-item state into the ledger so the service
...
export class InMemoryRuntimeAgentRegistry implements RuntimeAgentRegistry {
   ...
   * service boot path so long-running operations resume across restarts.
   */
  ...
  #leaseWorkLocked(
    ...
    // state across a service restart.
 ...
 * the operator-supplied service-trusted Ed25519 keypair. Operators wire this
...
export interface RuntimeAgentGatewayManifestIssuerOptions {
  ...
  /** Service-trusted Ed25519 private key used to sign manifests. */
