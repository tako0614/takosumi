...
// process memory. If the service restarts mid-flight, every long-running
// provider operation that an agent already leased loses its `leased`
// status and the service forgets it ever started — risking duplicate
 ...
 * a single-node service before the persistent adapter is wired up.
 */
 ...
 *
 * The service calls this once on boot before the registry starts handing
