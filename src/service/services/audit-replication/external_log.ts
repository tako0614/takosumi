 ...
 *     immutable storage) and exposes `readChain` so service boot can verify
...
export interface SelectAuditExternalReplicationSinkOptions {
   ...
   * because the service build does not bundle the AWS SDK.
   */
    ...
    if (!options.s3Port) {
      throw new AuditReplicationConfigurationError(
        "TAKOSUMI_AUDIT_REPLICATION_KIND=s3 requires an S3 port to be wired " +
          "into selectAuditExternalReplicationSink (the service does not " +
          "bundle the AWS SDK)",
      );
    }
    const retentionMode =
