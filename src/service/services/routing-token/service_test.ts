...
function buildService(options?: {
  ...
  const service = new RoutingTokenService({
    secret: options?.secret ?? "service-routing-secret-v1",
...
test("RoutingTokenService: rotation honors previously-issued tokens within the rotation window", async () => {
  ...
  tick(10 * 60 * 1000);
  service.rotate("service-routing-secret-v2");
  const verified = await service.verify({ token: issued.token });
  assert.equal(verified.ok, true);
...
test("RoutingTokenService: rotation rejects re-using the current secret", () => {
  const { service } = buildService({ secret: "service-routing-secret-v1" });
  assert.throws(
    () => service.rotate("service-routing-secret-v1"),
    (err: unknown) => {
      assert.ok(err instanceof DomainError);
      assert.equal(err.code, "conflict");
      return true;
    ...
    assert.throws(
      () =>
        new RoutingTokenService({
          secret: "service-routing-secret-v1",
          maxClockSkewMs: skew,
        }),
      (err: unknown) => {
        assert.ok(err instanceof DomainError);
