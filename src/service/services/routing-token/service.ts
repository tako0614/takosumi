 ...
 * cadence (default 1h). The service injects the active token as the
 ...
 * back into the service (or verify locally with the public secret) before
 * accepting the request.
 *
 * The token is minted with HMAC-SHA256, deliberately matching the existing
 * `takosumi-contract` internal-API signature primitive so the service does not
