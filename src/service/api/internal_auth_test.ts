...
test("verifyInternalResponse accepts a valid Worker -> service response", async () => {
 ...
 * instances can race against — i.e. the test analogue of two Takosumi replicas
...
test("readInternalAuth uses SqlReplayProtectionStore to reject cross-pod replays", async () => {
  // Two Takosumi replicas share the same `internal_request_replay_log` row
