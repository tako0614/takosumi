...
import type { createSpaceDomainServices } from "../domains/space/mod.ts";
...
export function registerInternalRoutes(
  ...
  app.get(TAKOSUMI_INTERNAL_PATHS.spaces, async (c) => {
    ...
    const spaces = await space.spaceQueries.listInternalSpaceSummaries({
      actor: auth.actor,
    });
  ...
  app.post(TAKOSUMI_INTERNAL_PATHS.spaces, async (c) => {
    ...
    const result = await space.spaces.createSpace({
      actor: auth.actor,
      spaceId: request.spaceId,
      name: request.name,
  ...
  app.get(TAKOSUMI_INTERNAL_PATHS.groups, async (c) => {
    ...
    const groups = await space.groupQueries.listGroups({
      actor: auth.actor,
      spaceId,
    });
  ...
  app.post(TAKOSUMI_INTERNAL_PATHS.groups, async (c) => {
    ...
    const result = await space.groups.createGroup({
      actor: auth.actor,
      spaceId: request.spaceId,
      groupId: request.groupId,
