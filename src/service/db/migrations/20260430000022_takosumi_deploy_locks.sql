...
-- Purpose:   Cross-process lease locks for deployment apply paths.
--            Each operation holds one lock per `(tenant_id, name)` while it
--            runs apply / destroy and renews `locked_until` until the
--            operation exits. If the kernel pod dies, a later caller can take
--            the lock after the lease expires.
--
-- Spec:      src/service/domains/deploy/takosumi_deployment_record_store_sql.ts
