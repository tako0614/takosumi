...
test("registry exposes registered implementations in registration order", () => {
  const a = buildImplementation("@example/a", ["https://example.test/kinds/v1/a"]);
  const b = buildImplementation("@example/b", ["https://example.test/kinds/v1/b"]);
...
test("registry resolves exact kind URI lookup", () => {
  ...
  assert.equal(
    findImplementationForKind(registry, "https://takosumi.com/kinds/v1/worker")?.name,
    "@takos/workers-reference",
  );
});

test("registry resolves operator-defined kind URI without normalization", () => {
  const operatorImplementation = buildImplementation(
    "@operator/lambda",
    ["https://operator.example.com/kinds/lambda"],
  );
  const registry = createOperatorImplementationRegistry([operatorImplementation]);

  assert.equal(
    findImplementationForKind(registry, "https://operator.example.com/kinds/lambda")
      ?.name,
    "@operator/lambda",
  );
});
...
test("registry rejects duplicate implementation name", () => {
  const a = buildImplementation("@example/dup", ["https://example.test/kinds/v1/a"]);
  const b = buildImplementation("@example/dup", ["https://example.test/kinds/v1/b"]);
...
test("registry rejects conflicting kind URI providers", () => {
  const a = buildImplementation("@example/a", ["https://example.test/kinds/v1/x"]);
  const b = buildImplementation("@example/b", ["https://example.test/kinds/v1/x"]);
