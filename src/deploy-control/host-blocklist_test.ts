...
test("parseIpv6 expands compressed, full, and zero-padded loopback equally", () => {
  const a = parseIpv6("::1");
...
test("parseIpv6 folds an embedded IPv4 tail", () => {
...
test("parseIpv6 returns null for non-IPv6 and rejects double '::'", () => {
  assert.equal(parseIpv6("example.test"), null);
  assert.equal(parseIpv6("1::2::3"), null);
  assert.equal(parseIpv6("12345::1"), null);
});

test("assertHostNotBlocked blocks loopback in every textual form", () => {
...
test("assertHostNotBlocked blocks NAT64 and 6to4 wrapping internal IPv4", () => {
...
test("assertHostNotBlocked blocks ULA, link-local, multicast, mapped, compat", () => {
...
test("assertHostNotBlocked allows public IP literals and hostnames", () => {
