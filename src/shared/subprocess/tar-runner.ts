/**
 * Shared low-level `tar` subprocess primitive (runtime-detecting).
 *
 * Single source of truth for the runtime-detecting `tar` primitive consumed by
 * both the installer (`installer/subprocess/tar-runner.ts`) and the
 ...
 * installer must not depend on the runtime-agent and the runtime-agent must not
 * depend on the installer. This module imports nothing from either layer and
 ...
 * static graph.
 *
 * Runs `tar` through `Deno.Command` on Deno and through `node:child_process` on
 * Node, piping the archive bytes to stdin and selecting the path at call time.
 * No build-time module mapping is required.
 *
 ...
 * with its decoded stdout. Selects the Deno or Node path at call time via
 * {@link denoCommand}. Throws `tar <args> failed: <stderr>` on a non-zero exit.
 */
