...
Deno.test("output stores current outputs, explicit bindings, and projections", async () => {
  ...
  const projections = new InMemoryOutputProjectionStore();
  ...
  const binding: OutputConsumerBinding = {
    id: "bind_web_docs",
    spaceId: "space_a",
    consumerGroupId: "web",
  ...
  const projection = projectOutput({
  ...
  assert.equal(
    (await outputs.findCurrentByAddress("space_a", "docs/site"))?.id,
    "pub_docs_1",
  );
  assert.deepEqual(
  ...
  assert.deepEqual(
    (await projections.listByOutput("pub_docs_1")).map((item) => item.id),
    ["projection_web_docs"],
  );
});
...
Deno.test("withdrawn outputs are omitted from current lookup by default", async () => {
  ...
  assert.equal(
    await outputs.findCurrentByAddress("space_a", "docs/site"),
    undefined,
  );
  assert.deepEqual((await outputs.list()).map((item) => item.id), []);
  assert.deepEqual(
    (await outputs.list({ includeWithdrawn: true })).map((item) => item.id),
    ["pub_old"],
  );
});
