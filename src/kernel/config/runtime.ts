/**
 * Runtime configuration loader for the Takosumi kernel.
 *
 * The kernel reads only minimal env-derived knobs here: environment
 * (`local` / `development` / `test` / `staging` / `production`) and
 * process role. Adapter / plugin selection is no longer driven by env
 * vars — operators inject adapters and `KernelPlugin[]` directly via
 * `createPaaSApp({ adapters, plugins })`. Legacy backend / adapter
...
async function rejectStaleSelectors(
  ...
  for (const key of STALE_SELECTOR_KEYS) {
    ...
    diagnostics.push({
      ...
      key,
      message:
        `${key} is no longer accepted by the Takosumi kernel; inject adapters / KernelPlugin instances directly via createPaaSApp({ adapters, plugins }) instead`,
    });
  }
}
