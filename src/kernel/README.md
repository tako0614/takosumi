...
Reference kernel for the manifestless Takosumi Installer API. It receives Source install/deploy requests over HTTP,
...
The kernel does not hold cloud provider credentials and does not run Terraform/OpenTofu.
...
`createPaaSApp` builds the Hono app, wires adapter ports, mounts route modules for the configured process role, and
