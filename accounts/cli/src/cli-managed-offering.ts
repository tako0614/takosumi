import {
  managedOfferingDomainIds,
  type ManagedOfferingEvidenceEntry,
  managedOfferingEvidenceEnvironments,
  type ManagedOfferingEvidenceReference,
  type ManagedOfferingEvidenceStatus,
  managedOfferingProductionTopologyKind,
  managedOfferingProductionTopologyMergeReportKind,
  managedOfferingProductionTopologyReportKind,
  managedOfferingPublicSummaryKind,
  managedOfferingPublicSummaryReportKind,
  managedOfferingReadinessKind,
  type ManagedOfferingReadinessReport,
  managedOfferingReadinessReportKind,
  managedOfferingRehearsalEnvironments,
  type ManagedOfferingRehearsalRun,
  managedOfferingRehearsalStepIds,
  managedOfferingRequiredEvidenceTypes,
  managedOfferingStructuredEvidenceRequirements,
...
} from "./cli-managed-offering-constants.ts";
import {
  actorIdentityValue,
  canonicalJson,
  isRecord,
...
export function validateManagedOfferingReadinessDocument(
  document: unknown,
): ManagedOfferingReadinessReport {
  const errors: string[] = [];
  if (!isRecord(document)) {
    return {
      kind: managedOfferingReadinessReportKind,
      ready: false,
      missingDomains: [...managedOfferingDomainIds],
      incompleteDomains: [],
      missingRehearsalSteps: [...managedOfferingRehearsalStepIds],
      incompleteRehearsalSteps: [],
      errors: ["document must be a JSON object"],
    };
  }

  if (document.kind !== managedOfferingReadinessKind) {
    errors.push(`kind must be ${managedOfferingReadinessKind}`);
  }
  ...
  const rehearsalRunResult = validateManagedOfferingRehearsalRun(
  ...
  const report: ManagedOfferingReadinessReport = {
    kind: managedOfferingReadinessReportKind,
...
export function buildManagedOfferingReadinessTemplate(): Record<
  string,
  unknown
> {
  const templateEntry = (scope: "domains" | "rehearsal", id: string) => ({
    id,
    status: "blocked" satisfies ManagedOfferingEvidenceStatus,
    owner: "",
    environment: "",
    reviewer: "",
    completedAt: "",
    evidence: requiredEvidenceTypesFor(scope, id).map((type) =>
      buildManagedOfferingEvidenceTemplateReference(scope, id, type)
    ),
  });
  return {
    kind: managedOfferingReadinessKind,
    rehearsalRun: {
      id: "",
      environment: "",
      owner: "",
    ...
    domains: managedOfferingDomainIds.map((id) => ({
      ...templateEntry("domains", id),
      requiredEvidenceTypes: requiredEvidenceTypesFor("domains", id),
    })),
    rehearsal: managedOfferingRehearsalStepIds.map((id) => ({
...
function buildManagedOfferingEvidenceTemplateReference(
  scope: "domains" | "rehearsal",
  id: string,
  type: string,
): Record<string, unknown> {
  const requirement = managedOfferingStructuredEvidenceRequirements[type];
  const reference: Record<string, unknown> = {
    type,
    ref: `vault://managed-readiness/<rehearsal-run-id>/${scope}/${id}/${type}`,
    summary: "",
    private: true,
    publicSummary: "",
  };
  ...
  for (
    const [field, allowedValues] of Object.entries(
  ...
  for (const field of requirement?.fields ?? []) {
    if (reference[field] === undefined) {
      reference[field] = managedOfferingTemplateFieldValue(field);
    }
  }
...
function managedOfferingTemplateFieldValue(field: string): unknown {
  ...
  if (field.endsWith("Ref")) return `vault://managed-readiness/<${field}>`;
  if (
    ["completedAt", "reviewedAt", "verifiedAt", "windowStart", "windowEnd"]
      .includes(field)
  ) {
...
function validateManagedOfferingRehearsalRun(
  value: unknown,
): {
  runId: string | null;
  ...
  const run = value as ManagedOfferingRehearsalRun;
  const runId = typeof run.id === "string" && run.id.trim().length > 0
    ? run.id.trim()
    : null;
  const environment = typeof run.environment === "string" &&
  ...
  if (
    environment &&
    !isAllowedManagedOfferingEnvironment(
      environment,
      managedOfferingRehearsalEnvironments,
    )
  ) {
...
function validateEvidenceEntries(
  ...
  const seen = new Map<string, ManagedOfferingEvidenceEntry>();
  ...
  for (const [index, rawEntry] of value.entries()) {
    ...
    const entry = rawEntry as ManagedOfferingEvidenceEntry;
    ...
    const environment = typeof entry.environment === "string"
      ? entry.environment.trim()
      : "";
    const allowedEnvironments = fieldName === "rehearsal"
      ? managedOfferingRehearsalEnvironments
      : managedOfferingEvidenceEnvironments;
    const environmentMessage = fieldName === "rehearsal"
    ...
    if (Array.isArray(entry.evidence)) {
      ...
      if (required.has(entry.id)) {
        ...
        for (
          const type of unexpectedEvidenceTypes(
            entry.evidence,
  ...
  if (fieldName === "rehearsal") {
    validateManagedOfferingRehearsalOrder(seen, requiredIds, errors);
  }
...
function validateManagedOfferingRehearsalOrder(
  entries: Map<string, ManagedOfferingEvidenceEntry>,
...
function requiredEvidenceTypesFor(
  ...
  return (managedOfferingRequiredEvidenceTypes[fieldName] as Record<
    string,
    readonly string[]
  >)[id] ?? [];
}
...
function isCompleteEvidenceEntry(
  entry: ManagedOfferingEvidenceEntry,
  rehearsalRun?: {
    runId: string | null;
    environment: string | null;
    startedAt: Date | null;
  ...
  const reviewer = typeof entry.reviewer === "string"
    ? entry.reviewer.trim()
    : "";
  ...
  if (
    entry.id === "production-topology" && environment !== "staging+production"
  ) {
    return false;
  }
...
function isCompleteEvidenceReference(value: unknown): boolean {
  ...
  // `ManagedOfferingEvidenceReference` declares every field as `unknown`, so
  ...
  const reference: ManagedOfferingEvidenceReference = value;
  if (
  ...
  if (managedOfferingEvidenceSummaryErrors(reference.summary).length > 0) {
    return false;
  }
  ...
  return typeof reference.publicSummary === "string" &&
    managedOfferingPublicSummaryErrors(reference.publicSummary, {
...
function hasValidStructuredEvidenceCrossFieldShape(
  ...
  switch (type) {
    case "stripe-sandbox":
      return hasStripeId(reference.checkoutSessionId, "cs_test_") &&
        hasStripeId(reference.webhookEventId, "evt_");
    case "stripe-live":
      return hasStripeId(reference.checkoutSessionId, "cs_live_") &&
        hasStripeId(reference.webhookEventId, "evt_");
    case "invoice":
    ...
    case "load-test":
      return typeof reference.tenantCount === "number" &&
        reference.tenantCount >= 2 &&
        stringValue(reference.tenantAInstallationId) !==
          stringValue(reference.tenantBInstallationId);
...
function hasRequiredDataClasses(
  ...
  const classes = Array.isArray(value)
    ? value.map((item) => stringValue(item)).filter((item): item is string =>
      item !== undefined
    )
    : typeof value === "string"
...
function hasRequiredEvidenceTypes(
  ...
  const completeTypes = new Set(
    evidence
      ...
      .map((item) => (item as ManagedOfferingEvidenceReference).type)
...
function isAllowedManagedOfferingEnvironment(
  environment: string,
...
export function formatManagedOfferingReadinessReport(
  report: ManagedOfferingReadinessReport,
): string {
  const lines = [
    "Managed offering launch readiness evidence is incomplete.",
  ];
  ...
  if (report.incompleteDomains.length > 0) {
    lines.push(
      `Incomplete P0 domains: ${report.incompleteDomains.join(", ")}`,
    );
  }
  ...
  if (report.incompleteRehearsalSteps.length > 0) {
    lines.push(
      `Incomplete rehearsal steps: ${
        report.incompleteRehearsalSteps.join(", ")
      }`,
    );
  }
...
export function defaultManagedOfferingPublicSummary(ready: boolean): string {
  return ready
    ? "P0 evidence and one staged launch rehearsal passed validator checks; operator approval is still required."
    : "Managed offering launch readiness remains blocked because validator checks have not passed.";
}

export function buildManagedOfferingPublicSummary(input: {
  document: unknown;
  report: ManagedOfferingReadinessReport & { evidenceDigest: string };
  evidenceRefClass: string | null;
  publicSummary: string;
}): Record<string, unknown> {
  const rehearsalRun = isRecord(input.document) &&
      isRecord(input.document.rehearsalRun)
    ? input.document.rehearsalRun
    : {};
  const completedAt = stringValue(rehearsalRun.completedAt);
  return {
    kind: managedOfferingPublicSummaryKind,
...
export function formatManagedOfferingPublicSummaryMarkdownRow(
...
export function validateManagedOfferingPublicSummaryArtifact(
  summary: unknown,
  readinessDocument: unknown,
  readinessReport: ManagedOfferingReadinessReport & { evidenceDigest: string },
): Record<string, unknown> & { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!isRecord(summary)) {
    return {
      kind: managedOfferingPublicSummaryReportKind,
      valid: false,
      ready: false,
  ...
  if (summary.kind !== managedOfferingPublicSummaryKind) {
    errors.push(`kind must be ${managedOfferingPublicSummaryKind}`);
  }
  ...
  if (!publicResult) {
    ...
    errors.push(
      ...managedOfferingPublicSummaryErrors(publicResult, {
        requireLaunchScope: ready,
      }),
    );
  }
  errors.push(
    ...managedOfferingSummaryRedactionErrors(
      JSON.stringify(summary),
      "managed offering public summary artifact",
    ),
  );

  const privateEvidenceRefClass = summary.privateEvidenceRefClass;
  const hasEvidenceRefClass = privateEvidenceRefClass !== null &&
    privateEvidenceRefClass !== undefined;
  if (
    hasEvidenceRefClass &&
  ...
  const rehearsalRun = isRecord(readinessDocument) &&
      isRecord(readinessDocument.rehearsalRun)
...
export function validateProductionTopologyDocument(
  ...
  if (!isRecord(document)) {
    return {
      kind: managedOfferingProductionTopologyReportKind,
      ready: false,
      environment: null,
  ...
  if (document.kind !== managedOfferingProductionTopologyKind) {
    errors.push(`kind must be ${managedOfferingProductionTopologyKind}`);
  }
  ...
  const roles = new Set(
    components.map((component) => stringValue(component.role)).filter(
      (role): role is string => Boolean(role),
    ),
  );
  const missingRoles = productionTopologyRequiredRoles.filter((role) =>
    !roles.has(role)
  );
  ...
  const accountsComponent = components.find((component) =>
    stringValue(component.role) === "accounts"
  );
...
function accountsWorkerSubstrateErrors(
  ...
  if (component.containerRuntime !== false) {
    errors.push("accounts component containerRuntime must be false");
  }
  ...
  errors.push(
    ...accountsWorkerConfigValidationErrors(
      component.wranglerConfigValidation,
    ),
  );
  const bindings = Array.isArray(component.bindings)
    ? component.bindings.filter((item): item is string =>
      typeof item === "string"
    )
    : [];
...
function accountsWorkerConfigValidationErrors(value: unknown): string[] {
  ...
  if (
    report.kind !== "takosumi.cloudflare-rendered-config-validation@v1"
  ) {
    errors.push(
    ...
    evidence: [
      {
        ...
        summary:
          `Production topology manifest declares ${components.length} required components for ${input.environment}.`,
        private: true,
        publicSummary:
          `Production topology manifest shape was reviewed for ${input.environment}.`,
      ...
      },
      {
        ...
        summary:
          `${deployableCount} deployable topology components are pinned by sha256 artifact digest.`,
        private: true,
        publicSummary:
          `Deployable topology components use immutable artifact digests for ${input.environment}.`,
        topologyEnvironment: input.environment,
      ...
      },
      {
        ...
        private: true,
        publicSummary:
          `Health probe evidence covers every topology component in ${input.environment}.`,
        topologyEnvironment: input.environment,
      ...
      },
      {
        ...
        private: true,
        publicSummary:
          `Public hostname TLS evidence was attached for ${input.environment}.`,
        topologyEnvironment: input.environment,
      ...
      },
      {
        ...
        private: true,
        publicSummary:
          `Rollback target evidence uses an immutable digest for ${input.environment}.`,
        topologyEnvironment: input.environment,
...
export function buildProductionTopologyTemplate(
  ...
  return {
    kind: managedOfferingProductionTopologyKind,
    environment,
    owner: "",
    reviewer: "",
    completedAt: "",
    manifestRef:
      `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/manifest`,
    migrationTranscriptRef:
      `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/migration-transcript`,
    tlsEvidenceRef:
      `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/tls`,
    artifactDigestEvidenceRef:
      `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/artifact-digests`,
    healthProbeEvidenceRef:
      `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/health-probes`,
    rollbackTarget: {
      ref:
        `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/rollback-target`,
      role: "accounts",
    ...
    components: productionTopologyRequiredRoles.map((role) => ({
      ...
      role,
      healthProbeRef:
        `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/${role}/health-probe`,
      ...(role === "accounts"
        ? {
          runtime: "cloudflare-worker",
          containerRuntime: false,
          wranglerConfigRef:
            `vault://managed-readiness/<rehearsal-run-id>/production-topology/${environment}/accounts/wrangler-config`,
...
export function mergeProductionTopologyPreflightReports(
  ...
  if (!stagingEntry || !productionEntry) {
    return {
      kind: managedOfferingProductionTopologyMergeReportKind,
      ready: false,
      errors,
    };
  }
...
function productionTopologyEntryFromPreflightReport(
  ...
  if (report.kind !== managedOfferingProductionTopologyReportKind) {
    errors.push(
      `${label}.kind must be ${managedOfferingProductionTopologyReportKind}`,
    );
  }
  ...
  const actualTypes = Array.isArray(entry.evidence)
    ? entry.evidence.map((item) =>
      isRecord(item) ? stringValue(item.type) : undefined
...
export async function managedOfferingReadinessDigest(
  document: unknown,
): Promise<string> {
  return `sha256:${await sha256Hex(canonicalJson(document))}`;
}

export function managedOfferingEvidenceSummaryErrors(value: string): string[] {
  const summary = value.trim();
  const errors: string[] = [];
  if (summary.length < 20) {
    errors.push(
      "managed offering evidence summary must be at least 20 characters",
    );
  }
  if (isPlaceholderEvidenceRef(summary)) {
    errors.push("managed offering evidence summary must not be a placeholder");
  }
  errors.push(
    ...managedOfferingSummaryRedactionErrors(
      summary,
      "managed offering evidence summary",
    ),
  );
  return errors;
}

export function managedOfferingPublicSummaryErrors(
  value: string,
...
export function managedOfferingSummaryRedactionErrors(
  summary: string,
  label: string,
): string[] {
  const errors: string[] = [];
