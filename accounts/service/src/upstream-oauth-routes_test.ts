...
import { handleAuthProvidersRequest } from "./upstream-oauth-routes.ts";
import type { UpstreamOAuthOptions } from "./mod.ts";
...
test("handleAuthProvidersRequest marks configured upstream providers enabled", async () => {
  ...
  expect(body.providers).toContainEqual({ id: "github", enabled: false });
