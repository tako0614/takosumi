...
test("InMemoryAccountsStore persists accounts and upstream identity links", () => {
  ...
  expect(store.findAccount("tsub_test")?.termsVersion).toEqual("terms-2026-05-13");
  expect(store.findAccount("tsub_test")?.termsAcceptedAt).toEqual(1000);
  expect(store.findUpstreamIdentity({
      providerId: "github",
      upstreamIssuer: "https://github.com",
      upstreamSubject: "12345",
    })?.subject).toEqual("tsub_test");
});
...
test("InMemoryAccountsStore indexes passkey credentials by subject", () => {
  ...
  expect(store.findPasskeyCredential("credential-1")?.subject).toEqual("tsub_test");
  expect(store.listPasskeyCredentialsForSubject("tsub_test").map((credential) =>
...
test("InMemoryAccountsStore manages personal access token metadata", () => {
  ...
  expect(store.findPersonalAccessToken("takpat_secret")?.tokenId).toEqual("pat_1");
  expect(store.listPersonalAccessTokensForSubject("tsub_test").map((token) =>
      token.name
    )).toEqual(["CLI"]);
  store.recordPersonalAccessTokenUsed("pat_1", 1500);
  expect(store.findPersonalAccessToken("takpat_secret")?.lastUsedAt).toEqual(1500);
  expect(store.revokePersonalAccessToken({
      subject: "tsub_test",
      tokenId: "pat_1",
      revokedAt: 1600,
...
test("InMemoryAccountsStore persists billing usage records per installation", () => {
  ...
  expect(store.listBillingUsageRecordsForInstallation("inst_1").map((record) => ({
...
test("InMemoryAccountsStore indexes billing accounts by subject and Stripe customer", () => {
  ...
  expect(store.findBillingAccountForSubject("tsub_test")?.stripeCustomerId).toEqual("cus_123");
  expect(store.findBillingAccountByStripeCustomerId("cus_123")?.billingAccountId).toEqual("billing-1");
  expect(store.findBillingAccount("billing-1")?.lastCreditId).toEqual("re_123");
  expect(store.findBillingAccount("billing-1")?.dunningAction).toEqual("marked_uncollectible");
  expect(store.findBillingAccount("billing-1")?.lastPlanToCode).toEqual("pro");
  expect(store.findBillingAccount("billing-1")?.taxAutomaticStatus).toEqual("complete");
});

test("InMemoryAccountsStore validates AppBinding records before saving", () => {
  const store = new InMemoryAccountsStore();
...
test("InMemoryAccountsStore persists AppInstallation ledger records", () => {
  ...
  expect(store.findLedgerAccount("acct_1")?.billingAccountId).toEqual("billing-1");
  expect(store.listSpacesForAccount("acct_1")[0]?.spaceId).toEqual("space_1");
  expect(store.listAppInstallationsForSpace("space_1")[0]?.installationId).toEqual("inst_1");
  expect(store.listAppInstallationsForSpace("space_1")[0]?.billingAccountId).toEqual("billing-inst-1");
  expect(store.listAppBindingsForInstallation("inst_1")[0]?.kind).toEqual("identity.oidc@v1");
...
test("InMemoryAccountsStore appends InstallationEvent records in order", () => {
  ...
  expect(store.listInstallationEvents("inst_1").map((event) => event.eventId)).toEqual(["evt_1", "evt_2"]);
});
...
test("InMemoryAccountsStore prunes expired and used launch tokens", () => {
  ...
  expect(store.pruneLaunchTokens({ expiredBefore: 2_500, usedBefore: 3_500 })).toEqual({ deleted: 2, expired: 1, used: 1 });
  expect(store.consumeLaunchToken({
      tokenHash: "sha256:expired",
      installationId: "inst_expired",
...
test("InMemoryAccountsStore indexes OIDC clients by installation", () => {
  ...
  expect(store.findOidcClientForInstallation("inst_1")?.clientId).toEqual("toc_client");
});
...
test("InMemoryAccountsStore rejects a ledger ownership change (consistent with D1/PG)", () => {
  ...
  expect(store.findLedgerAccount("acct_1")?.legalOwnerSubject).toEqual("tsub_alice");
