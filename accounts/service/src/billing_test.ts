...
test("normalizeStripeBillingEvent maps known Stripe event shapes", () => {
  expect(normalizeStripeBillingEvent(stripeEvent({
      id: "evt_checkout",
...
test("applyStripeBillingEvent suspends entitlements on chargeback and restores when dispute is won",
  async () => {
    ...
    expect(store.findAppInstallation("inst_dispute")?.status).toEqual("suspended");
...
test("customer.subscription.deleted captures cancellation reason details",
  async () => {
    const store = new InMemoryAccountsStore();
    store.saveAccount({
      subject: "tsub_account",
...
test("applyStripeBillingEvent updates billing account state from Stripe events", async () => {
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.stripeSubscriptionId).toEqual("sub_1");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.stripePriceId).toEqual("price_plus");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.planCode).toEqual("plus");
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.currentPeriodEndUnix).toEqual(1_700_100_000);
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastInvoiceId).toEqual("in_paid");
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.status).toEqual("past_due");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastInvoiceId).toEqual("in_failed");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.dunningStartedAt).toEqual(4000);
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.nextPaymentAttemptUnix).toEqual(1_700_300_000);
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.dunningAttemptCount).toEqual(1);
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.dunningAction).toEqual("retry_scheduled");
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.dunningAttemptCount).toEqual(2);
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.nextPaymentAttemptUnix).toEqual(1_700_400_000);
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.status).toEqual("unpaid");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.dunningAction).toEqual("marked_uncollectible");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.dunningExhaustedAt).toEqual(4300);
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastCreditKind).toEqual("credit_note");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastCreditId).toEqual("cn_recovery");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastCreditAmount).toEqual(500);
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastTaxEventId).toEqual("evt_tax");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.taxPolicyRef).toEqual("tax-policy://us-sales-tax");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.taxJurisdiction).toEqual("US");
  ...
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.status).toEqual("paused");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.currentPeriodEndUnix).toEqual(1_700_200_000);
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.stripePriceId).toEqual("price_pro");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.planCode).toEqual("pro");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")
      ?.lastPlanTransitionEventId).toEqual("evt_updated");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastPlanFromCode).toEqual("plus");
  expect(store.findBillingAccountByStripeCustomerId("cus_1")?.lastPlanToCode).toEqual("pro");
...
test("applyStripeBillingEvent reconciles billing entitlements into AppInstallation status", async () => {
  ...
  expect(store.listInstallationEvents("inst_paid").map((event) => event.eventType)).toEqual([
  ...
  expect(paid.applied ? paid.entitlementReconciliation.restoredInstallationIds : []).toEqual(["inst_paid"]);
...
test("reconcileBillingEntitlements does not restore non-billing suspensions", async () => {
  ...
  expect(store.findAppInstallation("inst_manual_suspend")?.status).toEqual("suspended");
});
...
test("handleStripeWebhook marks unhandled and ownerless events without retrying", async () => {
  const store = new InMemoryAccountsStore();
  const unknownPayload = JSON.stringify(stripeEvent({
    id: "evt_unknown",
    type: "customer.created",
    object: { id: "cus_1" },
  }));
  ...
  const ownerlessPayload = JSON.stringify(stripeEvent({
    id: "evt_ownerless",
    type: "checkout.session.completed",
    object: {
      customer: "cus_1",
  ...
  expect(store.findBillingWebhookEvent("evt_unknown")?.status).toEqual("skipped");
  expect(ownerless.status).toEqual("failed");
  expect(ownerless.errorMessage).toEqual("unknown_account");
  expect(store.findBillingWebhookEvent("evt_ownerless")?.status).toEqual("failed");
});
