...
import { applyStripeBillingEvent } from "./billing-apply.ts";
import {
  type AccountsStore,
...
test("applyStripeBillingEvent invoice.paid clears dunning state after payment_failed",
  async () => {
    ...
    const afterFailed = store.findBillingAccountByStripeCustomerId(
      "cus_pay_recovery",
    );
    ...
    const afterPaid = store.findBillingAccountByStripeCustomerId(
      "cus_pay_recovery",
    );
...
test("applyStripeBillingEvent rejects checkout missing subject or customer",
  async () => {
    const store = new InMemoryAccountsStore();
...
test("applyStripeBillingEvent rejects non-checkout events for unknown Stripe customers",
  async () => {
    const store = new InMemoryAccountsStore();
...
test("applyStripeBillingEvent treats checkout without payment status or subscription as incomplete",
  async () => {
    const store = new InMemoryAccountsStore();
    store.saveAccount({
      subject: "tsub_account",
...
test("applyStripeBillingEvent serializes concurrent different events without losing field updates (G15)",
  async () => {
    ...
    const stored = await store.findBillingAccountByStripeCustomerId(
      "cus_concurrent",
    );
...
test("applyStripeBillingEvent resolves a realistic dispute (no top-level customer) and freezes entitlements",
  async () => {
    const store = new InMemoryAccountsStore();
    store.saveAccount({
      subject: "tsub_account",
...
test("applyStripeBillingEvent fails closed when a dispute owner cannot be resolved",
  async () => {
    const store = new InMemoryAccountsStore();
    const result = await applyStripeBillingEvent({
      store,
...
test("applyStripeBillingEvent skips an unknown subscription status without suspending",
  async () => {
    const store = new InMemoryAccountsStore();
    store.saveAccount({
      subject: "tsub_account",
    ...
    expect(store.findBillingAccountByStripeCustomerId("cus_unknown_status")?.status).toEqual("active");
  },
);

test("applyStripeBillingEvent subscription_updated recovery clears stale dunning state",
  async () => {
    const store = new InMemoryAccountsStore();
    store.saveAccount({
      subject: "tsub_account",
...
test("applyStripeBillingEvent create path returns version 1 and a deterministic id",
  async () => {
    const store = new InMemoryAccountsStore();
    store.saveAccount({
      subject: "tsub_account",
    ...
    expect(stored?.billingAccountId).toEqual(result.billingAccount.billingAccountId);
    ...
    expect(second.billingAccount.billingAccountId).toEqual(result.billingAccount.billingAccountId);
