...
export function accountsMigrateD1HelpText(): string {
  return [
    "takosumi accounts migrate-d1 plan|apply|verify|status|backup-status",
    "",
    "Owner CLI for the checksummed Takosumi Accounts D1 migration catalog.",
    "Remote reads and atomic per-migration batches use the Cloudflare D1 REST API.",
    "",
    "Options:",
    "  --environment staging|production",
    "  --account-id <id>      Explicit Cloudflare account ID (never printed)",
    "  --database-id <id>     Explicit D1 database ID (never printed)",
    "  --source-commit <40hex>  Must equal the clean owning checkout observed by hardened Git before any external I/O",
    "  --backup-evidence-digest sha256:<hex>  Required by apply; owner-private evidence binding",
    "  --confirm-source-digest sha256:<hex>   Required by apply",
    "  --confirm-catalog-digest sha256:<hex>  Required by apply",
    "  --confirm-target-digest sha256:<hex>   Required by apply",
    "  --confirm-configuration-digest sha256:<hex>  Required by apply; binds backup and migration policy",
    "  --out <private-file>   Required by backup-status; absolute, new, outside source checkouts under owner 0700",
    "  --dry-run              Legacy alias for plan; performs zero remote calls",
    "",
    "Environment:",
    "  CLOUDFLARE_API_TOKEN   Required by apply/verify/status/backup-status; never printed",
    "",
    "plan performs zero remote calls. verify/status/backup-status are read-only.",
    "backup-status atomically writes the raw bookmark only to a new owner 0600 private file.",
    "each backfill chunk atomically re-fences exact v3; clean v4 is adopted read-only.",
    "apply performs bounded v3 activationDigest backfill, then sends one atomic D1",
    "batch per pending migration and never retries blindly.",
  ].join("\n");
}
...
export function launchReadinessValidateHelpText(): string {
  return [
    ...
    "  --contribution-file <path>  Trusted canonical contribution JSON selected by the document",
    "  --json",
    "",
...
export function launchReadinessPublicSummaryValidateHelpText(): string {
  return [
    ...
    "  --contribution-file <path>  Trusted canonical contribution JSON selected by the readiness file",
    "  --json",
    "",
...
export function launchReadinessOidcAccountSecurityEvidenceHelpText(): string {
  return [
    ...
    "  --overlap-jwks-file <path>       Captured JWKS while old/new signing keys overlap",
    "  --post-revocation-jwks-file <path>  Captured JWKS after previous-key removal",
    "  --rotation-log-file <path>       Structured identity-security rotation run log",
    "  --ref-prefix <ref>               Private evidence ref prefix",
    "  --json",
    "",
    "Merges OIDC account-security and secret-rotation-run-log evidence only",
    "after verifying public-only overlap/post-revocation JWKS snapshots and a",
    "strict takosumi.identity-security-rotation-log@v1 operator artifact.",
  ].join("\n");
}
...
export function launchReadinessTemplateHelpText(): string {
  return [
    ...
    "The full selected contribution definition is copied into the @v2 document.",
    "That copy is not authority: later validation requires the same owner-controlled",
    "--contribution-file and rejects any non-exact embedded content.",
