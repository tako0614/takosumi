...
Single entry point for deploying the three Takosumi-public properties to
...
DNS records for `takosumi.com`, `www.takosumi.com`, `docs.takosumi.com` and
`cloud.takosumi.com` are created automatically when you attach each custom
domain in the Cloudflare dashboard (Pages → Custom domains or Workers → Custom
domains).
...
**Source**: `takosumi/site/` (`index.html` + `build.sh` → `./dist/`).
...
- **Pages** (`takosumi.com`, `docs.takosumi.com`): redeploy the previous git
  commit via `deno task site:deploy` / `deno task docs:deploy`. Cloudflare Pages
  also keeps deployment history in the dashboard for one-click rollback.
- **Worker** (`cloud.takosumi.com`): `wrangler rollback` or redeploy the
  previous commit. D1 state is preserved across rollbacks.

## CI

Push-to-deploy is intentionally not wired here — operators run the deno tasks
from a workstation. To add GitHub Actions later, wrap the same tasks with
`CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` exposed as repo secrets; the
deploy commands themselves do not change.
